...
pub struct HeaderBar<'a, Message> {
    ...
    /// Custom height override (None = auto-computed from density).
    #[setters(skip)]
    custom_height: Option<f32>,

    /// Custom padding override (None = auto-computed from density).
    #[setters(skip)]
    custom_padding: Option<[u8; 4]>,
}
...
impl<'a, Message: Clone + 'static> HeaderBar<'a, Message> {
    ...
    /// Définit une hauteur personnalisée pour la header bar.
    ///
    /// Si non défini, la hauteur est calculée automatiquement selon la densité.
    /// Permet de créer des header bars plus compactes ou plus larges.
    #[must_use]
    ...
    /// Définit un padding personnalisé pour la header bar.
    ///
    /// Format: `[top, right, bottom, left]`.
    /// Si non défini, le padding est calculé automatiquement selon la densité.
    #[must_use]
    pub fn custom_padding(mut self, padding: [u8; 4]) -> Self {
...
impl<'a, Message: Clone + 'static> HeaderBar<'a, Message> {
    ...
    pub fn view(mut self) -> Element<'a, Message> {
        ...
        let padding = if let Some(custom) = self.custom_padding {
            [custom[0] as f32, custom[1] as f32, custom[2] as f32, custom[3] as f32]
        } else if is_ssd {
            [2.0, 8.0, 2.0, 8.0]
        } else {
            match (
                ...
                (Density::Compact, true) => [4.0, 8.0, 4.0, 8.0],
                (Density::Compact, false) => [3.0, 7.0, 4.0, 7.0],
                (_, true) => [8.0, 8.0, 8.0, 8.0],
                (_, false) => [7.0, 7.0, 8.0, 7.0],
            }
        };

        let header_height = self.custom_height.unwrap_or(32.0 + padding[0] + padding[2]);
        ...
        let mut widget = HeaderBarWidget::new(start, center, end)
            ...
            .padding(iced::Padding::from([padding[0] as u16, padding[1] as u16, padding[2] as u16, padding[3] as u16]))
