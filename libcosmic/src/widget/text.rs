...
/// Creates a [`Text`] widget with a specific color.
///
/// This bypasses the `.color()` method which requires a `Copy`-compatible style
/// class. Instead, it directly sets the class to `Text::Color(color)`.
pub fn colored<'a>(
    text: impl Into<Cow<'a, str>> + 'a,
    color: iced_core::Color,
) -> Text<'a, crate::Theme, Renderer> {
    use crate::theme::style::Text as TextStyle;
    iced::widget::Text::new(text.into())
        .font(crate::font::default())
        .class(TextStyle::Color(color))
}
