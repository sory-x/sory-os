...
$(BUILD)/chroot: $(BUILD)/debootstrap
	...
	# Copy GPG public key for SoryOS repository
	sudo cp "$(realpath ../soryos-apt/keyrings/soryos-archive-keyring.gpg)" "$@.partial/iso/soryos-archive-keyring.gpg" 2>/dev/null || true

	# Clean APT sources
	sudo truncate --size=0 "$@.partial/etc/apt/sources.list"

	# Temporarily set apt preferences
	sudo cp "data/apt-preferences" "$@.partial/etc/apt/preferences.d/soryos-iso"
	...
	# Run chroot script
	sudo $(CHROOT) "$@.partial" /bin/bash -e -c \
		"KEY=\"/iso/soryos-archive-keyring.gpg\" \
		UPDATE=1 \
		UPGRADE=1 \
		INSTALL=\"$(DISTRO_PKGS)\" \
		LANGUAGES=\"$(LANGUAGES)\" \
	...
	sudo rm "$@.partial/etc/apt/preferences.d/soryos-iso"
