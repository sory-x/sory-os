...
# Pop!_OS upstream dev repos (pour référence, non utilisé par SoryOS)
DEV_REPOS = ()

# Repos SoryOS (org sory-x)
SORYOS_REPOS = (
    "soryos-apt",
    "soryos-iso",
)
...
def foreach_soryos_repo(fn, selected=[]):
    selected = [item.rstrip('/') for item in selected]

    repos = github("https://api.github.com/orgs/sory-x/repos")
