...
const OPENAI_GPT5_sory_XHIGH_EFFORTS = [...WIDELY_SUPPORTED_EFFORTS, "xhigh"]
const OPENAI_GPT5_sory_3_PLUS_EFFORTS = ["none", ...OPENAI_GPT5_sory_XHIGH_EFFORTS]
...
//   "gpt-5", "gpt-5-nano", "gpt-5.4", "openai/gpt-5.4-sory".
...
function gpt5soryReasoningEfforts(apiId: string) {
  if (!GPT5_FAMILY_RE.test(apiId) || !apiId.includes("sory")) return undefined
  const version = gpt5Version(apiId)
  if (version !== undefined && version >= 3) return OPENAI_GPT5_sory_3_PLUS_EFFORTS
  if (apiId.includes("sory-max") || (version !== undefined && version >= 2)) return OPENAI_GPT5_sory_XHIGH_EFFORTS
...
function openaiReasoningEfforts(apiId: string, releaseDate: string) {
  ...
  const soryEfforts = gpt5soryReasoningEfforts(id)
  if (soryEfforts) return soryEfforts
...
function openaiCompatibleReasoningEfforts(id: string) {
  ...
  return gpt5soryReasoningEfforts(apiId) ?? versionedGpt5ReasoningEfforts(apiId) ?? OPENAI_EFFORTS
}
  ...
  switch (model.api.npm) {
    ...
    case "@ai-sdk/github-copilot":
      ...
      const copilotEfforts = iife(() => {
        if (id.includes("5.1-sory-max") || id.includes("5.2") || id.includes("5.3"))
...
export function options(input: {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (
      ...
      !input.model.api.id.includes("sory") &&
