...
describe("plugin.loader.shared", () => {
  ...
  it.live("skips legacy sory and copilot auth plugin specs", () =>
    withTmp(
      async (dir) => {
        await Bun.write(
          path.join(dir, "opencode.json"),
          JSON.stringify(
            {
              plugin: ["opencode-openai-sory-auth@1.0.0", "opencode-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
      ...
      (_tmp) =>
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(pkgs).not.toContain("opencode-openai-sory-auth@1.0.0")
