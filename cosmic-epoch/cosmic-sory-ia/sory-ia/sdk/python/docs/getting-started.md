...
- Python `>=3.10`
- uv
- installed `openai-sory-cli-bin` runtime package, or an explicit `sory_bin` override

## 2) Authenticate when needed

Existing sory auth state is reused automatically. To authenticate from the SDK,
...
with sory() as sory:
    ...
    thread = sory.thread_start(model="gpt-5.4", config={"model_reasoning_effort": "high"})
...
- `sory()` started and initialized `sory app-server`.
...
Use `async with Asyncsory()` as the normal async entrypoint. `Asyncsory`
...
async def main() -> None:
    async with Asyncsory() as sory:
        thread = await sory.thread_start(model="gpt-5.4", config={"model_reasoning_effort": "high"})
...
with sory() as sory:
    thread = sory.thread_resume(THREAD_ID)
...
```python
from openai_sory.types import ThreadReadResponse, Turn, TurnStatus
```
