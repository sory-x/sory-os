# OpenAI sory SDK — API Reference

Public surface of `openai_sory` for app-server v2.
...
- Version: `openai_sory.__version__`
- Requires Python >= 3.10
- Public app-server value and event types live in `openai_sory.types`

## sory (sync)

```python
sory(config: AppServerConfig | None = None)
```
...
## Asyncsory (async parity)

```python
Asyncsory(config: AppServerConfig | None = None)
```
...
`Asyncsory` initializes lazily. Context entry is the standard path because it
...
- one `sory` instance can stream multiple active turns concurrently
...
- `stream()` and `run()` consume only notifications for their own turn ID
- one `Asyncsory` instance can stream multiple active turns concurrently
