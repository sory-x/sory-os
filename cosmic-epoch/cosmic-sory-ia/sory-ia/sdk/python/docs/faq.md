...
- `sory` is the sync public API.
- `Asyncsory` is an async replica of the same public API shape.
- Prefer `async with Asyncsory()` for async code. It is the standard path for
  explicit startup/shutdown, and `Asyncsory` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `sory`.
...
`sory()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- published runtime package (`openai-sory-cli-bin`) is not installed
- local `sory_bin` override points to a missing file
