...
- `sync.py` (public sync surface: `sory`)
- `async.py` (public async surface: `Asyncsory`)

All examples intentionally use only public SDK exports from `openai_sory`
and `openai_sory.types`.
...
uses the installed `openai-sory-cli-bin` runtime package.

If the pinned `openai-sory-cli-bin` runtime is not already installed, the bootstrap
will download the matching GitHub release artifact, stage a temporary local
`openai-sory-cli-bin` package, install it into your active interpreter, and clean up
...
active interpreter and an installed `openai-sory-cli-bin` runtime package (either
