...
with sory(config=runtime_config()) as sory:
    ...
    reopened = sory.thread_resume(thread.id)
    listing_active = sory.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = sory.thread_archive(reopened.id)
    listing_archived = sory.thread_list(limit=20, archived=True)
    unarchived = sory.thread_unarchive(reopened.id)

    resumed = sory.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = sory.thread_fork(unarchived.id, model="gpt-5.4")
