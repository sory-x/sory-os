...
SDK_DISTRIBUTION_NAME = "openai-sory"
RUNTIME_DISTRIBUTION_NAME = "openai-sory-cli-bin"
...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "sory_app_server_protocol.v2.schemas.json"
...
def runtime_binary_name() -> str:
    return "sory.exe" if _is_windows() else "sory"


def staged_runtime_bin_path(root: Path) -> Path:
    return root / "src" / "sory_cli_bin" / "bin" / runtime_binary_name()


def staged_runtime_resource_path(root: Path, resource: Path) -> Path:
    """Stage runtime helper binaries beside the main bundled sory binary."""
    # Runtime wheels include the whole bin/ directory, so helper executables
    # should be staged beside the main sory binary instead of changing the
    # package template for each platform.
    return root / "src" / "sory_cli_bin" / "bin" / resource.name
...
def pinned_runtime_version() -> str:
    ...
    return normalize_sory_version(pins[0])


def pinned_runtime_sory_path() -> Path:
    """Return the bundled sory binary from the installed pinned runtime wheel."""
    ...
    normalized_installed_version = normalize_sory_version(installed_version)
    ...
    try:
        from sory_cli_bin import bundled_sory_path
    except ImportError as exc:
        raise RuntimeError(
            f"Installed {RUNTIME_DISTRIBUTION_NAME} package does not expose bundled_sory_path."
        ) from exc

    sory_path = bundled_sory_path()
    if not sory_path.exists():
        raise RuntimeError(f"Pinned sory runtime binary not found at {sory_path}.")
    return sory_path


def normalize_sory_version(version: str) -> str:
    ...
    if not re.fullmatch(r"[0-9]+(?:\.[0-9]+)*(?:(?:a|b|rc)[0-9]+)?", normalized):
        raise RuntimeError(f"Could not normalize sory version {version!r} to a PEP 440 version")
    return normalized
...
def stage_python_sdk_package(staging_dir: Path, sory_version: str) -> Path:
    package_version = normalize_sory_version(sory_version)
    _copy_package_tree(sdk_root(), staging_dir)
    sdk_bin_dir = staging_dir / "src" / "openai_sory" / "bin"
...
def generate_schema_from_pinned_runtime(schema_dir: Path) -> Path:
    ...
    sory_path = pinned_runtime_sory_path()
...
def generate_v2_all(schema_dir: Path) -> None:
    ...
    out_path = sdk_root() / "src" / "openai_sory" / "generated" / "v2_all.py"
...
def _notification_specs(schema_dir: Path) -> list[tuple[str, str]]:
    ...
    generated_source = (sdk_root() / "src" / "openai_sory" / "generated" / "v2_all.py").read_text()
...
def generate_notification_registry(schema_dir: Path) -> None:
    ...
    out = sdk_root() / "src" / "openai_sory" / "generated" / "notification_registry.py"
...
def _load_public_fields(
    ...
    if module_name == "openai_sory.generated.v2_all":
...
def _load_generated_v2_all_module() -> types.ModuleType:
    ...
    module_name = "_openai_sory_generated_v2_all_for_artifacts"
    sys.modules.pop(module_name, None)
    module_path = sdk_root() / "src" / "openai_sory" / "generated" / "v2_all.py"
...
def _render_async_thread_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(turn_fields),
        "    ) -> AsyncTurnHandle:",
        "        await self._sory._ensure_initialized()",
        "        wire_input = _to_wire_input(_normalize_run_input(input))",
        _approval_mode_assignment_line("_approval_mode_override_settings"),
        "        params = TurnStartParams(",
        "            thread_id=self.id,",
        ...
        *_model_arg_lines(turn_fields),
        "        )",
        "        turn = await self._sory._client.turn_start(",
        "            self.id,",
        "            wire_input,",
        "            params=params,",
        "        )",
        "        return AsyncTurnHandle(self._sory, self.id, turn.turn.id)",
    ]
    return "\n".join(lines)
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "openai_sory" / "api.py"
    ...
    source = _replace_generated_block(
        source,
        "Asyncsory.flat_methods",
        _render_async_sory_block(
            thread_start_fields,
            thread_list_fields,
            thread_resume_fields,
            thread_fork_fields,
...
def generate_types() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sory-python-schema-") as td:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_sdk_parser.add_argument(
        "--sory-version",
        help=(
            "sory release version to write into the staged SDK package and exact "
    ...
    stage_runtime_parser.add_argument(
        "runtime_binary",
        type=Path,
        help="Path to the sory binary to package for this platform",
    )
    stage_runtime_parser.add_argument(
        "--sory-version",
        help=(
            "sory release version to write into the staged runtime package. "
    ...
    stage_runtime_parser.add_argument(
        ...
        default=[],
        type=Path,
        help="Additional executable to package beside the sory runtime binary.",
    )
    return parser
...
def _resolve_sory_version(args: argparse.Namespace) -> str:
    versions = [
        value
        for value in (
            getattr(args, "sory_version", None),
    ...
    if not versions:
        raise RuntimeError("Pass --sory-version to stage Python release artifacts")

    normalized_versions = [normalize_sory_version(version) for version in versions]
    if len(set(normalized_versions)) != 1:
        raise RuntimeError("SDK and runtime package versions must match; pass one --sory-version")
...
def run_command(args: argparse.Namespace, ops: CliOps) -> None:
    if args.command == "generate-types":
        ...
        sory_version = _resolve_sory_version(args)
        ops.generate_types()
