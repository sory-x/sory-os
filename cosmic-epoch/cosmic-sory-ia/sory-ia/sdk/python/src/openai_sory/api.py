...
class sory:
    ...
    # BEGIN GENERATED: sory.flat_methods
    def thread_start(
        self,
        *,
    ...
    # END GENERATED: sory.flat_methods
...
class Asyncsory:
    """Async mirror of :class:`sory`.

    Prefer ``async with Asyncsory()`` so initialization and shutdown are
    ...
    async def __aenter__(self) -> "Asyncsory":
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "Asyncsory is not initialized yet. Prefer `async with Asyncsory()`; "
    ...
    # BEGIN GENERATED: Asyncsory.flat_methods
    ...
    # END GENERATED: Asyncsory.flat_methods
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        await self._sory._ensure_initialized()
        return await self._sory._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._sory._ensure_initialized()
        return await self._sory._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._sory._ensure_initialized()
        return await self._sory._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        await self._sory._ensure_initialized()
        return await self._sory._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        await self._sory._ensure_initialized()
        return await self._sory._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._sory._ensure_initialized()
        self._sory._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._sory._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._sory._client.unregister_turn_notifications(self.id)
