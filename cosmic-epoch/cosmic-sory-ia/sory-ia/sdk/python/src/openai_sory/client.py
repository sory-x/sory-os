...
RUNTIME_PKG_NAME = "openai-sory-cli-bin"
...
def _installed_sory_path() -> Path:
    try:
        from sory_cli_bin import bundled_sory_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned sory runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set AppServerConfig.sory_bin "
            "explicitly."
        ) from exc

    return bundled_sory_path()


@dataclass(frozen=True)
class soryBinResolverOps:
    installed_sory_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_sory_bin_resolver_ops() -> soryBinResolverOps:
    return soryBinResolverOps(
        installed_sory_path=_installed_sory_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_sory_bin(config: "AppServerConfig", ops: soryBinResolverOps) -> Path:
    if config.sory_bin is not None:
        sory_bin = Path(config.sory_bin)
        if not ops.path_exists(sory_bin):
            raise FileNotFoundError(
                f"sory binary not found at {sory_bin}. Set AppServerConfig.sory_bin "
                "to a valid binary path."
            )
        return sory_bin

    return ops.installed_sory_path()


def _resolve_sory_bin(config: "AppServerConfig") -> Path:
    return resolve_sory_bin(config, _default_sory_bin_resolver_ops())


@dataclass(slots=True)
class AppServerConfig:
    sory_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "sory_python_sdk"
    client_title: str = "sory Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class AppServerClient:
    """Synchronous typed JSON-RPC client for `sory app-server` over stdio."""
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            sory_bin = _resolve_sory_bin(self.config)
            args = [str(sory_bin)]
...
def default_sory_home() -> str:
    return str(Path.home() / ".sory")
