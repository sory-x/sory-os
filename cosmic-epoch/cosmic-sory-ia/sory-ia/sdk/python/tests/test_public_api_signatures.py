...
import openai_sory
import openai_sory.types as public_types
from openai_sory import (
    ApprovalMode,
    AppServerConfig,
    Asyncsory,
    AsyncThread,
...
from openai_sory._initialize_metadata import validate_initialize_metadata
from openai_sory.types import InitializeResponse
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert openai_sory.__version__ == pyproject["project"]["version"]
    assert AppServerConfig().client_version == openai_sory.__version__


def test_package_includes_py_typed_marker() -> None:
    """The wheel should advertise that inline type information is available."""
    marker = resources.files("openai_sory").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert openai_sory.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(openai_sory, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "AppServerClient": hasattr(openai_sory, "AppServerClient"),
        "AsyncAppServerClient": hasattr(openai_sory, "AsyncAppServerClient"),
        "InitializeResponse": hasattr(openai_sory, "InitializeResponse"),
        "ThreadStartParams": hasattr(openai_sory, "ThreadStartParams"),
        "TurnStartParams": hasattr(openai_sory, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(openai_sory, "TurnCompletedNotification"),
        "TurnStatus": hasattr(openai_sory, "TurnStatus"),
    } == {
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from openai_sory import *", namespace)
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from openai_sory.types import *", namespace)
...
def test_lifecycle_methods_are_sory_scoped() -> None:
    """Lifecycle operations should hang off the client rather than thread objects."""
    assert hasattr(sory, "thread_resume")
    assert hasattr(sory, "thread_fork")
    assert hasattr(sory, "thread_archive")
    assert hasattr(sory, "thread_unarchive")
    assert hasattr(Asyncsory, "thread_resume")
    assert hasattr(Asyncsory, "thread_fork")
    assert hasattr(Asyncsory, "thread_archive")
    assert hasattr(Asyncsory, "thread_unarchive")
    assert not hasattr(sory, "thread")
    assert not hasattr(Asyncsory, "thread")
    ...
    for fn in (
        sory.thread_archive,
        sory.thread_unarchive,
        Asyncsory.thread_archive,
        Asyncsory.thread_unarchive,
    ):
        _assert_no_any_annotations(fn)
...
def test_initialize_metadata_parses_user_agent_shape() -> None:
    ...
    payload = InitializeResponse.model_validate({"userAgent": "sory-cli/1.2.3"})
    parsed = validate_initialize_metadata(payload)
    assert parsed is payload
    assert parsed.userAgent == "sory-cli/1.2.3"
    assert parsed.serverInfo is not None
    assert parsed.serverInfo.name == "sory-cli"
