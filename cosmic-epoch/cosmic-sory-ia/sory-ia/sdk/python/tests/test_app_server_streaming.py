...
from openai_sory.generated.v2_all import (
...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with sory(config=harness.app_server_config()) as sory:
            thread = sory.thread_start()
            chunks = list(sory._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with Asyncsory(config=harness.app_server_config()) as sory:
                thread = await sory.thread_start()
                stream = sory._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(sory.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with sory(config=harness.app_server_config()) as sory:
            first_thread = sory.thread_start()
            second_thread = sory.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with Asyncsory(config=harness.app_server_config()) as sory:
                first_thread = await sory.thread_start()
                second_thread = await sory.thread_start()
