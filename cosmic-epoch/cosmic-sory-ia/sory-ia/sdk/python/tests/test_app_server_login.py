...
from openai_sory import AppServerConfig, sory
...
def test_api_key_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with sory(config=_app_server_config(harness)) as sory:
            sory.login_api_key("sk-sdk-login-test")
            result = sory.thread_start().run("prove api key auth")
...
def test_chatgpt_token_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with sory(config=_app_server_config(harness)) as sory:
            login = sory._client.account_login_start(
                LoginAccountParams(
            ...
            result = sory.thread_start().run("prove chatgpt token auth")
            request = harness.responses.single_request()
