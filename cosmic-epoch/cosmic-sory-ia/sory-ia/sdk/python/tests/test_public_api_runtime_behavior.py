...
import openai_sory.api as public_api_module
from openai_sory.api import (
    ApprovalMode,
    Asyncsory,
    sory,
)
from openai_sory.generated.v2_all import TurnStartParams
from openai_sory.models import InitializeResponse
...
def test_sory_init_failure_closes_client(monkeypatch: pytest.MonkeyPatch) -> None:
    closed: list[bool] = []
...
def test_async_sory_init_failure_closes_client() -> None:
    async def scenario() -> None:
        ...
        sory._client.start = fake_start  # type: ignore[method-assign]
        sory._client.initialize = fake_initialize  # type: ignore[method-assign]
        sory._client.close = fake_close  # type: ignore[method-assign]

        with pytest.raises(RuntimeError, match="missing required metadata"):
            await sory.models()

        assert close_calls == 1
        assert sory._initialized is False
        assert sory._init is None

    asyncio.run(scenario())


def test_async_sory_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        sory = Asyncsory()
        start_calls = 0
        initialize_calls = 0
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "sory-cli/1.2.3",
                    "serverInfo": {"name": "sory-cli", "version": "1.2.3"},
                }
            )
        ...
        sory._client.start = fake_start  # type: ignore[method-assign]
        sory._client.initialize = fake_initialize  # type: ignore[method-assign]
        sory._client.model_list = fake_model_list  # type: ignore[method-assign]

        await asyncio.gather(sory.models(), sory.models())
