...
from openai_sory import Asyncsory, sory
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with Asyncsory(config=harness.app_server_config()) as sory:
            ...
            server = sory.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": sory.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with sory(config=harness.app_server_config()) as sory:
            active_thread = sory.thread_start()
            archived_thread = sory.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            sory.thread_archive(archived_thread.id)
            active_list = sory.thread_list(archived=False)
            archived_list = sory.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with Asyncsory(config=harness.app_server_config()) as sory:
                ...
                resumed = await sory.thread_resume(thread.id)
                forked = await sory.thread_fork(thread.id)
                archive_response = await sory.thread_archive(thread.id)
                unarchived = await sory.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with sory(config=harness.app_server_config()) as sory:
            ...
            forked = sory.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with sory(config=harness.app_server_config()) as sory:
            ...
            archived = sory.thread_archive(thread.id)
            unarchived = sory.thread_unarchive(thread.id)
...
def test_models_rpc(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        with sory(config=harness.app_server_config()) as sory:
            models = sory.models(include_hidden=True)
