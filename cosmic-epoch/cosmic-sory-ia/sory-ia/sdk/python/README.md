# OpenAI sory Python SDK (Experimental)

Experimental Python SDK for `sory app-server` JSON-RPC v2 over stdio, with a small default surface optimized for real scripts and apps.

The generated wire-model layer is sourced from the pinned `openai-sory-cli-bin`
...
event types live in `openai_sory.types`.
...
Published SDK builds pin an exact `openai-sory-cli-bin` runtime dependency
with the same version as the SDK. Pass `AppServerConfig(sory_bin=...)` only
when you intentionally want to run against a specific local app-server binary.

## Quickstart

```python
from openai_sory import sory

with sory() as sory:
    # Call login_api_key(...) first when this app-server session is not
    # already authenticated.
    thread = sory.thread_start(model="gpt-5")
...
with sory() as sory:
    sory.login_api_key("sk-...")
    account = sory.account()
...
Published SDK builds are pinned to an exact `openai-sory-cli-bin` package
...
- Package: `openai-sory`
- Runtime package: `openai-sory-cli-bin`
- Python: `>=3.10`
- Target protocol: sory `app-server` JSON-RPC v2
- Versioning rule: the SDK package version is the underlying sory runtime version

## Notes

- `sory()` is eager and performs startup + `initialize` in the constructor.
- Use context managers (`with sory() as sory:`) to ensure shutdown.
