...
describe("soryExec", () => {
  it("rejects when exit happens before stdout closes", async () => {
    ...
    if (result.status === "rejected") {
      ...
      expect(result.error.message).toMatch(/sory Exec exited/);
    }
  });
  ...
  it("allows overriding the env passed to the sory CLI", async () => {
    const { soryExec } = await import("../src/exec");
    spawnMock.mockClear();
    const child = new FakeChildProcess();
    spawnMock.mockReturnValue(child as unknown as child_process.ChildProcess);
    ...
    process.env.sory_ENV_SHOULD_NOT_LEAK = "leak";

    try {
      const exec = new soryExec("sory", {
        sory_HOME: "/tmp/sory-home",
        CUSTOM_ENV: "custom",
      });
      ...
      expect(spawnEnv.sory_HOME).toBe("/tmp/sory-home");
      expect(spawnEnv.CUSTOM_ENV).toBe("custom");
      expect(spawnEnv.sory_ENV_SHOULD_NOT_LEAK).toBeUndefined();
      expect(spawnEnv.sory_API_KEY).toBe("test");
      expect(spawnEnv.sory_INTERNAL_ORIGINATOR_OVERRIDE).toBeDefined();
      expect(commandArgs).toContain("--config");
      expect(commandArgs).toContain(`openai_base_url=${JSON.stringify("https://example.test")}`);
    } finally {
      delete process.env.sory_ENV_SHOULD_NOT_LEAK;
    }
  });
});
