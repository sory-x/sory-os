...
import { soryExecSpy } from "./soryExecSpy";
...
import { createMockClient, createTestClient } from "./testsory";
...
describe("sory", () => {
  ...
  it("passes turn options to exec", async () => {
    ...
    const { args: spawnArgs, restore } = soryExecSpy();
    const { client, cleanup } = createMockClient(url);
  ...
  it("passes soryOptions config overrides as TOML --config flags", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("lets thread options override soryOptions config overrides", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("forwards images to exec", async () => {
    ...
    const { args: spawnArgs, restore } = soryExecSpy();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sory-images-"));
  ...
  it("runs in provided working directory", async () => {
    ...
    const { args: spawnArgs, restore } = soryExecSpy();
    const workingDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "sory-working-dir-"));
    const { client, cleanup } = createTestClient({
      baseUrl: url,
      apiKey: "test",
    });
  ...
  it("sets the sory sdk originator header", async () => {
    const { url, close, requests } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [sse(responseStarted(), assistantMessage("Hi!"), responseCompleted())],
    });
    ...
    try {
      ...
      if (Array.isArray(originatorHeader)) {
        expect(originatorHeader).toContain("sory_sdk_ts");
      } else {
        expect(originatorHeader).toBe("sory_sdk_ts");
      }
    } finally {
      cleanup();
      await close();
...
/**
 * Given a list of args to `sory` and a `key`, collects all `--config`
