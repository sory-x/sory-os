#!/usr/bin/env python3
"""Collect recent openai/sory issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a sory owner digest."
    )
    parser.add_argument(
        "--repo", default="openai/sory", help="OWNER/REPO, default openai/sory"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(sory(?: desktop| app|\.app| cli)?|desktop|windows sory app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
