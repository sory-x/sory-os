...
"""Verify sory-tui does not depend on or import sory-core directly."""
...
TUI_ROOT = ROOT / "sory-rs" / "tui"
TUI_MANIFEST = TUI_ROOT / "Cargo.toml"
FORBIDDEN_PACKAGE = "sory-core"
FORBIDDEN_SOURCE_PATTERNS = (
    re.compile(r"\bsory_core::"),
    re.compile(r"\buse\s+sory_core\b"),
    re.compile(r"\bextern\s+crate\s+sory_core\b"),
)
...
def main() -> int:
    ...
    print("sory-tui must not depend on or import sory-core directly.")
    print(
        "Use the app-server protocol/client boundary instead; temporary embedded "
        "startup gaps belong behind sory_app_server_client::legacy_core."
    )
    print()
    ...
    for path in sorted(TUI_ROOT.glob("**/*.rs")):
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            if any(pattern.search(line) for pattern in FORBIDDEN_SOURCE_PATTERNS):
                failures.append(f"{relative_path(path)}:{line_number} imports `sory_core`")
    return failures
