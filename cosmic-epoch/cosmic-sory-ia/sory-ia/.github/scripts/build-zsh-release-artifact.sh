...
work_root="$(mktemp -d "${temp_root%/}/sory-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/sory-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${sory_WRAPPER_LOG:?missing sory_WRAPPER_LOG}"
printf '%s\n' "$@" > "$sory_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
sory_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" sory-zsh)
