...
remote_path='~/code/sory-sync'
local_exec_server_port="${sory_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${sory_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/sory-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/sory-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/sory-sync"
remote_sory_rs="$remote_repo_root/sory-rs"

cd "${remote_sory_rs}"
cargo build -p sory-cli --bin sory

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/sory exec-server --listen ws://127.0.0.1:0 \
...
echo "Start sory via: "
printf '  sory_EXEC_SERVER_URL=ws://127.0.0.1:%s sory -C %q\n' \
