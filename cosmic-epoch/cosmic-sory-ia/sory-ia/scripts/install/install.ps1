...
function Get-ReleaseAssetMetadata {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/openai/sory/releases/tags/rust-v$ResolvedVersion"
    $asset = $release.assets | Where-Object { $_.name -eq $AssetName } | Select-Object -First 1
    if ($null -eq $asset) {
        throw "Could not find release asset $AssetName for sory $ResolvedVersion."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded sory archive checksum did not match release metadata. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Resolve-Version {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/openai/sory/releases/latest"
    if (-not $release.tag_name) {
        Write-Error "Failed to resolve the latest sory release version."
        exit 1
    }
...
function Get-VersionFromBinary {
    ...
    if (-not (Test-Path -LiteralPath $soryPath -PathType Leaf)) {
        return $null
    }

    try {
        $versionOutput = & $soryPath --version 2>$null
    } catch {
        return $null
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -soryPath (Join-Path $StandaloneCurrentDir "sory.exe")
...
function Test-OldStandaloneBinLayout {
    ...
    $knownFiles = @(
        "sory.exe",
        "rg.exe",
        "sory-command-runner.exe",
        "sory-windows-sandbox.exe",
        "sory-windows-sandbox-setup.exe"
    )
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older sory install at $VisibleBinDir"
    Write-WarningStep "To continue, sory needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current sory setup now?")) {
...
function Add-JunctionSupportType {
    if (([System.Management.Automation.PSTypeName]'soryInstaller.Junction').Type) {
        return
    }
...
function Set-JunctionTarget {
    ...
    [soryInstaller.Junction]::SetTarget($LinkPath, $TargetPath)
}
...
function Test-ReleaseIsComplete {
    ...
    $expectedFiles = @(
        "sory.exe",
        "sory-resources\sory-command-runner.exe",
        "sory-resources\sory-windows-sandbox-setup.exe",
        "sory-resources\rg.exe"
    )
...
function Get-ExistingsoryCommand {
    $existing = Get-Command sory -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ExistingsoryManager {
    param(
        [string]$ExistingPath,
        [string]$VisibleBinDir
    )
...
function Get-ConflictingInstall {
    ...
    $existingPath = Get-ExistingsoryCommand
    $manager = Get-ExistingsoryManager -ExistingPath $existingPath -VisibleBinDir $VisibleBinDir
    if ($null -eq $manager) {
        return $null
    }

    Write-Step "Detected existing $manager-managed sory at $existingPath"
    Write-WarningStep "Multiple managed sory installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        @("remove", "-g", "@openai/sory")
    } else {
        @("uninstall", "-g", "@openai/sory")
    }
    $uninstallCommand = if ($manager -eq "bun") { "bun" } else { "npm" }

    if (Prompt-YesNo "Uninstall the existing $manager-managed sory now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed sory. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed sory installed. PATH order will determine which sory runs."
    }
}

function Test-VisiblesoryCommand {
    param(
        [string]$VisibleBinDir
    )

    $soryCommand = Join-Path $VisibleBinDir "sory.exe"
    & $soryCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed sory command failed verification: $soryCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "sory requires a 64-bit version of Windows."
    exit 1
}
...
$soryHome = if ([string]::IsNullOrWhiteSpace($env:sory_HOME)) {
    Join-Path $env:USERPROFILE ".sory"
} else {
    $env:sory_HOME
}
$standaloneRoot = Join-Path $soryHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\OpenAI\sory\bin"
if ([string]::IsNullOrWhiteSpace($env:sory_INSTALL_DIR)) {
    $visibleBinDir = $defaultVisibleBinDir
} else {
    $visibleBinDir = $env:sory_INSTALL_DIR
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating sory CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating sory CLI"
} else {
    Write-Step "Installing sory CLI"
}
...
$packageAsset = "sory-npm-$npmTag-$resolvedVersion.tgz"
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("sory-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target)) {
            ...
            Write-Step "Downloading sory CLI"
            ...
            $resourcesDir = Join-Path $stagingDir "sory-resources"
            New-Item -ItemType Directory -Force -Path $resourcesDir | Out-Null
            $copyMap = @{
                "sory/sory.exe" = "sory.exe"
                "sory/sory-command-runner.exe" = "sory-resources\sory-command-runner.exe"
                "sory/sory-windows-sandbox-setup.exe" = "sory-resources\sory-windows-sandbox-setup.exe"
                "path/rg.exe" = "sory-resources\rg.exe"
            }
        ...
        try {
            ...
            Test-VisiblesoryCommand -VisibleBinDir $visibleBinDir
        } catch {
...
Write-Step "Current PowerShell session: sory"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: sory"
Write-Host "sory CLI $resolvedVersion installed successfully."

$soryCommand = Join-Path $visibleBinDir "sory.exe"
if (Prompt-YesNo "Start sory now?") {
    Write-Step "Launching sory"
    & $soryCommand
}
