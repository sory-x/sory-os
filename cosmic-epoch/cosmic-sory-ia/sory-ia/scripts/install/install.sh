...
BIN_DIR="${sory_INSTALL_DIR:-$HOME/.local/bin}"
BIN_PATH="$BIN_DIR/sory"
sory_HOME_DIR="${sory_HOME:-$HOME/.sory}"
STANDALONE_ROOT="$sory_HOME_DIR/packages/standalone"
...
download_file() {
  ...
  echo "curl or wget is required to install sory." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/openai/sory/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/openai/sory/releases/tags/rust-v%s\n' "$resolved_version"
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the sory download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded sory archive checksum did not match release metadata." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install sory." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/openai/sory/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest sory release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> sory installer >>>"
  end_marker="# <<< sory installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.sory.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  ...
  "$sory_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/sory" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_sory() {
  command -v sory 2>/dev/null || true
}

classify_existing_sory() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && sory"
      step "Future terminals: open a new terminal and run: sory"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: sory"
      step "Future terminals: open a new terminal and run: sory"
      ;;
  esac
}

maybe_launch_sory_now() {
  if prompt_yes_no "Start sory now?"; then
    step "Launching sory"
    "$BIN_PATH"
  fi
}
...
detect_conflicting_install() {
  existing_path="$(resolve_existing_sory)"
  manager="$(classify_existing_sory "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed sory at $existing_path"
  warn "Multiple managed sory installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask sory"
      ;;
    bun)
      uninstall_cmd="bun remove -g @openai/sory"
      ;;
    *)
      uninstall_cmd="npm uninstall -g @openai/sory"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed sory now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed sory. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed sory installed. PATH order will determine which sory runs."
  fi
}
...
install_release() {
  ...
  rm -rf "$stage_release"
  mkdir -p "$stage_release/sory-resources"
  cp "$vendor_root/sory/sory" "$stage_release/sory"
  cp "$vendor_root/path/rg" "$stage_release/sory-resources/rg"
  chmod 0755 "$stage_release/sory"
  chmod 0755 "$stage_release/sory-resources/rg"
  if [ -f "$vendor_root/sory-resources/bwrap" ]; then
    cp "$vendor_root/sory-resources/bwrap" "$stage_release/sory-resources/bwrap"
    chmod 0755 "$stage_release/sory-resources/bwrap"
  fi
...
release_dir_is_complete() {
  ...
  [ -d "$release_dir" ] &&
    [ -x "$release_dir/sory" ] &&
    [ -x "$release_dir/sory-resources/rg" ] &&
    [ "$(basename "$release_dir")" = "$expected_version-$expected_target" ] &&
    case "$expected_target" in
      *linux*) [ -x "$release_dir/sory-resources/bwrap" ] ;;
      *) true ;;
    esac
}
...
update_visible_command() {
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.sory.$$"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/sory" "$tmp_link"
}
...
asset="sory-npm-$npm_tag-$resolved_version.tgz"
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating sory CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating sory CLI"
else
  step "Installing sory CLI"
fi
...
printf 'sory CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_sory_now
