...
- `openai/sory` release assets for published musl release pairs
...
`RUSTY_V8_SRC_BINDING_PATH` to point at those `openai/sory` release assets
...
the exact resolved `v8` crate version in `sory-rs/Cargo.lock`.
