#!/usr/bin/env python3
"""Stage and optionally package the @openai/sory npm module."""
...
sory_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = sory_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "sory-rs" / "responses-api-proxy" / "npm"
sory_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"
sory_NPM_NAME = "@openai/sory"

# `npm_name` is the local optional-dependency alias consumed by `bin/sory.js`.
# The underlying package published to npm is always `@openai/sory`.
sory_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "sory-linux-x64": {
        "npm_name": "@openai/sory-linux-x64",
        "npm_tag": "linux-x64",
    ...
    "sory-linux-arm64": {
        "npm_name": "@openai/sory-linux-arm64",
        "npm_tag": "linux-arm64",
    ...
    "sory-darwin-x64": {
        "npm_name": "@openai/sory-darwin-x64",
        "npm_tag": "darwin-x64",
    ...
    "sory-darwin-arm64": {
        "npm_name": "@openai/sory-darwin-arm64",
    ...
    "sory-win32-x64": {
        "npm_name": "@openai/sory-win32-x64",
        "npm_tag": "win32-x64",
    ...
    "sory-win32-arm64": {
        "npm_name": "@openai/sory-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "sory": ["sory", *sory_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "sory": [],
    "sory-linux-x64": ["bwrap", "sory", "rg"],
    "sory-linux-arm64": ["bwrap", "sory", "rg"],
    "sory-darwin-x64": ["sory", "rg"],
    "sory-darwin-arm64": ["sory", "rg"],
    "sory-win32-x64": ["sory", "rg", "sory-windows-sandbox-setup", "sory-command-runner"],
    "sory-win32-arm64": ["sory", "rg", "sory-windows-sandbox-setup", "sory-command-runner"],
    "sory-responses-api-proxy": ["sory-responses-api-proxy"],
    "sory-sdk": [],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in sory_PLATFORM_PACKAGES.items()
}

PACKAGE_CHOICES = tuple(PACKAGE_NATIVE_COMPONENTS)

COMPONENT_DEST_DIR: dict[str, str] = {
    "bwrap": "sory-resources",
    "sory": "sory",
    "sory-responses-api-proxy": "sory-responses-api-proxy",
    "sory-windows-sandbox-setup": "sory",
    "sory-command-runner": "sory",
    "rg": "path",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the sory CLI npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="sory",
        help="Which npm package to stage (default: sory).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "sory":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/sory.js --version\n"
                    f"    node {staging_dir_str}/bin/sory.js --help\n\n"
                )
            elif package == "sory-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/sory-responses-api-proxy.js --help\n\n"
                )
            elif package in sory_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="sory-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "sory":
        ...
        shutil.copy2(sory_CLI_ROOT / "bin" / "sory.js", bin_dir / "sory.js")
        rg_manifest = sory_CLI_ROOT / "bin" / "rg"
        ...
        package_json_path = sory_CLI_ROOT / "package.json"
    elif package in sory_PLATFORM_PACKAGES:
        platform_package = sory_PLATFORM_PACKAGES[package]
        ...
        with open(sory_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            sory_package_json = json.load(fh)

        package_json = {
            "name": sory_NPM_NAME,
            "version": platform_version,
            "license": sory_package_json.get("license", "Apache-2.0"),
            "os": [platform_package["os"]],
            "cpu": [platform_package["cpu"]],
            "files": ["vendor"],
            "repository": sory_package_json.get("repository"),
        }

        engines = sory_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = sory_package_json.get("packageManager")
        if isinstance(package_manager, str):
            package_json["packageManager"] = package_manager
    elif package == "sory-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "sory-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "sory-responses-api-proxy.js")
    ...
    elif package == "sory-sdk":
        package_json_path = sory_SDK_ROOT / "package.json"
        stage_sory_sdk_sources(staging_dir)
    else:
    ...
    if package == "sory":
        ...
        package_json["optionalDependencies"] = {
            sory_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{sory_NPM_NAME}@"
                f"{compute_platform_package_version(version, sory_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["sory"]
            if platform_package != "sory"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[sory_NPM_NAME] = version
...
def stage_sory_sdk_sources(staging_dir: Path) -> None:
    package_root = sory_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("sory-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="sory-npm-pack-") as pack_dir_str:
