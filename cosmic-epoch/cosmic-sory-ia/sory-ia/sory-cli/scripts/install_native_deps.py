#!/usr/bin/env python3
"""Install sory native binaries (Rust CLI, bwrap, and ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/openai/sory/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. sory-<target>.zst)
...
BINARY_COMPONENTS = {
    "bwrap": BinaryComponent(
        ...
        dest_dir="sory-resources",
        binary_basename="bwrap",
        targets=LINUX_TARGETS,
    ),
    "sory": BinaryComponent(
        artifact_prefix="sory",
        dest_dir="sory",
        binary_basename="sory",
    ),
    "sory-responses-api-proxy": BinaryComponent(
        artifact_prefix="sory-responses-api-proxy",
        dest_dir="sory-responses-api-proxy",
        binary_basename="sory-responses-api-proxy",
    ),
    "sory-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="sory-windows-sandbox-setup",
        dest_dir="sory",
        binary_basename="sory-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "sory-command-runner": BinaryComponent(
        artifact_prefix="sory-command-runner",
        dest_dir="sory",
        binary_basename="sory-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native sory binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to bwrap, sory, sory-windows-sandbox-setup,"
            " sory-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    sory_cli_root = (args.root or sory_CLI_ROOT).resolve()
    vendor_dir = sory_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or [
        "bwrap",
        "sory",
        "sory-windows-sandbox-setup",
        "sory-command-runner",
        "rg",
    ]
    ...
    with _gha_group(f"Download native artifacts from workflow {workflow_id}"):
        with tempfile.TemporaryDirectory(prefix="sory-native-artifacts-") as artifacts_dir_str:
