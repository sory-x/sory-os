...
- The legacy rule matcher lives in `sory-execpolicy-legacy`.
...
- From the sory CLI, run `sory execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
sory execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
sory execpolicy check \
...
```bash
cargo run -p sory-execpolicy -- check --rules path/to/policy.rules git status
```
