...
use sory_api::AuthError;
use sory_api::AuthProvider;
use sory_api::SharedAuthProvider;
use sory_aws_auth::AwsAuthContext;
use sory_aws_auth::AwsAuthError;
use sory_aws_auth::AwsRequestToSign;
use sory_client::Request;
use sory_client::RequestBody;
use sory_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_sory_error)?;
...
fn aws_auth_error_to_sory_error(error: AwsAuthError) -> SoryErr {
    SoryErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer sory agent requests fail even though raw Responses requests work.
