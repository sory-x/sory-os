...
use sory_models_manager::manager::OpenAiModelsManager;
use sory_models_manager::manager::SharedModelsManager;
use sory_models_manager::manager::StaticModelsManager;
use sory_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that sory may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "sory-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether sory should have a unified provider-specific auth
    /// manager throughout the codebase; that is a larger refactor than this change.
    fn auth_manager(&self) -> Option<Arc<AuthManager>>;

    /// Returns the current provider-scoped auth value, if one is configured.
    async fn auth(&self) -> Option<soryAuth>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Returns provider configuration adapted for the API client.
    async fn api_provider(&self) -> sory_protocol::error::Result<Provider> {
        let auth = self.auth().await;
        self.info()
            .to_api_provider(auth.as_ref().map(soryAuth::auth_mode))
    }

    /// Returns the provider base URL that will be used at request time.
    async fn runtime_base_url(&self) -> sory_protocol::error::Result<Option<String>> {
        Ok(self.info().base_url.clone())
    }

    /// Returns the auth provider used to attach request credentials.
    async fn api_auth(&self) -> sory_protocol::error::Result<SharedAuthProvider> {
        let auth = self.auth().await;
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .map(|auth| match &auth {
                    soryAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    soryAuth::Chatgpt(_)
                    | soryAuth::ChatgptAuthTokens(_)
                    | soryAuth::AgentIdentity(_) => {
...
mod tests {
    ...
    use sory_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn test_sory_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("sory-model-provider-test-{}", std::process::id()))
    }
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("sory-bedrock".to_string()),
                region: None,
            })),
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_sory_home(), /*config_model_catalog*/ None);
    ...
    async fn amazon_bedrock_provider_uses_configured_static_catalog_when_present() {
        let custom_model =
            sory_models_manager::model_info::model_info_from_slug("custom-bedrock-model");
