...
use sory_client::build_reqwest_client_with_custom_ca;
use sory_client::with_chatgpt_cloudflare_cookie_store;
...
use sory_protocol::protocol::RateLimitReachedType;
...
impl Client {
    pub fn new(base_url: impl Into<String>) -> Result<Self> {
        ...
        Ok(Self {
            base_url,
            http,
            auth_provider: sory_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &soryAuth) -> Result<Self> {
        Ok(Self::new(base_url)?
            .with_user_agent(get_sory_user_agent())
            .with_auth_provider(sory_model_provider::auth_provider_from_auth(auth)))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("sory-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("sory"))
            .cloned();
    ...
    pub async fn get_rate_limits_many(&self) -> Result<Vec<RateLimitSnapshot>> {
        let url = match self.path_style {
            PathStyle::SoryApi => format!("{}/api/sory/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::SoryApi => format!("{}/api/sory/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::SoryApi => format!("{}/api/sory/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::SoryApi => format!(
                "{}/api/sory/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the managed requirements file from sory-backend.
    ///
    /// `GET /api/sory/config/requirements` (sory API style) or
    ...
    pub async fn get_config_requirements_file(
        ...
        let url = match self.path_style {
            PathStyle::SoryApi => format!("{}/api/sory/config/requirements", self.base_url),
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::SoryApi => format!("{}/api/sory/tasks", self.base_url),
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::SoryApi => format!(
                "{}/api/sory/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
...
mod tests {
    ...
    use sory_backend_openapi_models::models::AdditionalRateLimitDetails;
    use sory_backend_openapi_models::models::RateLimitReachedKind;
    use sory_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "sory_other".to_string(),
                metered_feature: "sory_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("sory"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("sory_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("sory_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("sory"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let sory_client = Client {
            ...
            chatgpt_account_id: None,
            chatgpt_account_is_fedramp: false,
            path_style: PathStyle::SoryApi,
        };
        assert_eq!(
            sory_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/sory/accounts/send_add_credits_nudge_email"
        );
