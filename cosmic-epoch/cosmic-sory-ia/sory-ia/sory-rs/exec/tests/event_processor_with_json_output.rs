...
use serde_json::json;
use sory_app_server_protocol::CollabAgentState as ApiCollabAgentState;
use sory_app_server_protocol::CollabAgentStatus as ApiCollabAgentStatus;
use sory_app_server_protocol::CollabAgentTool;
use sory_app_server_protocol::CollabAgentToolCallStatus as ApiCollabAgentToolCallStatus;
use sory_app_server_protocol::CommandAction;
use sory_app_server_protocol::CommandExecutionSource;
use sory_app_server_protocol::CommandExecutionStatus as ApiCommandExecutionStatus;
use sory_app_server_protocol::ErrorNotification;
use sory_app_server_protocol::FileUpdateChange as ApiFileUpdateChange;
use sory_app_server_protocol::ItemCompletedNotification;
use sory_app_server_protocol::ItemStartedNotification;
use sory_app_server_protocol::McpToolCallError;
use sory_app_server_protocol::McpToolCallResult;
use sory_app_server_protocol::McpToolCallStatus as ApiMcpToolCallStatus;
use sory_app_server_protocol::PatchApplyStatus as ApiPatchApplyStatus;
use sory_app_server_protocol::PatchChangeKind as ApiPatchChangeKind;
...
use sory_app_server_protocol::WebSearchAction as ApiWebSearchAction;
...
use sory_exec::AgentMessageItem;
use sory_exec::soryStatus;
use sory_exec::CollabAgentState;
use sory_exec::CollabAgentStatus;
use sory_exec::CollabTool;
use sory_exec::CollabToolCallItem;
use sory_exec::CollabToolCallStatus;
use sory_exec::CollectedThreadEvents;
use sory_exec::CommandExecutionItem;
use sory_exec::CommandExecutionStatus;
use sory_exec::ErrorItem;
use sory_exec::EventProcessorWithJsonOutput;
use sory_exec::ExecThreadItem;
use sory_exec::FileChangeItem;
use sory_exec::FileUpdateChange as ExecFileUpdateChange;
use sory_exec::ItemCompletedEvent;
use sory_exec::ItemStartedEvent;
use sory_exec::ItemUpdatedEvent;
use sory_exec::McpToolCallItem;
use sory_exec::McpToolCallItemError;
use sory_exec::McpToolCallItemResult;
use sory_exec::McpToolCallStatus;
use sory_exec::PatchApplyStatus;
use sory_exec::PatchChangeKind;
use sory_exec::ReasoningItem;
use sory_exec::ThreadErrorEvent;
use sory_exec::ThreadEvent;
use sory_exec::ThreadItemDetails;
use sory_exec::ThreadStartedEvent;
use sory_exec::TodoItem;
use sory_exec::TodoListItem;
use sory_exec::TurnCompletedEvent;
use sory_exec::TurnFailedEvent;
use sory_exec::TurnStartedEvent;
use sory_exec::Usage;
use sory_exec::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_configured = SessionConfiguredEvent {
        ...
        thread_source: None,
        thread_name: None,
        model: "sory-mini-latest".to_string(),
        model_provider_id: "test-provider".to_string(),
        service_tier: None,
        approval_policy: AskForApproval::Never,
        approvals_reviewer: sory_protocol::config_types::ApprovalsReviewer::User,
...
fn plan_update_emits_started_then_updated_then_completed() {
    ...
    assert_eq!(
        ...
        CollectedThreadEvents {
            ...
            status: soryStatus::InitiateShutdown,
        }
    );
}
...
fn token_usage_update_is_emitted_on_turn_completion() {
    ...
    let usage_update =
        processor.collect_thread_events(ServerNotification::ThreadTokenUsageUpdated(
            sory_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn failed_turn_clears_stale_final_message() {
    ...
    assert_eq!(collected.status, soryStatus::Running);
    ...
    assert_eq!(collected.status, soryStatus::InitiateShutdown);
...
fn model_reroute_surfaces_as_error_item() {
    ...
    let collected = processor.collect_thread_events(ServerNotification::ModelRerouted(
        sory_app_server_protocol::ModelReroutedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            from_model: "gpt-5".to_string(),
            to_model: "gpt-5-mini".to_string(),
            reason: sory_app_server_protocol::ModelRerouteReason::HighRiskCyberActivity,
        },
    ));
