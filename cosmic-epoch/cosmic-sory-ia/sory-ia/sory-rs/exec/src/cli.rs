...
#[command(
    version,
    override_usage = "sory exec [OPTIONS] [PROMPT]\n       sory exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running sory outside a Git repository.
    ...
    /// Do not load `$sory_HOME/config.toml`; auth still uses `sory_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "sory resume --last <prompt>" treats the positional as a prompt, not a session id.
