...
pub use event_processor_with_jsonl_output::soryStatus;
...
use sory_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use sory_app_server_client::EnvironmentManager;
use sory_app_server_client::ExecServerRuntimePaths;
use sory_app_server_client::InProcessAppServerClient;
use sory_app_server_client::InProcessClientStartArgs;
use sory_app_server_client::InProcessServerEvent;
...
use sory_app_server_protocol::Thread as AppServerThread;
use sory_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use sory_cloud_requirements::cloud_requirements_loader_for_storage;
use sory_config::ConfigLoadError;
use sory_config::ConfigLoadOptions;
use sory_config::LoaderOverrides;
use sory_config::format_config_error_with_source;
use sory_core::StateDbHandle;
use sory_core::check_execpolicy_for_warnings;
...
use sory_core::config::load_config_as_toml_with_cli_and_load_options;
use sory_core::config::resolve_oss_provider;
use sory_core::config::resolve_profile_v2_config_path;
use sory_core::find_thread_meta_by_name_str;
use sory_core::format_exec_policy_error_with_source;
use sory_core::path_utils;
use sory_feedback::soryFeedback;
use sory_git_utils::get_git_repo_root;
use sory_login::AuthConfig;
use sory_login::default_client::set_default_client_residency_requirement;
use sory_login::default_client::set_default_originator;
use sory_login::enforce_login_restrictions;
use sory_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use sory_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use sory_otel::set_parent_from_context;
use sory_otel::traceparent_context_from_env;
...
use sory_protocol::protocol::ReviewTarget;
use sory_protocol::protocol::RolloutItem;
use sory_protocol::protocol::RolloutLine;
...
use sory_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use sory_utils_cli::SharedCliOptions;
use sory_utils_oss::ensure_oss_provider_ready;
use sory_utils_oss::get_default_model_for_oss_provider;
...
enum StdinPromptBehavior {
    ...
    /// `sory exec` behavior for `sory exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `sory exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("sory_exec".to_string()) {
        tracing::warn!(?err, "Failed to set sory exec originator override {err:?}");
    }
    ...
    #[allow(clippy::print_stderr)]
    let sory_home = match find_sory_home() {
        Ok(sory_home) => sory_home,
        Err(err) => {
            eprintln!("Error finding sory home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&sory_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        sory_core::otel_init::build_provider(
            &config,
    ...
    sory_core::otel_init::record_process_start(otel.as_ref(), "sory_exec");
    sory_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "sory_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.sory_self_exe.clone(),
        arg0_paths.sory_linux_sandbox_exe.clone(),
    )?;
    let state_db = sory_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        EnvironmentManager::from_env(local_runtime_paths).await?
    } else {
        EnvironmentManager::from_sory_home(config.sory_home.clone(), local_runtime_paths).await?
    };
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "sory_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = sory_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what sory
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            sory_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("sory initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        sory_app_server_protocol::TurnStatus::Failed
                            | sory_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn session_configured_from_thread_response(
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<sory_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.sory_home, session_id, Some(state_db.as_ref()))
                .await?
