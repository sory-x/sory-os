...
`sory-tools` is the shared support crate for building, adapting, planning, and
executing model-visible tool sets outside `sory-core`.
...
compatibility-sensitive orchestration in `sory-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `sory-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `sory-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `sory-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `sory-core`.
3. Leave compatibility-sensitive adapters in `sory-core` while downstream
...
`sory-core`, that is a sign to revisit the extraction boundary before adding
more here.
