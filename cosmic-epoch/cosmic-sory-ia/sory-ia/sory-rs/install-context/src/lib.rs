...
const RESOURCES_DIRNAME: &str = "sory-resources";
...
pub enum InstallContext {
    Standalone {
        ...
        /// `~/.sory/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        release_dir: PathBuf,
    ...
    /// A sory binary launched through the npm-managed `sory.js` shim.
    Npm,
    /// A sory binary launched through the bun-managed `sory.js` shim.
    Bun,
    /// A sory binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled sory binaries, custom
    /// internal launchers, and tests that execute sory from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let sory_home = sory_utils_home_dir::find_sory_home().ok();
        Self::from_exe_with_sory_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_sory_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        sory_home: Option<&Path>,
    ) -> Self {
        if managed_by_npm {
            return Self::Npm;
        }
        ...
        if let Some(exe_path) = current_exe
            && let Some(standalone_context) = standalone_install_context(exe_path, sory_home)
        {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("sory_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("sory_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn standalone_install_context(
    ...
    let canonical_sory_home = std::fs::canonicalize(sory_home?).ok()?;
    let release_dir = canonical_exe.parent()?.to_path_buf();
    let releases_root = canonical_sory_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        let sory_home = tempfile::tempdir()?;
        let release_dir = sory_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "sory.exe" } else { "sory" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_sory_home(
            /*is_macos*/ false,
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_sory_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/sory")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*sory_home*/ None,
        );
        ...
        let bun_context = InstallContext::from_exe_with_sory_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/sory")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_sory_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/sory")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*sory_home*/ None,
        );
