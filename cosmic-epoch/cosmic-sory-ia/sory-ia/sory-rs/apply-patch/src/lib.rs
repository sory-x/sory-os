...
/// Special argv[1] flag used when the sory executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `sory-apply-patch` (to avoid forcing
/// `sory-arg0` to depend on `sory-core`), it remains part of the "sory core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const sory_CORE_APPLY_PATCH_ARG1: &str = "--sory-run-as-apply-patch";
