...
use sory_login::AuthCredentialsStoreMode;
use sory_login::AuthManager;
use sory_login::soryAuth;
use sory_login::ExternalAuth;
use sory_login::ExternalAuthRefreshContext;
use sory_login::ExternalAuthTokens;
use sory_login::TokenData;
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    async fn uses_sory_backend(&self) -> bool {
        self.uses_sory_backend
    }
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(sory_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(sory_home: &Path) -> soryAuth {
    let auth_dot_json = sory_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: sory_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(sory_home).expect("sory home should be created");
    std::fs::write(
...
async fn get_model_info_tracks_fallback_usage() {
    let sory_home = tempdir().expect("temp dir");
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-sory/gpt-image".to_string();
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let manager = openai_manager_for_tests(sory_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(sory_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(sory_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(sory_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager = openai_manager_for_tests(sory_home.path().to_path_buf(), endpoint.clone());
    ...
    async fn uses_sory_backend(&self) -> bool {
        match self.auth_manager.as_ref() {
            Some(auth_manager) => auth_manager
                ...
                .await
                .as_ref()
                .is_some_and(soryAuth::uses_sory_backend),
            None => false,
        }
    }
...
async fn refresh_available_models_skips_network_when_external_api_key_overrides_chatgpt_auth() {
    ...
    let sory_home = tempdir().expect("temp dir");
    let auth_manager =
        AuthManager::from_auth_for_testing(soryAuth::create_dummy_chatgpt_auth_for_testing());
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(sory_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        sory_home.path().to_path_buf(),
        endpoint.clone(),
