...
pub enum Command {
    /// Submit a new sory Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a sory Cloud task.
    Status(StatusCommand),
    /// List sory Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a sory Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a sory Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in sory Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `sory cloud` to browse).
    ...
    /// Git branch to run in sory Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// sory Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// sory Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// sory Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
