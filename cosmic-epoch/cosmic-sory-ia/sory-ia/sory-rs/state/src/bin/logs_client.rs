...
use clap::Parser;
use clap::ValueEnum;
use sory_state::LogQuery;
use sory_state::LogRow;
use sory_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "sory-state-logs")]
#[command(about = "Tail sory logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to sory_HOME. Defaults to $sory_HOME or ~/.sory.
    #[arg(long, env = "sory_HOME")]
    sory_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --sory-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let sory_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(sory_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let sory_home = args.sory_home.clone().unwrap_or_else(default_sory_home);
    Ok(sory_state::logs_db_path(sory_home.as_path()))
}

fn default_sory_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".sory");
    }
    PathBuf::from(".sory")
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["sory-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["sory-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["sory-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["sory-state-logs", "--level", "WARN"])
