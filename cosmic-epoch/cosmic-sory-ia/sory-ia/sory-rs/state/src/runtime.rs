...
impl StateRuntime {
    /// Initialize the state runtime using the provided sory home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `sory_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(sory_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        Self::init_inner(
            sory_home,
            default_provider,
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(sory_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let state_path = state_db_path(sory_home.as_path());
        let logs_path = logs_db_path(sory_home.as_path());
    ...
    /// Return the configured sory home directory for this runtime.
    pub fn sory_home(&self) -> &Path {
...
pub fn state_db_path(sory_home: &Path) -> PathBuf {
    sory_home.join(state_db_filename())
}
...
pub fn logs_db_path(sory_home: &Path) -> PathBuf {
    sory_home.join(logs_db_filename())
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        tokio::fs::create_dir_all(&sory_home)
            .await
            .expect("create sory home");
        let path = state_db_path(sory_home.as_path());
