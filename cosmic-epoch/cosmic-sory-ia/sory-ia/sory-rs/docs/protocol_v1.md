...
These are entities exit on the sory backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `sory` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `sory`
   - The core engine of sory
...
2. `Session`
   - The `sory`'s current configuration and state
   - `sory` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `sory` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `sory` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `sory`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `sory`, as `sory` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `sory` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `sory`)
...
- `Event`
  - These are messages sent on the `EQ` (`sory` -> UI)
...
sequenceDiagram
    ...
    user->>sory: Op::ConfigureSession
    sory-->>session: create session
    sory->>user: Event::SessionConfigured
