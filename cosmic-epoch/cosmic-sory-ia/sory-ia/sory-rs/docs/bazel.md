...
This repository uses Bazel to build the Rust workspace under `sory-rs`.
...
-   `rules_rs` imports third-party crates from `sory-rs/Cargo.toml` and
  `sory-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `sory_rust_crate`, which wraps `rust_library`,
...
- Each crate in `sory-rs/*/BUILD.bazel` typically uses `sory_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `sory_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `sory_rust_crate` to configure it.
