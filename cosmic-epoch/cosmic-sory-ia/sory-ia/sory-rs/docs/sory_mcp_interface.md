# sory MCP Server Interface [experimental]

This document describes sory's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local sory engine.

- Status: experimental and subject to change without notice
- Server binary: `sory mcp-server` (or `sory-mcp-server`)
- Transport: standard MCP over stdio (JSON-RPC 2.0, line-delimited)

## Overview

sory exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
- Notifications
  ...
  - `sory/event/*` stream notifications for live agent events
...
Run sory as an MCP server and connect an MCP client:

```bash
sory mcp-server | your_mcp_client
```
...
Use the separate `sory mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current sory build with `model/list`. The request accepts optional pagination inputs:
...
- `sory/event` with the serialized sory event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `sory` and `sory-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, sory mirrors the content blocks inside `structuredContent` alongside the `threadId`.

Example:

```json
{
  "content": [{ "type": "text", "text": "Hello from sory" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from sory"
  }
}
```
...
When sory needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:
