...
use sory_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use sory_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "sory plugin marketplace")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sory plugin marketplace add",
    after_help = "Examples:\n  sory plugin marketplace add ./path/to/marketplace\n  sory plugin marketplace add owner/repo --ref main\n  sory plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sory plugin marketplace upgrade",
    after_help = "Examples:\n  sory plugin marketplace upgrade\n  sory plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sory plugin marketplace remove",
    after_help = "Example:\n  sory plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>) -> Result<()> {
    ...
    let default_install_root = marketplace_install_root(config.sory_home.as_path());
