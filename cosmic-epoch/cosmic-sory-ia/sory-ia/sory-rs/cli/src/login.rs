...
//! interactive-session layers. Direct `sory login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `sory-login.log` artifact that
...
use sory_login::ServerOptions;
use sory_login::login_with_access_token;
use sory_login::login_with_api_key;
use sory_login::logout_with_revoke;
use sory_login::run_device_code_login;
...
const CHATGPT_LOGIN_DISABLED_MESSAGE: &str =
    "Sory account login is disabled. Use API key login instead.";
const API_KEY_LOGIN_DISABLED_MESSAGE: &str =
    "API key login is disabled. Use Sory account login instead.";
const ACCESS_TOKEN_LOGIN_DISABLED_MESSAGE: &str =
    "Access token login is disabled. Use API key login instead.";
const LOGIN_SUCCESS_MESSAGE: &str = "Successfully logged in";

/// Installs a small file-backed tracing layer for direct `sory login` flows.
///
...
/// command produce a durable `sory-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match sory_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("sory-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("sory_cli=info,sory_core=info,sory_login=info"));
    ...
    // Direct `sory login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `sory login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | sory login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv sory_ACCESS_TOKEN | sory login --with-access-token`.",
...
/// `sory login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match soryAuth::from_auth_storage(
        &config.sory_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
    )
    .await
    {
        Ok(Some(auth)) => match auth.auth_mode() {
            ...
            AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens => {
                eprintln!("Logged in using Sory account");
                std::process::exit(0);
            }
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout_with_revoke(&config.sory_home, config.cli_auth_credentials_store_mode).await {
        Ok(true) => {
