...
pub(super) fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.sory_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match sory_app_server::app_server_control_socket_path(&config.sory_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run sory app-server daemon version for more details.");
    }
    check
}
