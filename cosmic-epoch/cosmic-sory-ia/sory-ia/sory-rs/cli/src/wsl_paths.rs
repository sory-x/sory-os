...
pub use sory_utils_path::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\sory.zip").as_deref(),
            Some("/mnt/c/Temp/sory.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/sory.tgz").as_deref(),
            Some("/mnt/d/Work/sory.tgz")
        );
        assert!(win_path_to_wsl("/home/user/sory").is_none());
    }
