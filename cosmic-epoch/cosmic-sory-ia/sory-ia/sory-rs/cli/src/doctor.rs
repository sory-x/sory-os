//! Implements the `sory doctor` diagnostic report.
//!
...
use sory_api::ResponsesWebsocketClient;
...
use sory_install_context::InstallContext;
use sory_install_context::StandalonePlatform;
use sory_login::AuthDotJson;
use sory_login::AuthManager;
use sory_login::SORY_ACCESS_TOKEN_ENV_VAR;
use sory_login::SORY_API_KEY_ENV_VAR;
use sory_login::soryAuth;
use sory_login::OPENAI_API_KEY_ENV_VAR;
use sory_login::default_client::build_reqwest_client;
use sory_login::default_client::default_headers;
use sory_login::load_auth_dot_json;
use sory_model_provider::create_model_provider;
use sory_protocol::protocol::AskForApproval;
use sory_terminal_detection::Multiplexer;
use sory_terminal_detection::TerminalInfo;
use sory_terminal_detection::TerminalName;
use sory_terminal_detection::terminal_info;
...
/// Options for building a local sory diagnostic report.
///
...
///
/// This is the CLI entry point for sory doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (config_check, network_check, terminal_check, state_check, reachability_check) = tokio::join!(
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun sory doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        sory_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load sory config")
}
...
/// JSON support report emitted by `sory doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        sory_version: report.sory_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("sory_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "sory_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = sory_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH sory entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH sory #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @openai/sory would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update sory so the JS shim provides sory_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("sory_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("sory");
...
fn sory_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["sory"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "sory"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("sory_HOME: {}", config.sory_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.sory_home.join(sory_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.sory_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        SORY_API_KEY_ENV_VAR,
        SORY_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(&config.sory_home, config.cli_auth_credentials_store_mode) {
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run sory login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no sory credentials were found",
        )
        .details(details)
        .remediation("Run sory login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run sory login again."),
    }
}
...
fn stored_auth_mode(auth: &sory_login::AuthDotJson) -> &'static str {
    match stored_auth_mode_value(auth) {
        sory_app_server_protocol::AuthMode::ApiKey => "api_key",
        sory_app_server_protocol::AuthMode::Chatgpt => "chatgpt",
        sory_app_server_protocol::AuthMode::ChatgptAuthTokens => "chatgpt_auth_tokens",
        sory_app_server_protocol::AuthMode::AgentIdentity => "agent_identity",
    }
}

fn stored_auth_mode_value(auth: &AuthDotJson) -> sory_app_server_protocol::AuthMode {
    if let Some(mode) = auth.auth_mode {
        return mode;
    }
    if auth.openai_api_key.is_some() {
        sory_app_server_protocol::AuthMode::ApiKey
    } else {
        sory_app_server_protocol::AuthMode::Chatgpt
    }
}
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        sory_app_server_protocol::AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(SORY_API_KEY_ENV_VAR);
        ...
        sory_app_server_protocol::AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
        ...
        sory_app_server_protocol::AuthMode::AgentIdentity => {
            if auth
                .agent_identity
                .as_deref()
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["sory_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "sory-linux-sandbox helper",
        arg0_paths.sory_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.sory_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "sory_HOME", &config.sory_home);
    path_readiness(&mut details, "log dir", &config.log_dir);
    path_readiness(&mut details, "sqlite home", &config.sqlite_home);
    let state_db = sory_state::state_db_path(&config.sqlite_home);
    let log_db = sory_state::logs_db_path(&config.sqlite_home);
    ...
    rollout_stats_details(&mut details, &config.sory_home);
    ...
    if status == CheckStatus::Fail {
        check = check
            .remediation("Back up sory_HOME, then remove or repair the affected SQLite database.");
    }
    check
}
...
async fn sqlite_integrity_detail(
    ...
    match sory_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, sory_home: &Path) {
    let active = collect_rollout_stats(&sory_home.join("sessions"));
    let archived = collect_rollout_stats(&sory_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &soryAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match sory_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "sory_HOME was resolved without config",
        )
        .detail(format!("sory_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "sory_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_reachability_plan(config: &Config) -> ReachabilityPlan {
    let stored_auth =
        load_auth_dot_json(&config.sory_home, config.cli_auth_credentials_store_mode)
            .ok()
            .flatten();
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(SORY_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(SORY_ACCESS_TOKEN_ENV_VAR) {
        return ProviderAuthReachabilityMode::Chatgpt;
    }
    match stored_auth.map(stored_auth_mode_value) {
        Some(sory_app_server_protocol::AuthMode::ApiKey) => ProviderAuthReachabilityMode::ApiKey,
        Some(
            sory_app_server_protocol::AuthMode::Chatgpt
            | sory_app_server_protocol::AuthMode::ChatgptAuthTokens
            | sory_app_server_protocol::AuthMode::AgentIdentity,
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/sory");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("sory")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/sory");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("sory"),
            }
        );
    }
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            sory_self_exe: Some(PathBuf::from("/bin/sory")),
            sory_linux_sandbox_exe: Some(PathBuf::from("/bin/sory-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/sory-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.sory_self_exe, arg0_paths.sory_self_exe);
        assert_eq!(
            overrides.sory_linux_sandbox_exe,
            arg0_paths.sory_linux_sandbox_exe
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "sory_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("sory_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running sory."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running sory.".to_string())
        );
    }
    ...
    fn stored_auth_validation_rejects_missing_api_key() {
        let auth = AuthDotJson {
            auth_mode: Some(sory_app_server_protocol::AuthMode::ApiKey),
            openai_api_key: None,
            tokens: None,
            last_refresh: None,
            agent_identity: None,
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-sory-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-sory-doctor-mcp\" is not resolvable",
            )
        }));
    }
