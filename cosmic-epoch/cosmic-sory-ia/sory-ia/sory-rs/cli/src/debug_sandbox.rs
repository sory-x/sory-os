...
#[cfg(target_os = "macos")]
use sory_core::spawn::sory_SANDBOX_ENV_VAR;
...
#[cfg(target_os = "macos")]
use sory_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use sory_sandboxing::seatbelt::create_seatbelt_command_args;
...
pub async fn run_command_under_seatbelt(
    ...
    _sory_linux_sandbox_exe: Option<PathBuf>,
) -> anyhow::Result<()> {
...
async fn run_command_under_sandbox(
    ...
    let network = network_proxy
        .as_ref()
        .map(sory_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(sory_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let sory_linux_sandbox_exe = config
                .sory_linux_sandbox_exe
                .expect("sory-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use sory_windows_sandbox::spawn_windows_sandbox_session_elevated;
    use sory_windows_sandbox::spawn_windows_sandbox_session_legacy;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(sory_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_sory_home(
        cli_overrides,
        sory_linux_sandbox_exe,
        options,
        /*sory_home*/ None,
...
async fn load_debug_sandbox_config_with_sory_home(
    ...
    // For legacy configs, `sory sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            sory_linux_sandbox_exe: sory_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        sory_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config(
    ...
    if let Some(sory_home) = sory_home {
        builder = builder
            .sory_home(sory_home.clone())
            .fallback_cwd(Some(sory_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(sory_home.path().join("config.toml"), config)?;
        Ok(())
    }
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_sory_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_sory_home(
            ...
            Some(sory_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = sory_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
