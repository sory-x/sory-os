...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = sory_command(sory_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(sory_home.path()).await?;
    ...
    let mut list_cmd = sory_command(sory_home.path())?;
    ...
    let mut list_json_cmd = sory_command(sory_home.path())?;
    ...
    let mut get_cmd = sory_command(sory_home.path())?;
    ...
    assert!(stdout.contains("remove: sory mcp remove docs"));

    let mut get_json_cmd = sory_command(sory_home.path())?;
    get_json_cmd
