...
`sory-network-proxy` is sory's local network policy enforcement proxy. It runs:
...
`sory-network-proxy` reads from sory's merged `config.toml` (via `sory-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $sory_HOME/proxy/ (ca.pem + ca.key).
...
```bash
cargo run -p sory-network-proxy --
```
...
`sory-network-proxy` can be embedded as a library with a thin API:

```rust
use sory_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `sory-network-proxy` is embedded in managed sory runtime, policy decisions emit structured
OTEL-compatible events with `target=sory_otel.network_proxy`.

Event name:

- `sory.network_proxy.policy_decision`
...
This section documents the protections implemented by `sory-network-proxy`, and the boundaries of
