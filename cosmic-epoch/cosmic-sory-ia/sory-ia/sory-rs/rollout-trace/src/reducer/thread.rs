...
impl TraceReducer {
    ...
    /// Starts a sory turn inside an existing thread.
    pub(super) fn start_sory_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.sory_turns.contains_key(&sory_turn_id) {
            bail!("duplicate sory turn start for {sory_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.sory_turns.insert(
            sory_turn_id.clone(),
            soryTurn {
                sory_turn_id,
                thread_id,
    ...
    /// Marks a sory turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_sory_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.sory_turns.get(&sory_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "sory turn end for {sory_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.sory_turns.get_mut(&sory_turn_id) else {
            bail!("sory turn end referenced unknown turn {sory_turn_id}");
        };
