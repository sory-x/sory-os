//! Mapping from sory protocol events into raw rollout-trace events.
//!
...
//! `sory-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `sory-rollout-trace`.
//!
...
use sory_protocol::protocol::PatchApplyStatus;
use sory_protocol::protocol::TurnAbortReason;
use serde::Serialize;

use crate::AgentThreadId;
use crate::soryTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct soryTurnTraceEvent {
    pub context_turn_id: soryTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn sory_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<soryTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let sory_turn_id = event.turn_id.clone();
            Some(soryTurnTraceEvent {
                context_turn_id: sory_turn_id.clone(),
                payload: RawTraceEventPayload::soryTurnStarted {
                    sory_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(soryTurnTraceEvent {
                context_turn_id: sory_turn_id.clone(),
                payload: RawTraceEventPayload::soryTurnEnded {
                    sory_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a sory_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a sory_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a sory_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a sory_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a sory_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a sory_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a sory_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a sory_protocol::protocol::CollabCloseEndEvent),
}
