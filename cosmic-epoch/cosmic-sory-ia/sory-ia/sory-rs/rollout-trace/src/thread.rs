...
use crate::protocol_event::sory_turn_trace_event;
...
pub const sory_ROLLOUT_TRACE_ROOT_ENV: &str = "sory_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `sory_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the sory
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(sory_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed sory turn lifecycle events from protocol lifecycle events.
    pub fn record_sory_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            sory_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what sory did while executing that boundary.
    pub fn record_tool_call_event(&self, sory_turn_id: impl Into<soryTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(sory_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_sory_turn_id: impl Into<soryTurnId>,
    ...
    pub fn record_sory_turn_started(&self, sory_turn_id: impl Into<soryTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let sory_turn_id = sory_turn_id.into();
        context.append_with_context_best_effort(
            sory_turn_id.clone(),
            RawTraceEventPayload::soryTurnStarted {
                sory_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one sory turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        sory_turn_id: soryTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            sory_turn_id: Some(sory_turn_id),
        };
