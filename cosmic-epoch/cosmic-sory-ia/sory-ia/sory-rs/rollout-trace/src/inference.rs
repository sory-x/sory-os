...
const INFERENCE_CALL_ID_HEADER: &str = "x-sory-inference-call-id";
...
///
/// A sory turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one sory turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        sory_turn_id: soryTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                sory_turn_id: attempt.context.sory_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps sory-core focused on
    ...
    /// Records a provider stream that sory intentionally stopped consuming.
    ///
...
/// sory received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        sory_turn_id: Some(context.sory_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.sory_turn_id, "turn-1");
