...
//! - sensitive subpaths such as `.git`, `.agents`, and `.sory` remain
...
use sory_protocol::permissions::is_protected_metadata_name;
...
use sory_protocol::protocol::FileSystemSpecialPath;
use sory_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.sory`.
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(SoryErr::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_sory = temp_dir.path().join("real-sory");
        let logical_sory = logical_home.join(".sory");
        let real_memories = real_sory.join("memories");
        let logical_memories = logical_sory.join("memories");
        ...
        std::os::unix::fs::symlink(&real_sory, &logical_sory)
            .expect("create symlinked sory home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".sory"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_sory = path_to_string(&temp_dir.path().join(".sory"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_sory));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_sory)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
