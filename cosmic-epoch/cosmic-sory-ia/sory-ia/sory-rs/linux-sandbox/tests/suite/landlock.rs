...
fn sory_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_sory-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> sory_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<sory_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let sory_linux_sandbox_exe = Some(sory_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &sory_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(SoryErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(SoryErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<sory_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> sory_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(SoryErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sory_linux_sandbox_exe: Option<PathBuf> = Some(sory_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_sory_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_sory = tmpdir.path().join(".sory");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_sory).expect("create .sory");

    let git_target = dot_git.join("config");
    let sory_target = dot_sory.join("config.toml");
    ...
    let sory_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", sory_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".sory write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(sory_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_sory_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-sory");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_sory = tmpdir.path().join(".sory");
    symlink(&decoy, &dot_sory).expect("create .sory symlink");
    ...
    let sory_output = expect_denied(
        ...
        .await,
        ".sory symlink replacement should be denied",
    );
    assert_ne!(sory_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_sory_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(SoryErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".sory symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|sory|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_sory_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".sory"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .sory directory creation should be denied",
    );
    assert_ne!(mkdir_sory_output.exit_code, 0);
    assert!(!subdir.join(".sory").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = sory_linux_sandbox_exe()
        .parent()
