...
use sory_connectors::DirectoryListResponse;
use sory_connectors::filter::filter_disallowed_connectors;
use sory_connectors::merge::merge_connectors;
use sory_connectors::merge::merge_plugin_connectors;
use sory_core::config::Config;
pub use sory_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use sory_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use sory_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use sory_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use sory_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use sory_core::connectors::with_app_enabled_state;
...
async fn connector_auth(config: &Config) -> anyhow::Result<soryAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_sory_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require sory backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(config: &Config) -> Option<Vec<AppInfo>> {
    ...
    let connectors = sory_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = sory_connectors::list_all_connectors_with_options(
        cache_context,
...
async fn plugin_apps_for_config(config: &Config) -> Vec<AppConnectorId> {
    ...
    PluginsManager::new(config.sory_home.to_path_buf())
...
mod tests {
    ...
    use sory_connectors::metadata::connector_install_url;
    use sory_plugin::AppConnectorId;
