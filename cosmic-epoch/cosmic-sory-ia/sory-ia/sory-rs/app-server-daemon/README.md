# sory-app-server-daemon

> `sory-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`sory-app-server-daemon` backs the machine-readable `sory app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for sory instances launched over SSH, including fresh developer
...
```sh
sory app-server daemon start
sory app-server daemon restart
sory app-server daemon enable-remote-control
sory app-server daemon disable-remote-control
sory app-server daemon stop
sory app-server daemon version
sory app-server daemon bootstrap --remote-control
```
...
```sh
curl -fsSL https://chatgpt.com/sory/install.sh | sh
$HOME/.sory/packages/standalone/current/sory app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `sory_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes sory is installed through `install.sh` and always launches
the standalone managed binary under `sory_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `sory_HOME/packages/standalone/current/sory` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `sory_HOME/packages/standalone/current/sory` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `sory remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `sory_HOME`, so a concurrent
...
The daemon stores its local state under `sory_HOME/app-server-daemon/`:
