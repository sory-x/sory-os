...
#[cfg(unix)]
use crate::managed_install::resolved_managed_sory_bin;
...
async fn update_once(
    ...
    let managed_sory_bin = resolved_managed_sory_bin(&daemon.managed_sory_bin).await?;
    let managed_identity = executable_identity(&managed_sory_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_sory_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_sory_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_sory_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed sory binary {}",
            managed_sory_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://chatgpt.com/sory/install.sh")
        .await
        .context("failed to fetch standalone sory updater")?
        .error_for_status()
        .context("standalone sory updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone sory updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone sory updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone sory updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone sory updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone sory updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone sory updater exited with status {status}")
    }
}
