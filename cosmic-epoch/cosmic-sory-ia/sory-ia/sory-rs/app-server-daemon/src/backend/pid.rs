...
impl PidBackend {
    pub(crate) fn new(sory_bin: PathBuf, pid_file: PathBuf, remote_control_enabled: bool) -> Self {
    ...
    pub(crate) fn new_update_loop(sory_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            sory_bin,
            pid_file,
    ...
    pub(crate) async fn start(&self) -> Result<Option<u32>> {
        ...
        let mut command = Command::new(&self.sory_bin);
        command
