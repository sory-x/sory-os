...
use managed_install::managed_sory_bin;
#[cfg(unix)]
use managed_install::managed_sory_version;
use serde::Serialize;
use settings::DaemonSettings;
use sory_app_server_transport::app_server_control_socket_path;
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "sory app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_sory_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let sory_home = find_sory_home().context("failed to resolve sory_HOME")?;
        let socket_path = app_server_control_socket_path(sory_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = sory_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_sory_bin: managed_sory_bin(sory_home.as_path()),
        })
    }
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by sory app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_sory_version(managed_sory_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_sory_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_sory_bin)?;
        }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        Ok(BootstrapOutput {
            ...
            managed_sory_path: self.managed_sory_bin.clone(),
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_sory_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_sory_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_sory_bin(&self) -> Result<()> {
        if self.managed_sory_bin.is_file() {
            return Ok(());
        }

        let managed_sory_path = self.managed_sory_bin.display();
        Err(anyhow!(
            "managed standalone sory install not found at {managed_sory_path}\n\n\
             This command requires the standalone install managed by the sory installer, because \
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_sory_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            sory_bin: managed_sory_bin.to_path_buf(),
