...
use sory_network_proxy::ConfigReloader;
use sory_network_proxy::ConfigState;
use sory_network_proxy::NetworkMode;
use sory_network_proxy::NetworkProxy;
use sory_network_proxy::NetworkProxyConfig;
use sory_network_proxy::NetworkProxyConstraints;
use sory_network_proxy::NetworkProxyState;
use sory_network_proxy::build_config_state;
...
use sory_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/sory-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.sory".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/sory-readable");
    let unreadable = absolute_path("/tmp/sory-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/sory-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/sory-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/sory-proxy-use";
    let explicit_socket = "/tmp/sory-browser-use";
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .managed_by_sory(/*managed_by_sory*/ false)
        .build()
        .await?;
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_sory_subpaths() {
    ...
    // write to .sory/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.sory carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_sory_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .sory/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_sory_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .sory is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_sory_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_sory = repo_root.join(".sory");
    let config_toml = dot_sory.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_sory.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_sory = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_sory_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_sory),
        "missing {expected_dot_sory}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .sory/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".sory")).expect("create .sory");
    fs::write(
        vulnerable_root.join(".sory").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .sory/config.toml");
    ...
    let dot_sory_canonical = vulnerable_root_canonical.join(".sory");
