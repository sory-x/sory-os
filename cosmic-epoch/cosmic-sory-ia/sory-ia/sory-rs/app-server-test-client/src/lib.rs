...
use sory_app_server_protocol::DynamicToolSpec;
...
use sory_app_server_protocol::JSONRPCMessage;
use sory_app_server_protocol::JSONRPCNotification;
use sory_app_server_protocol::JSONRPCRequest;
...
use sory_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "sory-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $sory_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the sory app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "sory", version, about = "Bootstrap sory app-server", long_about = None)]
struct Cli {
    /// Path to the `sory` CLI binary. When set, requests use stdio by
    /// spawning `sory app-server` as a child process.
    #[arg(long, env = "sory_BIN", global = true)]
    sory_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--sory-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "sory_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `sory` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `sory app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/sory-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `sory app-server --listen`.
    ...
    /// Send a user message through the sory app-server.
    SendMessage {
        /// User message to send to sory.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the sory app-server.
    GetAccountRateLimits,
    /// List the available models from the sory app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the sory app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "sory_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let sory_bin = sory_bin.unwrap_or_else(|| PathBuf::from("sory"));
            serve(&sory_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(sory_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(sory_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(sory_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(sory_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if sory_bin.is_some() && url.is_some() {
        bail!("--sory-bin and --url are mutually exclusive");
    }
    if let Some(sory_bin) = sory_bin {
        return Ok(Endpoint::Spawnsory(sory_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if sory_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --sory-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(sory_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(sory_bin);
        if let Some(sory_bin_parent) = sory_bin.parent() {
            let mut path = OsString::from(sory_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", sory_bin.display()))?;
...
fn serve(sory_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/sory-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,sory_=trace {}",
        shell_quote(&sory_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started sory app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::Spawnsory(sory_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut soryClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = soryClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = soryClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (sory_bin, url) {
        (Some(_), Some(_)) => bail!("--sory-bin and --url are mutually exclusive"),
        (Some(sory_bin), None) => {
            let server = BackgroundAppServer::spawn(&sory_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve sory-app-server-test-client binary path")?;
...
impl soryClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::Spawnsory(sory_bin) => Self::spawn_stdio(sory_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(sory_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let sory_bin_display = sory_bin.display();
        let mut cmd = Command::new(sory_bin);
        if let Some(sory_bin_parent) = sory_bin.parent() {
            let mut path = OsString::from(sory_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut sory_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{sory_bin_display}` app-server"))?;

        let stdin = sory_app_server
            .stdin
            .take()
            .context("sory app-server stdin unavailable")?;
        let stdout = sory_app_server
            .stdout
            .take()
            .context("sory app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: sory_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `sory-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "sory-toy-app-server".to_string(),
                    title: Some("sory Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: sory_app_server_protocol::LoginAccountParams::Chatgpt {
                sory_streamlined_login: false,
            },
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: sory_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to sory app-server")?;
                    return Ok(());
                }
                bail!("sory app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from sory app-server")?;
                if bytes == 0 {
                    bail!("sory app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = sory_core::otel_init::build_provider(
            &config,
...
impl Drop for soryClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[sory app-server exited: {status}]");
            return;
        }
