...
pub enum AuthMode {
    /// OpenAI API key provided by the caller and stored by sory.
    ApiKey,
    /// ChatGPT OAuth managed by sory (tokens persisted and refreshed by sory).
    Chatgpt,
    ...
    ChatgptAuthTokens,
    /// Programmatic sory auth backed by a registered Agent Identity.
...
client_request_definitions! {
    ...
    /// Spawn a standalone process (argv vector) without a sory sandbox.
...
/// Superset of [`sory_file_search::FileMatch`]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
server_notification_definitions! {
    ...
    /// This event is internal-only. Used by sory Cloud.
