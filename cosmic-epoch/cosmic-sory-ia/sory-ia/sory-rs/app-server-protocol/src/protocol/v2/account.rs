...
use sory_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use sory_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use sory_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use sory_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that sory-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, sory attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// sory attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that sory was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `sory`).
