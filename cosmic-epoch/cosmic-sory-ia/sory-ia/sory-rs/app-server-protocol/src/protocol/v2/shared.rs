...
use sory_experimental_api_macros::ExperimentalApi;
use sory_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use sory_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use sory_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
use ts_rs::TS;
...
/// This translation layer make sure that we expose sory error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `SoryErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum SoryErrorInfo {
...
impl From<CoreSoryErrorInfo> for SoryErrorInfo {
    fn from(value: CoreSoryErrorInfo) -> Self {
        match value {
            CoreSoryErrorInfo::ContextWindowExceeded => SoryErrorInfo::ContextWindowExceeded,
            CoreSoryErrorInfo::UsageLimitExceeded => SoryErrorInfo::UsageLimitExceeded,
            CoreSoryErrorInfo::ServerOverloaded => SoryErrorInfo::ServerOverloaded,
            CoreSoryErrorInfo::CyberPolicy => SoryErrorInfo::CyberPolicy,
            CoreSoryErrorInfo::HttpConnectionFailed { http_status_code } => {
                SoryErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreSoryErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                SoryErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreSoryErrorInfo::InternalServerError => SoryErrorInfo::InternalServerError,
            CoreSoryErrorInfo::Unauthorized => SoryErrorInfo::Unauthorized,
            CoreSoryErrorInfo::BadRequest => SoryErrorInfo::BadRequest,
            CoreSoryErrorInfo::ThreadRollbackFailed => SoryErrorInfo::ThreadRollbackFailed,
            CoreSoryErrorInfo::SandboxError => SoryErrorInfo::SandboxError,
            CoreSoryErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                SoryErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreSoryErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                SoryErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreSoryErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                SoryErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreSoryErrorInfo::Other => SoryErrorInfo::Other,
        }
    }
}
