...
use sory_protocol::auth::KnownPlan as InternalKnownPlan;
...
#[tokio::test]
#[serial(sory_auth_env)]
async fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    let auth = soryAuth::from_auth_storage(
        dir.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
    )
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = soryAuth::from_auth_dot_json(
        sory_home.path(),
...
fn write_auth_file(params: AuthFileParams, sory_home: &Path) -> std::io::Result<String> {
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(SORY_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let _access_token_guard = EnvVarGuard::set(SORY_ACCESS_TOKEN_ENV_VAR, &agent_identity);

    let chatgpt_base_url = format!("{}/backend-api", server.uri());
    let _authapi_guard =
        EnvVarGuard::set("sory_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = super::load_auth(
        sory_home.path(),
        /*enable_sory_api_key_env*/ false,
        AuthCredentialsStoreMode::File,
    ...
    let soryAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(sory_home.path()).exists(),
...
#[tokio::test]
#[serial(sory_auth_env)]
async fn load_auth_keeps_sory_api_key_env_precedence() {
    let sory_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(SORY_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(SORY_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    let sory_home = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    login_with_api_key(sory_home.path(), "sk-test", AuthCredentialsStoreMode::File)
        .expect("seed api key");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(SORY_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let sory_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        sory_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/sory-backend/agent-identity",
        "aud": "sory-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let _authapi_guard =
        EnvVarGuard::set("sory_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = soryAuth::from_agent_identity_jwt(&jwt, Some(&chatgpt_base_url))
        .await
