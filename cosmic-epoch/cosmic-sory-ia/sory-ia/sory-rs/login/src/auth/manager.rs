...
use sory_agent_identity::decode_agent_identity_jwt;
use sory_agent_identity::fetch_agent_identity_jwks;
use sory_app_server_protocol::AuthMode;
use sory_app_server_protocol::AuthMode as ApiAuthMode;
...
use sory_client::SoryHttpClient;
use sory_config::types::AuthCredentialsStoreMode;
use sory_protocol::account::PlanType as AccountPlanType;
use sory_protocol::auth::PlanType as InternalPlanType;
use sory_protocol::auth::RefreshTokenFailedError;
use sory_protocol::auth::RefreshTokenFailedReason;
use thiserror::Error;
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "sory_REFRESH_TOKEN_URL_OVERRIDE";
pub const REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "sory_REVOKE_TOKEN_URL_OVERRIDE";
...
impl soryAuth {
    async fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(sory_home.to_path_buf(), storage_mode);
    ...
    pub fn uses_sory_backend(&self) -> bool {
        matches!(
            self,
    ...
    /// Returns `None` if sory backend auth does not expose an account id.
    ...
    /// Returns false if sory backend auth omits the FedRAMP claim.
    ...
    /// Returns `None` if sory backend auth does not expose an account email.
    ...
    /// Returns `None` if sory backend auth does not expose a ChatGPT user id.
...
pub const MISTRAL_API_KEY_ENV_VAR: &str = "MISTRAL_API_KEY";
...
pub const COHERE_API_KEY_ENV_VAR: &str = "COHERE_API_KEY";
pub const GROQ_API_KEY_ENV_VAR: &str = "GROQ_API_KEY";
pub const PERPLEXITY_API_KEY_ENV_VAR: &str = "PERPLEXITY_API_KEY";
pub const TOGETHER_API_KEY_ENV_VAR: &str = "TOGETHER_API_KEY";
pub const FIREWORKS_API_KEY_ENV_VAR: &str = "FIREWORKS_API_KEY";
...
pub const SORY_API_KEY_ENV_VAR: &str = "SORY_API_KEY";
pub const SORY_ACCESS_TOKEN_ENV_VAR: &str = "SORY_ACCESS_TOKEN";

/// All known provider API key environment variables.
pub const ALL_PROVIDER_ENV_VARS: &[(&str, &str)] = &[
    (OPENAI_API_KEY_ENV_VAR, "openai"),
    (ANTHROPIC_API_KEY_ENV_VAR, "anthropic"),
    (MISTRAL_API_KEY_ENV_VAR, "mistral"),
    (GEMINI_API_KEY_ENV_VAR, "gemini"),
    (GOOGLE_API_KEY_ENV_VAR, "google"),
    (DEEPSEEK_API_KEY_ENV_VAR, "deepseek"),
    (COHERE_API_KEY_ENV_VAR, "cohere"),
    (GROQ_API_KEY_ENV_VAR, "groq"),
    (PERPLEXITY_API_KEY_ENV_VAR, "perplexity"),
    (TOGETHER_API_KEY_ENV_VAR, "together"),
    (FIREWORKS_API_KEY_ENV_VAR, "fireworks"),
    (DEEPINFRA_API_KEY_ENV_VAR, "deepinfra"),
    (OPENROUTER_API_KEY_ENV_VAR, "openrouter"),
    (XAI_API_KEY_ENV_VAR, "xai"),
    (NVIDIA_API_KEY_ENV_VAR, "nvidia"),
    (CEREBRAS_API_KEY_ENV_VAR, "cerebras"),
];
...
/// Detect a provider API key from environment variables.
/// Returns `(provider_name, api_key)` if found.
pub fn detect_provider_api_key_from_env() -> Option<(String, String)> {
    for &(env_var, provider) in ALL_PROVIDER_ENV_VARS {
        if let Some(key) = read_non_empty_env_var(env_var) {
...
pub fn read_sory_api_key_from_env() -> Option<String> {
    read_non_empty_env_var(SORY_API_KEY_ENV_VAR)
}

pub fn read_sory_access_token_from_env() -> Option<String> {
    read_non_empty_env_var(SORY_ACCESS_TOKEN_ENV_VAR)
}
...
/// Delete the auth.json file inside `sory_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    sory_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(sory_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(sory_home, &auth_dot_json, auth_credentials_store_mode)
}
...
pub async fn enforce_login_restrictions(config: &AuthConfig) -> std::io::Result<()> {
    ...
    if let Some(expected_account_ids) = config.forced_chatgpt_workspace_id.as_deref() {
        let chatgpt_account_id = match &auth {
            soryAuth::ApiKey(_) => return Ok(()),
            soryAuth::AgentIdentity(_) => auth.get_account_id(),
            soryAuth::Chatgpt(_) | soryAuth::ChatgptAuthTokens(_) => {
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(sory_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(sory_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(sory_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(sory_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

async fn load_auth(
    sory_home: &Path,
    enable_sory_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    chatgpt_base_url: Option<&str>,
) -> std::io::Result<Option<soryAuth>> {
    // API key via env var takes precedence over any other auth method.
    if enable_sory_api_key_env && let Some(api_key) = read_sory_api_key_from_env() {
        return Ok(Some(soryAuth::from_api_key(api_key.as_str())));
    }
    ...
    if let Some(auth_dot_json) = ephemeral_storage.load()? {
        let auth = soryAuth::from_auth_dot_json(
            sory_home,
            auth_dot_json,
            AuthCredentialsStoreMode::Ephemeral,
            chatgpt_base_url,
    ...
    if let Some(agent_identity) = read_sory_access_token_from_env() {
        return soryAuth::from_agent_identity_jwt(&agent_identity, chatgpt_base_url)
            .await
            .map(Some);
    }
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(soryAuth::api_auth_mode),
            )
            .field(
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(soryAuth::get_account_id);
        let mode = if manager.has_external_api_key_auth()
            || cached_auth
                .as_ref()
                .is_some_and(soryAuth::is_external_chatgpt_tokens)
        {
    ...
    pub fn has_next(&self) -> bool {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(soryAuth::is_chatgpt_auth)
        {
            return false;
        }
...
/// hands out cloned `soryAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
/// `sory_core::config::Config`, but this trait keeps `sory-login` independent
/// from `sory-core`.
pub trait AuthManagerConfig {
    /// Returns the sory home directory used for auth storage.
...
impl Debug for AuthManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AuthManager")
            .field("sory_home", &self.sory_home)
            .field("inner", &self.inner)
            .field("enable_sory_api_key_env", &self.enable_sory_api_key_env)
            .field(
                "auth_credentials_store_mode",
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific soryAuth, for testing only.
    pub fn from_auth_for_testing(auth: soryAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            sory_home: PathBuf::from("non-existent"),
    ...
    /// Create an AuthManager with a specific soryAuth and sory home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: soryAuth, sory_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            permanent_refresh_failure: None,
        };
    ...
    pub fn auth_cached(&self) -> Option<soryAuth> {
        self.inner.read().ok().and_then(|c| c.auth.clone())
    }

    pub fn refresh_failure_for_auth(&self, auth: &soryAuth) -> Option<RefreshTokenFailedError> {
    ...
    pub async fn auth(&self) -> Option<soryAuth> {
    ...
    async fn reload_if_account_id_matches(
        ...
        let new_auth = self.load_auth_from_storage().await;
        let new_account_id = new_auth.as_ref().and_then(soryAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&soryAuth>, b: Option<&soryAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => match (a.api_auth_mode(), b.api_auth_mode()) {
                ...
                (ApiAuthMode::AgentIdentity, ApiAuthMode::AgentIdentity) => match (a, b) {
                    (soryAuth::AgentIdentity(a), soryAuth::AgentIdentity(b)) => {
                        a.record() == b.record()
                    }
                    _ => false,
                },
    ...
    fn auths_equal(a: Option<&soryAuth>, b: Option<&soryAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    async fn load_auth_from_storage(&self) -> Option<soryAuth> {
        load_auth(
            &self.sory_home,
            self.enable_sory_api_key_env,
            self.auth_credentials_store_mode,
    ...
    fn set_cached_auth(&self, new_auth: Option<soryAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    pub fn sory_api_key_env_enabled(&self) -> bool {
        self.enable_sory_api_key_env
    }
    ...
    async fn resolve_external_api_key_auth(&self) -> Option<soryAuth> {
        ...
        match external_auth.resolve().await {
            Ok(Some(tokens)) => Some(soryAuth::from_api_key(&tokens.access_token)),
            Ok(None) => None,
            Err(err) => {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        if auth_before_reload
            .as_ref()
            .is_some_and(soryAuth::is_api_key_auth)
        {
            return Ok(());
        }
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(soryAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            soryAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            soryAuth::Chatgpt(chatgpt_auth) => {
            ...
            soryAuth::ApiKey(_) | soryAuth::AgentIdentity(_) => Ok(()),
        };
    ...
    pub async fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.sory_home, self.auth_credentials_store_mode)?;
    ...
    pub async fn logout_with_revoke(&self) -> std::io::Result<bool> {
        ...
        let result = logout_all_stores(&self.sory_home, self.auth_credentials_store_mode)?;
        // Always reload to clear any cached auth (even if file absent).
        self.reload().await;
        Ok(result)
    }
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        ...
        self.auth_cached().as_ref().map(soryAuth::api_auth_mode)
    }
    ...
    pub fn auth_mode(&self) -> Option<AuthMode> {
        ...
        self.auth_cached().as_ref().map(soryAuth::auth_mode)
    }

    pub fn current_auth_uses_sory_backend(&self) -> bool {
        matches!(
            self.auth_mode(),
    ...
    fn is_stale_for_proactive_refresh(auth: &soryAuth) -> bool {
        let chatgpt_auth = match auth {
            soryAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
