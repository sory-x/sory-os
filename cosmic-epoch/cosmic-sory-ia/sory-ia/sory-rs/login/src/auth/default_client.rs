//! Default sory HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`SoryHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`sory_login::default_client`] from other crates in this
//! workspace.

use reqwest::header::HeaderMap;
use reqwest::header::HeaderValue;
use reqwest::header::USER_AGENT;
use sory_client::BuildCustomCaTransportError;
use sory_client::SoryHttpClient;
pub use sory_client::SoryRequestBuilder;
...
/// See https://github.com/openai/sory/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by sory. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "sory_cli_rs";
pub const SORY_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "sory_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-sory-residency";

pub use sory_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(SORY_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(SORY_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "sory-tui"
        || originator_value == "sory_vscode"
        || originator_value.starts_with("sory ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "sory_atlas" || originator_value == "sory_chatgpt_desktop"
}

pub fn get_sory_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized sory user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base sory user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default sory originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> SoryHttpClient {
    let inner = build_reqwest_client();
    SoryHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary sory HTTP traffic.
///
/// This starts from the standard sory user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `sory_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary sory HTTP traffic.
///
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_sory_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("sory_SANDBOX").as_deref() == Ok("seatbelt")
}
