...
// Keep in sync with the sory CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub sory_streamlined_login: bool,
    ...
    match path.as_str() {
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to sory failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "sory_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("sory_cli_simplified_flow".to_string(), "true".to_string()),
...
pub(crate) async fn persist_tokens_async(
    ...
    let sory_home = sory_home.to_path_buf();
    let (previous_auth, auth) = tokio::task::spawn_blocking(move || {
        let previous_auth = match load_auth_dot_json(&sory_home, auth_credentials_store_mode) {
            Ok(auth) => auth,
            Err(err) => {
        ...
        save_auth(&sory_home, &auth, auth_credentials_store_mode)?;
...
fn compose_success_url(
    ...
    if sory_streamlined_login {
        params.push(("sory_streamlined_login", "true".to_string()));
    }
    let qs = params
        .drain(..)
        .map(|(k, v)| format!("{}={}", k, urlencoding::encode(&v)))
...
/// Returns true when the OAuth callback represents a missing sory entitlement.
fn is_missing_sory_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_sory_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_sory_entitlement_error(error_code, error_description) {
        return "sory is not enabled for your workspace. Contact your workspace administrator to request access to sory.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_sory_entitlement_error(code, error_description) {
            (
                "You do not have access to sory".to_string(),
                "This account is not currently authorized to use sory in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to sory.".to_string(),
                "Contact your workspace administrator to get access to sory, then return to sory and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to sory to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_sory_entitlement_error;
    ...
    async fn persist_tokens_async_revokes_previous_auth_without_failing_login() -> anyhow::Result<()>
    {
        ...
        let auth = load_auth_dot_json(sory_home.path(), AuthCredentialsStoreMode::File)?
    ...
    fn compose_success_url_omits_streamlined_success_by_default() {
        ...
        assert_eq!(
            url.query_pairs()
                .find(|(key, _)| key == "sory_streamlined_login"),
            None
        );
    }
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_sory_entitlement");
        assert!(is_missing_sory_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to sory"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_sory_entitlement"));
    }
}
