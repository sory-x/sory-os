...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = sory_home.clone();

    let opts = ServerOptions {
        sory_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: sory_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = sory_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_sory_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let sory_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_sory_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to sory"),
        "error body should clearly explain the sory access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_sory_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to sory to retry"),
    ...
    assert!(
        !body.contains("get access to sory"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not sory"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        sory_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_sory_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        sory_home: first_sory_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: sory_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_sory_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        sory_home: second_sory_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: sory_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
