...
impl McpProcess {
    pub async fn new(sory_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(sory_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = sory_utils_cargo_bin::cargo_bin("sory-mcp-server")
            .context("should find binary for sory-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = sory_login::default_client::originator().value;
    ...
    pub async fn send_sory_tool_call(
        &mut self,
        params: soryToolCallParam,
    ) -> anyhow::Result<i64> {
        let sory_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "sory".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(sory_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "sory/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "sory/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `sory-mcp-server` child process.
        //
