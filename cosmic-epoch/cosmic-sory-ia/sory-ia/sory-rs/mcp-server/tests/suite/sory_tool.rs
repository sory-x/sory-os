...
use serde_json::json;
use sory_core::spawn::sory_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use sory_mcp_server::soryToolCallParam;
use sory_mcp_server::ExecApprovalElicitRequestParams;
use sory_mcp_server::ExecApprovalResponse;
use sory_mcp_server::PatchApprovalElicitRequestParams;
use sory_mcp_server::PatchApprovalResponse;
use sory_protocol::protocol::FileChange;
use sory_protocol::protocol::ReviewDecision;
use sory_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(sory_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a sory sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "sory" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let sory_request_id = mcp_process
        .send_sory_tool_call(soryToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            sory_request_id.to_string(),
            params.sory_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `sory` tool call completes and that the file was created.
    let sory_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(sory_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(sory_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    sory_mcp_tool_call_id: String,
    sory_event_id: String,
    thread_id: sory_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow sory to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let sory_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        sory_elicitation: "exec-approval".to_string(),
        sory_mcp_tool_call_id,
        sory_event_id,
        sory_command: command,
        sory_cwd: workdir.to_path_buf(),
        sory_call_id: "call1234".to_string(),
        sory_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "sory" tool request that will trigger the apply_patch command
    let sory_request_id = mcp_process
        .send_sory_tool_call(soryToolCallParam {
    ...
    // Verify the original `sory` tool call completes
    let sory_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(sory_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_sory_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = sory_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn sory_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `sory mcp` with a specific config.toml.
    let sory_home = TempDir::new()?;
    create_config_toml(sory_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(sory_home.path()).await?;
    ...
    .await??;
    assert_eq!(sory_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(sory_response.id, RequestId::Number(sory_request_id));
    assert_eq!(
        sory_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": sory_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("sory tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow sory to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        sory_elicitation: "patch-approval".to_string(),
        sory_mcp_tool_call_id,
        sory_event_id,
        sory_reason: reason,
        sory_grant_root: grant_root,
        sory_changes: changes,
        sory_call_id: "call1234".to_string(),
    })?;
...
/// Create a sory config that uses the mock server as the model provider.
