//! Asynchronous worker that executes a **sory** tool-call inside a spawned
...
use sory_core::config::Config as soryConfig;
...
/// To adhere to MCP `tools/call` response format, include the sory
...
/// Run a complete sory session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_sory_tool_session(
    id: RequestId,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!("Failed to start sory session: {e}"))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the sory submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_sory_uuid.lock().await.remove(&id);
        return;
    }

    run_sory_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_sory_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_sory_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<SoryThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(sory_event_to_notification(&event)) above has
