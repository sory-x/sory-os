...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the sory tool call.
    #[serde(rename = "threadId")]
    pub thread_id: ThreadId,
    pub sory_elicitation: String,
    pub sory_mcp_tool_call_id: String,
    pub sory_event_id: String,
    pub sory_call_id: String,
    pub sory_command: Vec<String>,
    pub sory_cwd: PathBuf,
    pub sory_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    sory_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow sory to run `{escaped_command}` in `{cwd}`?",
    ...
    let params = ExecApprovalElicitRequestParams {
        ...
        sory_mcp_tool_call_id: tool_call_id.clone(),
        sory_event_id: event_id.clone(),
        sory_call_id: call_id,
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, sory).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `sory`.
