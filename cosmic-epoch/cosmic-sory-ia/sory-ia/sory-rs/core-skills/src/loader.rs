...
use sory_config::default_project_root_markers;
use sory_config::merge_toml_values;
use sory_config::project_root_markers_from_config;
...
use sory_utils_plugins::plugin_namespace_for_skill_path;
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$sory_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$sory_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/sory/` on Unix, so treat
                // `/etc/sory/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
...
async fn discover_skills_under_root(
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by sory itself.
