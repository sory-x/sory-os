...
use sory_execpolicy::RuleRef;
use sory_execpolicy::rule::PatternToken;
use sory_execpolicy::rule::PrefixPattern;
use sory_execpolicy::rule::PrefixRule;
use std::sync::Arc;
use thiserror::Error;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: sory merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `sory-execpolicy`.
