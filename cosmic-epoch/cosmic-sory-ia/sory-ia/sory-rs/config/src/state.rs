...
use sory_app_server_protocol::ConfigLayer;
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("sory-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, sory_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.sory/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ConfigLayerSource::Mdm { .. } => None,
            ConfigLayerSource::System { file } => file.parent(),
            ConfigLayerSource::User { file, .. } => file.parent(),
            ConfigLayerSource::Project { dot_sory_folder } => Some(dot_sory_folder.clone()),
    ...
    /// Returns the `.sory/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$sory_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$sory_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_sory_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_sory_folder: current_project_dot_sory_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_sory_folder {
                ...
                if previous == current_project_dot_sory_folder
                    || !current_project_dot_sory_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_sory_folder = Some(current_project_dot_sory_folder);
        }
    }
