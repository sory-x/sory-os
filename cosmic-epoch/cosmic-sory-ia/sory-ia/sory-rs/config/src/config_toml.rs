//! Schema-heavy configuration TOML types used by sory.
...
use sory_app_server_protocol::ForcedChatgptWorkspaceIds as ApiForcedChatgptWorkspaceIds;
use sory_app_server_protocol::Tools;
use sory_app_server_protocol::UserSavedConfig;
use sory_features::FeaturesToml;
use sory_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use sory_protocol::config_types::WebSearchToolConfig;
...
use sory_utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.sory/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by sory will likely degrade model performance.
    ...
    /// file: Use a file in the sory home directory.
    ...
    /// Defaults to `$sory_SQLITE_HOME` when set. Otherwise uses `$sory_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where sory writes log files, for example `sory-tui.log`.
    /// Defaults to `$sory_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned sory Apps MCP requests.
    ...
    /// directories for `.sory` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different sory version.
    pub allow_sory_version_mismatch: Option<bool>,
...
pub use sory_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use sory_protocol::protocol::RealtimeVoice;
