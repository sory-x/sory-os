...
This module is the canonical place to **load and describe sory configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `sory_config::loader`:

- `load_config_layers_state(fs, sory_home, cwd_opt, cli_overrides, options, cloud_requirements, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
