...
use sory_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/sory/config.toml";
...
/// - system    `/etc/sory/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\sory\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/sory/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\sory\config.toml` (Windows)
/// - user      `${sory_HOME}/config.toml`
/// - profile   `${sory_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.sory/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.sory/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/sory/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, sory_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(sory_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sory_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so sory_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, sory_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/sory/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_sory_system_dir() -> PathBuf {
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_sory_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let sory_home_abs = AbsolutePathBuf::from_absolute_path(sory_home)?;
    let sory_home_normalized =
        normalize_path(sory_home_abs.as_path()).unwrap_or_else(|_| sory_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_sory_abs = dir.join(".sory");
        if !fs
            .get_metadata(&dot_sory_abs, /*sandbox*/ None)
            .await
        ...
        let dot_sory_normalized =
            normalize_path(dot_sory_abs.as_path()).unwrap_or_else(|_| dot_sory_abs.to_path_buf());
        if dot_sory_abs == sory_home_abs || dot_sory_normalized == sory_home_normalized {
            continue;
        }
        let config_file = dot_sory_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_sory_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_sory_abs.as_path())?;
...
/// from a file like `/etc/sory/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for sory to function correctly.
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("sory").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("sory").join("config.toml"))
        );
    }
}
