...
use sory_features::legacy_feature_keys;
use std::path::Path;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == sory_features::Feature::Artifact {
            continue;
        }
        if feature.id == sory_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sory_features::FeatureToml<
                    sory_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sory_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sory_features::FeatureToml<
                    sory_features::AppsMcpPathOverrideConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sory_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sory_features::FeatureToml<
                    sory_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
