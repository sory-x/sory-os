...
use sory_config::HooksFile;
use sory_config::types::McpServerConfig;
use sory_config::types::PluginConfig;
use sory_config::types::PluginMcpServerConfig;
use sory_core_skills::SkillMetadata;
use sory_core_skills::config_rules::SkillConfigRules;
use sory_core_skills::config_rules::resolve_disabled_skill_paths;
use sory_core_skills::config_rules::skill_config_rules_from_stack;
use sory_core_skills::loader::SkillRoot;
use sory_core_skills::loader::load_skills_from_roots;
use sory_exec_server::LOCAL_FS;
use sory_plugin::AppConnectorId;
use sory_plugin::LoadedPlugin;
use sory_plugin::PluginCapabilitySummary;
use sory_plugin::PluginHookSource;
use sory_plugin::PluginId;
use sory_plugin::PluginIdError;
use sory_plugin::PluginLoadOutcome;
...
use sory_utils_plugins::find_plugin_manifest_path;
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(sory_home.to_path_buf()).map_err(|err| err.to_string())?;
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_sory_home(
            sory_home,
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(sory_home, &source).map_err(|err| {
...
fn configured_plugins_from_sory_home(
    sory_home: &Path,
...
pub fn configured_curated_plugin_ids_from_sory_home(sory_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_sory_home(
        sory_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".sory-plugin/plugin.json"));
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(sory_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
fn normalize_plugin_mcp_server_value(
    ...
    if let Some(JsonValue::Object(mut oauth)) = object.remove("oauth") {
        if oauth.remove("callbackPort").is_some() {
            warn!(
                plugin = %plugin_root.display(),
                "plugin MCP server OAuth callbackPort is ignored; sory uses global MCP OAuth callback settings"
            );
        }
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = sory_home.join("plugins/.marketplace-plugin-source-staging");
