...
pub fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&sory_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(sory_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = sory_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
        ...
        for entry in fs::read_dir(&marketplace_root).map_err(|err| {
            ...
            if is_remote_plugin_cache_mutation_in_flight(sory_home, marketplace_name, &plugin_name)
            {
                continue;
            }
...
fn remote_plugin_cache_root(sory_home: &Path) -> PathBuf {
    sory_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(sory_home.path()),
        };
    ...
    fn stale_remote_plugin_cleanup_removes_old_shared_with_me_cache_and_keeps_canonical_cache() {
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = sory_home
            .path()
            .join(PLUGINS_CACHE_DIR)
