...
use sory_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use sory_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
...
// Keep this comfortably above a normal sync attempt so we do not race another sory process.
...
pub fn curated_plugins_repo_path(sory_home: &Path) -> PathBuf {
    sory_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn read_curated_plugins_sha(sory_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(sory_home).as_path())
}

fn curated_plugins_sha_path(sory_home: &Path) -> PathBuf {
    sory_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_openai_plugins_repo(sory_home: &Path) -> Result<String, String> {
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    match sync_openai_plugins_repo_via_git(sory_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_openai_plugins_repo_via_http(sory_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(sory_home) {
...
fn sync_openai_plugins_repo_via_git(sory_home: &Path, git_binary: &str) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(sory_home);
    let sha_path = sory_home.join(CURATED_PLUGINS_SHA_FILE);
...
fn sync_openai_plugins_repo_via_backup_archive(
    ...
    let repo_path = curated_plugins_repo_path(sory_home);
    let sha_path = curated_plugins_sha_path(sory_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(sory_home: &Path) -> bool {
    curated_plugins_repo_path(sory_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && sory_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
