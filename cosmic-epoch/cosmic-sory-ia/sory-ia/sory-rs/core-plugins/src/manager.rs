...
use crate::loader::configured_curated_plugin_ids_from_sory_home;
...
use sory_config::PluginConfigEdit;
use sory_config::apply_user_plugin_config_edits;
use sory_config::clear_user_plugin;
use sory_config::set_user_plugin_enabled;
use sory_config::types::PluginConfig;
use sory_config::version_for_toml;
use sory_core_skills::SkillMetadata;
use sory_hooks::plugin_hook_declarations;
...
impl PluginsManager {
    pub fn new(sory_home: PathBuf) -> Self {
        Self::new_with_restriction_product(sory_home, Some(Product::Sory))
    }
    ...
    pub fn new_with_restriction_product(
        ...
        // Product restrictions are enforced at marketplace admission time for a given sory_HOME:
        ...
        // contents of that sory_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single sory_HOME is only used by one product.
        Self {
            sory_home: sory_home.clone(),
            store: PluginStore::new(sory_home),
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if resolved.plugin_id.marketplace_name == OPENAI_CURATED_MARKETPLACE_NAME {
                let curated_plugin_version = read_curated_plugins_sha(self.sory_home.as_path())
                    .ok_or_else(|| {
        ...
        let sory_home = self.sory_home.clone();
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(sory_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = if self.store.active_plugin_root(&plugin_id).is_some() {
            Some(installed_plugin_telemetry_metadata(self.sory_home.as_path(), &plugin_id).await)
        } else {
            None
        };
        let store = self.store.clone();
        ...
        clear_user_plugin(&self.sory_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.sory_home.as_path());
        ...
        let config_result = if config_edits.is_empty() {
            ...
            apply_user_plugin_config_edits(&self.sory_home, config_edits).await
        };
        self.clear_cache();
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path =
            if matches!(plugin.source, MarketplacePluginSource::Git { .. }) && plugin.installed {
                ...
                let materialized = tokio::task::spawn_blocking(move || {
                    materialize_marketplace_plugin_source(sory_home.as_path(), &source)
                })
                .await
                .map_err(|err| {
                    MarketplaceError::InvalidPlugin(format!(
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest.paths,
            self.restriction_product,
            &sory_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
        )
        .await;
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_openai_plugins_repo(sory_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_sory_home(sory_home.as_path());
    ...
    fn run_non_curated_plugin_cache_refresh_loop(self: Arc<Self>) {
        loop {
            ...
            let refresh_result = match request.mode {
                NonCuratedCacheRefreshMode::IfVersionChanged => {
                    refresh_non_curated_plugin_cache(self.sory_home.as_path(), &request.roots)
                }
    ...
    fn marketplace_roots(
        ...
        let curated_repo_root = curated_plugins_repo_path(self.sory_home.as_path());
