...
pub(crate) fn write_curated_plugin_sha(sory_home: &Path) {
    write_curated_plugin_sha_with(sory_home, TEST_CURATED_PLUGIN_SHA);
}

pub(crate) fn write_curated_plugin_sha_with(sory_home: &Path, sha: &str) {
    write_file(&sory_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(sory_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let sory_home = AbsolutePathBuf::try_from(sory_home).expect("sory home should be absolute");
