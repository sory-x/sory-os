...
use sory_config::AppToolApproval;
...
use sory_config::McpServerOAuthConfig;
use sory_config::McpServerToolConfig;
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "sory-test@example.com"]);
    run_git(repo, &["config", "user.name", "sory Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(config_toml: &str, sory_home: &Path) -> PluginLoadOutcome {
    write_file(&sory_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(sory_home, sory_home).await;
    PluginsManager::new(sory_home.to_path_buf())
        .plugins_for_config(&config)
        .await
}

async fn load_config(sory_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(sory_home, cwd).await
}
...
"#;
    ...
    let outcome = load_plugins_from_config(config_toml, sory_home.path()).await;
...
"#,
    ...
    let config = load_config(sory_home.path(), sory_home.path()).await;
    let manager = PluginsManager::new(sory_home.path().to_path_buf());
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let sory_home = TempDir::new().unwrap();
    let plugin_a_root = sory_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = sory_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".sory-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".sory-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(sory_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![sory_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![sory_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(sory_home.path(), sory_home.path()).await;
    let outcome = PluginsManager::new(sory_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugins_for_config_reloads_when_plugin_hooks_enablement_changes() {
  ...
  "hooks": {
    ...
    let config_without_plugin_hooks = load_config(sory_home.path(), sory_home.path()).await;
    ...
    let config_with_plugin_hooks = load_config(sory_home.path(), sory_home.path()).await;
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".sory-plugin/plugin.json").is_file());
}
...
"#,
    ...
    write_file(
        &repo_root.join(".sory/config.toml"),
        r#"[[skills.config]]
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    let result = manager
        .sync_plugins_from_remote(
            &config,
            Some(&soryAuth::create_dummy_chatgpt_auth_for_testing()),
            /*additive_only*/ false,
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_sory() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["sory-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["sory-plugin".to_string()]);
}
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_sory_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_sory_home(tmp.path())
            .into_iter()
...
async fn load_plugins_ignores_project_config_files() {
    let sory_home = TempDir::new().unwrap();
    let project_root = sory_home.path().join("project");
    let plugin_root = sory_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".sory/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_sory_folder: AbsolutePathBuf::try_from(project_root.join(".sory")).unwrap(),
            },
    ...
    let outcome = load_plugins_from_layer_stack(
        ...
        &PluginStore::new(sory_home.path().to_path_buf()),
        Some(Product::Sory),
