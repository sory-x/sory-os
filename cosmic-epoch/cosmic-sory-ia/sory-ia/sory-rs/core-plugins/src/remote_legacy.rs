...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::Sory).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_sory_backend()) {
        request =
...
fn ensure_sory_backend_auth(
    auth: Option<&soryAuth>,
) -> Result<&soryAuth, RemotePluginMutationError> {
    let Some(auth) = auth else {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_sory_backend_auth(auth)?;
