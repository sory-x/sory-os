...
fn external_migration_is_experimental_and_disabled_by_default() {
    ...
    assert_eq!(
        ...
        Some(
            "Show a startup prompt when sory detects migratable external agent config for this machine or project."
        )
    );
...
#[test]
fn sory_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::soryHooks));
    assert_eq!(feature_for_key("sory_hooks"), Some(Feature::soryHooks));
}
