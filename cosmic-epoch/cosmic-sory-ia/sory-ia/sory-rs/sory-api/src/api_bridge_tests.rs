...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, SoryErr::ServerOverloaded));
}
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let SoryErr::CyberPolicy { message } = err else {
        panic!("expected SoryErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let SoryErr::InvalidRequest(message) = err else {
        panic!("expected SoryErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("sory_other"),
    );
    headers.insert(
        "x-sory-other-limit-name",
        http::HeaderValue::from_static("sory_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let SoryErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected SoryErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let SoryErr::UnexpectedStatus(err) = err else {
        panic!("expected SoryErr::UnexpectedStatus, got {err:?}");
    };
