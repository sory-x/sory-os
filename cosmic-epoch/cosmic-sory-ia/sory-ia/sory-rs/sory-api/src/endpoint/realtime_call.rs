...
use sory_client::RequestTelemetry;
use http::HeaderMap;
...
const MULTIPART_BOUNDARY: &str = "sory-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=sory-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "sory_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    use sory_client::Request;
    use sory_client::Response;
    use sory_client::StreamResponse;
    use sory_client::TransportError;
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/sory"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/sory/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
