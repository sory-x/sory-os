...
const X_SORY_TURN_STATE_HEADER: &str = "x-sory-turn-state";
...
async fn connect_websocket(
    ...
    // If a sory-specific CA bundle is configured, build an explicit rustls connector so this
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-sory-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-sory-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
