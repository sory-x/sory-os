...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_sory_home = AbsolutePathBuf::try_from(sory_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        sory_home: response_sory_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("sory_vscode/"));
    assert_eq!(response_sory_home, expected_sory_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "sory_app_server_daemon".to_string(),
            title: Some("sory App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("sory_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_sory_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let sory_home = TempDir::new()?;
    create_config_toml(sory_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "sory-backend".to_string(),
            title: Some("sory Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        sory_home.path(),
        &[(
            "sory_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("sory_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("sory_originator_via_env_var/"));
    assert_eq!(response_sory_home, expected_sory_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        sory_home.path(),
        &[("sory_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let sory_home = TempDir::new()?;
    let notify_file = sory_home.path().join("notify.json");
    let notify_capture = cargo_bin("sory-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(sory_home, server_uri, approval_policy, "")
}
