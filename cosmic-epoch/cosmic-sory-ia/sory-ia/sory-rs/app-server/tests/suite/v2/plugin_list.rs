...
use sory_app_server_protocol::PluginAuthPolicy;
...
use sory_core::config::set_project_trust_level;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "sory_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
const ALTERNATE_MARKETPLACE_RELATIVE_PATH: &str = ".claude-plugin/marketplace.json";
const ALTERNATE_PLUGIN_MANIFEST_RELATIVE_PATH: &str = ".claude-plugin/plugin.json";

fn write_plugins_enabled_config(sory_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    let home = sory_home.path().to_string_lossy().into_owned();
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.sory-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.sory-plugin/plugin.json"),
    ...
    assert_eq!(
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![PluginSummary {
                ...
                availability: sory_app_server_protocol::PluginAvailability::Available,
                interface: None,
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_sory_plugins_disabled() -> Result<()> {
    let sory_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_sory_plugins_setting() -> Result<()> {
    let sory_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.sory-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        sory_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.sory-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    auth_policy: PluginAuthPolicy::OnInstall,
                    availability: sory_app_server_protocol::PluginAvailability::Available,
                    interface: Some(sory_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let sory_home = TempDir::new()?;
    std::fs::create_dir_all(sory_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(sory_home.path())?;
    std::fs::write(
        sory_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "sory-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&sory_home, "sory-curated", "enabled-plugin")?;
    write_installed_plugin(&sory_home, "sory-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "sory-curated",
...
[plugins."enabled-plugin@sory-curated"]
enabled = true

[plugins."disabled-plugin@sory-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "sory-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@sory-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@sory-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@sory-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let sory_home = TempDir::new()?;
    std::fs::create_dir_all(sory_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&sory_home, "sory-curated", "shared-plugin")?;
    std::fs::write(
        sory_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "sory-curated",
...
[plugins."shared-plugin@sory-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".sory"))?;
    std::fs::write(
        workspace_enabled.path().join(".sory/config.toml"),
        r#"[plugins."shared-plugin@sory-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@sory-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@sory-curated");
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = sory_home.path().join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".sory-plugin"))?;
    std::fs::write(
        cached_plugin_root.join(".sory-plugin/plugin.json"),
        r##"{
  "name": "toolkit",
  "interface": {
    "displayName": "Toolkit",
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
    let sory_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(sory_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        sory_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_openai_curated_marketplace(sory_home.path(), &["linear"])?;
  ...
  {"id":"1","name":"linear","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    let marker_path = sory_home
        .path()
        .join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE);

    {
        let mut mcp = McpProcess::new_with_plugin_startup_tasks(sory_home.path()).await?;
        timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&installed_path.join(".sory-plugin/plugin.json")).await?;
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&sory_home, "chatgpt-global", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&sory_home, "chatgpt-global", "stale", "1.0.0")?;
    ...
    let old_path = sory_home
        .path()
        .join("plugins/cache/chatgpt-global/linear/1.0.0");
    let new_path = sory_home
        .path()
        .join("plugins/cache/chatgpt-global/linear/1.2.3");
    let stale_path = sory_home.path().join("plugins/cache/chatgpt-global/stale");
    ...
    wait_for_path_exists(&new_path.join(".sory-plugin/plugin.json")).await?;
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        sory_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        plugin.availability,
        sory_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn plugin_list_remote_marketplace_replaces_local_marketplace_with_same_name() -> Result<()> {
    ...
    let local_plugin_root = sory_home
        .path()
        .join(".agents/plugins/plugins/local-linear/.sory-plugin");
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_plugin_sync_config(sory_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(sory_home.path(), &["linear", "gmail"])?;
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(sory_home, marketplace_name, plugin_name, "local")
}
...
fn write_plugin_sync_config(sory_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        sory_home.join("config.toml"),
        format!(
            r#"
...
fn write_openai_curated_marketplace(
    ...
    let curated_root = sory_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.sory-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(sory_home.join(".tmp"))?;
    std::fs::write(
        sory_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(sory_home.join(".tmp"))?;
    std::fs::write(
        sory_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
