...
use sory_app_server_protocol::RemoteControlDisableResponse;
use sory_app_server_protocol::RemoteControlEnableResponse;
use sory_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let sory_home = TempDir::new()?;
    let _backend = BlockingRemoteControlBackend::start(sory_home.path()).await?;
    let mut mcp = McpProcess::new(sory_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_status_read_returns_connecting_status_after_enable() -> Result<()> {
    let sory_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(sory_home.path()).await?;
    let mut mcp = McpProcess::new(sory_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl BlockingRemoteControlBackend {
    async fn start(sory_home: &std::path::Path) -> Result<Self> {
