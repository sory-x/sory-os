...
const SERVER_MODEL: &str = "gpt-5.3-sory";
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: sory_app_server_protocol::TurnError {
                message: CYBER_POLICY_MESSAGE.to_string(),
                sory_error_info: Some(SoryErrorInfo::CyberPolicy),
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.sory_error_info == Some(SoryErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
