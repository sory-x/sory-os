...
use sory_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut sory_core::config::Config) {
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, sory_home: &Path) -> Result<()> {
    Mock::given(method("POST"))
        .and(path("/sory/analytics-events/events"))
        ...
        loop {
            ...
            for request in &requests {
                if request.method != "POST"
                    || request.url.path() != "/sory/analytics-events/events"
                {
                    continue;
                }
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "sory_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("sory_thread_initialized event should be present"))
}
