...
use sory_app_server_protocol::HookEventName;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                sory_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_read_rejects_invalid_remote_plugin_name() -> Result<()> {
    let sory_home = TempDir::new()?;
    write_remote_plugin_catalog_config(sory_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = McpProcess::new(sory_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
"#,
    ...
    write_installed_plugin(&sory_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(sory_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let sory_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&sory_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "sory-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(sory_home.path().join(".tmp"))?;
    std::fs::write(
        sory_home
            .path()
...
[plugins."demo-plugin@sory-curated"]
enabled = true

[hooks.state."demo-plugin@sory-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&sory_home, "sory-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "sory-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@sory-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            sory_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@sory-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            sory_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@sory-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            sory_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@sory-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_returns_app_needs_auth() -> Result<()> {
    ...
    let sory_home = TempDir::new()?;
    write_connectors_config(sory_home.path(), &server_url)?;
    write_chatgpt_auth(
        sory_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(sory_home: &TempDir) -> Result<()> {
    std::fs::write(
        sory_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/sory/apps", mcp_service);
...
fn write_connectors_config(sory_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
