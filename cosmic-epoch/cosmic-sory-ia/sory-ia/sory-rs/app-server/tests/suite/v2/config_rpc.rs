...
use serde_json::json;
use sory_app_server_protocol::AppConfig;
use sory_app_server_protocol::AppToolApproval;
use sory_app_server_protocol::AppsConfig;
...
use sory_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use sory_app_server_protocol::ToolsV2;
use sory_app_server_protocol::WriteStatus;
use sory_core::config::set_project_trust_level;
use sory_protocol::config_types::TrustLevel;
use sory_protocol::config_types::WebSearchContextSize;
use sory_protocol::config_types::WebSearchLocation;
...
fn write_config(sory_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let sory_home_path = sory_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(sory_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("sory")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let sory_home = TempDir::new()?;
    write_config(&sory_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_sory_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let sory_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", sory_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(sory_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let sory_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(sory_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(sory_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[sory_app_server_protocol::ConfigLayer],
