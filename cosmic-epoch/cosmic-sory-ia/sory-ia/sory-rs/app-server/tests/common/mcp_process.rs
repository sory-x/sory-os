...
use sory_app_server_protocol::ClientNotification;
...
use sory_app_server_protocol::ConfigReadParams;
use sory_app_server_protocol::ConfigValueWriteParams;
use sory_app_server_protocol::ExperimentalFeatureListParams;
use sory_app_server_protocol::FeedbackUploadParams;
use sory_app_server_protocol::FsCopyParams;
use sory_app_server_protocol::FsCreateDirectoryParams;
use sory_app_server_protocol::FsGetMetadataParams;
use sory_app_server_protocol::FsReadDirectoryParams;
use sory_app_server_protocol::FsReadFileParams;
use sory_app_server_protocol::FsRemoveParams;
use sory_app_server_protocol::FsUnwatchParams;
use sory_app_server_protocol::FsWatchParams;
use sory_app_server_protocol::FsWriteFileParams;
...
use sory_app_server_protocol::JSONRPCError;
...
use sory_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use sory_app_server_protocol::ProcessKillParams;
use sory_app_server_protocol::ProcessResizePtyParams;
use sory_app_server_protocol::ProcessSpawnParams;
use sory_app_server_protocol::ProcessWriteStdinParams;
...
use sory_app_server_protocol::ThreadRealtimeListVoicesParams;
...
pub const DEFAULT_CLIENT_NAME: &str = "sory-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "sory_APP_SERVER_DISABLE_MANAGED_CONFIG";

impl McpProcess {
    pub async fn new(sory_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(sory_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }

    pub async fn new_without_managed_config(sory_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(sory_home, &[(DISABLE_MANAGED_CONFIG_ENV_VAR, Some("1"))]).await
    }
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(sory_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(sory_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(sory_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(sory_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(sory_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(sory_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = sory_utils_cargo_bin::cargo_bin("sory-app-server")
            .context("should find binary for sory-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.current_dir(sory_home);
        ...
        cmd.env(
            ...
            sory_home.join("managed_config.toml"),
        );
        cmd.env_remove(SORY_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("sory-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: sory_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `sory-app-server` child process.
        //
