...
use sory_app_server_protocol::AuthMode as LoginAuthMode;
use sory_app_server_protocol::ChatgptAuthTokensRefreshParams;
...
use sory_app_server_protocol::ExperimentalApi;
...
use sory_app_server_protocol::experimental_required_message;
...
use sory_login::auth::ExternalAuth;
use sory_login::auth::ExternalAuthRefreshContext;
use sory_login::auth::ExternalAuthRefreshReason;
use sory_login::auth::ExternalAuthTokens;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: soryFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = sory_core::thread_store_from_config(config.as_ref(), state_db.clone());
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = sory_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = sory_request.experimental_reason()
        ...
        let serialization_scope = sory_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: sory_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match sory_request {
