...
use sory_config::types::PluginConfig;
...
use sory_core_plugins::marketplace::MarketplacePluginInstallPolicy;
...
use sory_core_plugins::marketplace_add::add_marketplace;
use sory_core_plugins::marketplace_add::is_local_marketplace_source;
use sory_external_agent_migration::build_mcp_config_from_external;
use sory_external_agent_migration::count_missing_commands;
use sory_external_agent_migration::count_missing_subagents;
use sory_external_agent_migration::hook_migration_event_names;
use sory_external_agent_migration::import_commands;
use sory_external_agent_migration::import_hooks;
use sory_external_agent_migration::import_subagents;
use sory_external_agent_migration::missing_command_names;
use sory_external_agent_migration::missing_subagent_names;
use sory_external_agent_sessions::ExternalAgentSessionMigration;
use sory_external_agent_sessions::detect_recent_sessions;
...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "sory.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "sory.external_agent_config.import";
...
impl ExternalAgentConfigService {
    pub(crate) fn new(sory_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(sory_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            sory_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.sory_home.join("config.toml"),
            |repo_root| repo_root.join(".sory").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.sory_home.join("hooks.json"),
            |repo_root| repo_root.join(".sory").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.sory_home.join("agents"),
            |repo_root| repo_root.join(".sory").join("agents"),
        );
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                .sory_home(self.sory_home.clone())
                .fallback_cwd(Some(self.sory_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.sory_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.sory_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.sory_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome = add_marketplace(self.sory_home.clone(), request).await;
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".sory").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".sory").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(());
            } else {
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Sory.matches_product_restriction(products))
            })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "sory");
    }
    rewritten
}
...
fn emit_migration_metric(
    ...
    let Some(metrics) = sory_otel::global() else {
        return;
    };
