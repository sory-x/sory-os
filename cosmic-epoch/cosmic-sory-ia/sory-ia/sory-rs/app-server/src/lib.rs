...
use sory_analytics::AppServerRpcTransport;
...
use sory_app_server_protocol::TextPosition as AppTextPosition;
use sory_app_server_protocol::TextRange as AppTextRange;
use sory_config::ConfigLoadError;
use sory_config::TextRange as CoreTextRange;
use sory_core::ExecPolicyError;
...
use sory_rollout::state_db as rollout_state_db;
...
const OTEL_SERVICE_NAME: &str = "sory-app-server";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_sory_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_sory_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let otel = sory_core::otel_init::build_provider(
        &config,
    ...
    sory_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    sory_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let state_db = match rollout_state_db::try_init(&config).await {
        Ok(state_db) => Some(state_db),
        Err(err) => {
            let state_db_path = sory_state::state_db_path(config.sqlite_home.as_path());
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match sory_core::personality_migration::maybe_migrate_personality(
                    &config.sory_home,
                    &config_toml,
                    state_db.clone(),
                )
                .await
                {
                    Ok(sory_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        sory_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | sory_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | sory_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
