...
use sory_app_server_protocol::Config as ApiConfig;
use sory_app_server_protocol::ConfigBatchWriteParams;
use sory_app_server_protocol::ConfigLayerMetadata;
...
use sory_app_server_protocol::OverriddenMetadata;
...
use sory_config::merge_toml_values;
use sory_core::config::deserialize_config_toml_with_base;
use sory_core::config::edit::ConfigEdit;
use sory_core::config::edit::ConfigEditsBuilder;
use sory_core::config::validate_feature_requirements_for_config_toml;
use sory_core::path_utils;
use sory_core::path_utils::SymlinkWritePaths;
use sory_core::path_utils::resolve_symlink_write_paths;
use sory_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.sory_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .sory/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_sory_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_sory_folder.display(),
        ),
