...
use sory_cloud_requirements::cloud_requirements_loader;
...
/// Shared app-server entry point for loading effective sory configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn replace_cloud_requirements_loader(
        ...
        let loader =
            cloud_requirements_loader(auth_manager, chatgpt_base_url, self.sory_home.clone());
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(sory_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        let mut config = Config::load_default_with_cli_overrides_for_sory_home(
        ...
        {
            let user_config_path = self.loader_overrides.user_config_path(self.sory_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = sory_core::config::ConfigBuilder::default()
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.sory_self_exe = self.arg0_paths.sory_self_exe.clone();
        config.sory_linux_sandbox_exe = self.arg0_paths.sory_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(sory_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(sory_home: PathBuf) -> Self {
        Self::new_for_tests(
            sory_home,
            Vec::new(),
