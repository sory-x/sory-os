...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match sory_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: sory_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> sory_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        sory_protocol::protocol::SessionSource::SubAgent(
            sory_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id,
                depth,
                agent_path,
...
) -> Option<sory_app_server_protocol::ActivePermissionProfile> {
...
pub(super) fn thread_response_sandbox_policy(
    permission_profile: &sory_protocol::models::PermissionProfile,
    cwd: &Path,
) -> sory_app_server_protocol::SandboxPolicy {
    let file_system_policy = permission_profile.file_system_sandbox_policy();
    let sandbox_policy = sory_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        permission_profile,
        &file_system_policy,
