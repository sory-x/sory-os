...
use sory_app_server_protocol::CommandMigration;
...
use sory_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
...
use sory_app_server_protocol::ExternalAgentConfigMigrationItemType;
use sory_app_server_protocol::HookMigration;
use sory_app_server_protocol::JSONRPCErrorError;
use sory_app_server_protocol::McpServerMigration;
use sory_app_server_protocol::MigrationDetails;
use sory_app_server_protocol::PluginsMigration;
...
use sory_external_agent_sessions::ExternalAgentSessionMigration as CoreSessionMigration;
use sory_external_agent_sessions::ImportedExternalAgentSession;
use sory_external_agent_sessions::PendingSessionImport;
use sory_external_agent_sessions::prepare_validated_session_imports;
use sory_external_agent_sessions::record_imported_session;
...
impl ExternalAgentConfigRequestProcessor {
    pub(crate) fn new(
        ...
        Self {
            outgoing,
            migration_service: ExternalAgentConfigService::new(sory_home.clone()),
            sory_home,
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| sory_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
                        ...
                        subagents: details
                            .subagents
                            .into_iter()
                            .map(|subagent| sory_app_server_protocol::SubagentMigration {
                                name: subagent.name,
                            })
                            .collect(),
                        commands: details
    ...
    async fn import_external_agent_session(
        ...
        if let Some(title) = title
            && let Some(name) = sory_core::util::normalize_thread_name(&title)
        {
            imported_thread
                .thread
    ...
    fn prepare_validated_session_imports(
        ...
        prepare_validated_session_imports(&self.sory_home, sessions)
    }

    fn record_imported_session(&self, source_path: &std::path::Path, imported_thread_id: ThreadId) {
        if let Err(err) = record_imported_session(&self.sory_home, source_path, imported_thread_id)
        {
            tracing::warn!(
                error = %err,
