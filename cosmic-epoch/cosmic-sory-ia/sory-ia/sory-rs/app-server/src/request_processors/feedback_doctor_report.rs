...
//! the configured sory executable as a subprocess, accepts only valid JSON from
//! `sory doctor --json`, derives a small set of Sentry tags, and otherwise
...
use sory_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `sory-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `sory doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn sory, finish before the timeout, or parse JSON means the
