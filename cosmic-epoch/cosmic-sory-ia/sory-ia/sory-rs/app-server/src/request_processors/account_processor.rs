...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "sory_APP_SERVER_LOGIN_ISSUER";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth.as_ref().map(soryAuth::api_auth_mode),
            plan_type: auth.as_ref().and_then(soryAuth::account_plan_type),
        }
    }
    ...
    async fn login_v2(
        ...
        match params {
            ...
            LoginAccountParams::Chatgpt {
                sory_streamlined_login,
            } => {
                self.login_chatgpt_v2(request_id, sory_streamlined_login)
                    .await;
            }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self.login_chatgpt_response(sory_streamlined_login).await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self.login_chatgpt_common(sory_streamlined_login).await?;
    ...
    async fn login_chatgpt_device_code_response(
        ...
        let opts = self
            .login_chatgpt_common(/*sory_streamlined_login*/ false)
            .await?;
            ...
            match auth {
                Some(auth) => {
                    ...
                    let (reported_auth_method, token_opt) =
                        if matches!(auth, soryAuth::AgentIdentity(_))
    ...
    > {
        ...
        let snapshots = client
            ...
            .await
            .map_err(|err| internal_error(format!("failed to fetch sory rate limits: {err}")))?;
        if snapshots.is_empty() {
            return Err(internal_error(
                "failed to fetch sory rate limits: no snapshots returned",
            ));
        }
