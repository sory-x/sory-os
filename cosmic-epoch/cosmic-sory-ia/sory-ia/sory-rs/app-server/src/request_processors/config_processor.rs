...
use sory_app_server_protocol::ConfigReadResponse;
use sory_app_server_protocol::ConfigRequirements;
use sory_app_server_protocol::ConfigRequirementsReadResponse;
use sory_app_server_protocol::ConfigValueWriteParams;
use sory_app_server_protocol::ConfigWriteErrorCode;
use sory_app_server_protocol::ConfigWriteResponse;
use sory_app_server_protocol::ConfiguredHookHandler;
use sory_app_server_protocol::ConfiguredHookMatcherGroup;
use sory_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use sory_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use sory_app_server_protocol::JSONRPCErrorError;
use sory_app_server_protocol::ManagedHooksRequirements;
use sory_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use sory_app_server_protocol::NetworkDomainPermission;
use sory_app_server_protocol::NetworkRequirements;
use sory_app_server_protocol::NetworkUnixSocketPermission;
...
use sory_config::HookEventsToml;
use sory_config::HookHandlerConfig as CoreHookHandlerConfig;
use sory_config::ManagedHooksRequirementsToml;
use sory_config::MatcherGroup as CoreMatcherGroup;
use sory_config::ResidencyRequirement as CoreResidencyRequirement;
use sory_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use sory_core::ThreadManager;
use sory_features::canonical_feature_for_key;
use sory_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    async fn refresh_apps_list_after_experimental_feature_enablement_set(&self) {
        ...
        if !config.features.apps_enabled_for_auth(
            auth.as_ref()
                .is_some_and(sory_login::soryAuth::uses_sory_backend),
        ) {
            return;
        }
    ...
    ) -> Result<sory_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = sory_core_plugins::toggles::collect_plugin_enabled_candidates(
    ...
    async fn emit_plugin_toggle_events(
        ...
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata = sory_core_plugins::loader::installed_plugin_telemetry_metadata(
                self.config_manager.sory_home(),
                &plugin_id,
            )
            .await;
            if enabled {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(sory_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(sory_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
...
) -> sory_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => sory_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: sory_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(sory_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(sory_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(sory_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: sory_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        sory_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        sory_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: sory_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        sory_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        sory_config::NetworkUnixSocketPermissionToml::None => NetworkUnixSocketPermission::None,
    }
}
