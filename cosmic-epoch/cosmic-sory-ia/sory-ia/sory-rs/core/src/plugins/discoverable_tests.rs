...
async fn list_tool_suggest_discoverable_plugins_deduplicates_allowlisted_configured_plugin() {
    ...
    let marketplace_root = sory_home
        .path()
...
"#
    ...
    write_file(
        &sory_home.path().join(crate::config::CONFIG_TOML_FILE),
        &format!(
            r#"[features]
plugins = true
...
async fn list_tool_suggest_discoverable_plugins_normalizes_description() {
    ...
    write_plugins_feature_config(sory_home.path());
    write_file(
        &curated_root.join("plugins/slack/.sory-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    PluginsManager::new(sory_home.path().to_path_buf())
...
async fn list_tool_suggest_discoverable_plugins_does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["build-ios-apps", "life-science-research"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.sory-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        normalized_logs
            .matches("build-ios-apps/.sory-plugin/plugin.json")
            .count(),
        1
    );
    assert_eq!(
        normalized_logs
            .matches("life-science-research/.sory-plugin/plugin.json")
            .count(),
        1
    );
}
