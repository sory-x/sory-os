...
pub(crate) fn write_plugins_feature_config(sory_home: &Path) {
    write_file(
...
pub(crate) async fn load_plugins_config(sory_home: &Path) -> crate::config::Config {
    ConfigBuilder::default()
        .sory_home(sory_home.to_path_buf())
        .fallback_cwd(Some(sory_home.to_path_buf()))
        .build()
        .await
