...
use sory_protocol::error::SandboxErr;
...
use sory_protocol::protocol::ExecCommandOutputDeltaEvent;
use sory_protocol::protocol::ExecOutputStream;
use sory_protocol::protocol::SandboxPolicy;
use sory_sandboxing::SandboxCommand;
use sory_sandboxing::SandboxManager;
use sory_sandboxing::SandboxTransformRequest;
use sory_sandboxing::SandboxType;
use sory_sandboxing::SandboxablePreference;
use sory_utils_absolute_path::AbsolutePathBuf;
use sory_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use sory_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: sory_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: sory_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            sory_linux_sandbox_exe: sory_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(SoryErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        sory_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = sory_otel::global() {
        let _ = metrics.counter(
            "sory.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_sory_home;
    use sory_windows_sandbox::run_windows_sandbox_capture_elevated;
    use sory_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        SoryErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let sory_home = find_sory_home().map_err(|err| {
        SoryErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve sory_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_elevated(
                sory_windows_sandbox::ElevatedSandboxCaptureRequest {
                    policy_json_or_preset: policy_str.as_str(),
                    sandbox_policy_cwd: &sandbox_cwd,
                    sory_home: sory_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(SoryErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, SoryErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(SoryErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(SoryErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(SoryErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// sory_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
...
pub(crate) fn resolve_windows_restricted_token_filesystem_overrides(
    ...
    let additional_deny_read_paths = sory_windows_sandbox::resolve_windows_deny_read_paths(
        file_system_sandbox_policy,
        sandbox_policy_cwd,
    )?;
...
pub(crate) fn resolve_windows_elevated_filesystem_overrides(
    ...
    let split_readable_roots: Vec<PathBuf> = file_system_sandbox_policy
        ...
        .into_iter()
        .map(sory_utils_absolute_path::AbsolutePathBuf::into_path_buf)
        .map(&normalize_path)
        .collect();
...
fn has_reopened_writable_descendant(
    writable_roots: &[sory_protocol::protocol::WritableRoot],
) -> bool {
