...
impl ResolvedTurnEnvironments {
    ...
    pub(crate) fn primary_environment(&self) -> Option<Arc<sory_exec_server::Environment>> {
        self.primary()
...
pub(crate) fn resolve_environment_selections(
    ...
    for selected_environment in environments {
        ...
        let environment = environment_manager
            ...
            .ok_or_else(|| {
                SoryErr::InvalidRequest(format!("unknown turn environment id `{environment_id}`"))
            })?;
...
"#,
        ...
        let cwd = AbsolutePathBuf::current_dir().expect("cwd");
        let manager = EnvironmentManager::from_sory_home(temp_dir.path(), test_runtime_paths())
            .await
