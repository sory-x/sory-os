//! Bridges Apps SDK-style `openai/fileParams` metadata into sory's MCP flow.
//!
//! Strategy:
...
//!
//! Model-visible schema masking is owned by `sory-mcp` alongside MCP tool
//! inventory, so this module only handles the execution-time argument rewrite.

use crate::session::session::Session;
use crate::session::turn_context::TurnContext;
use sory_api::upload_local_file;
...
async fn build_uploaded_local_argument_value(
    ...
    let Some(auth) = auth else {
        return Err(
            "ChatGPT auth is required to upload local files for sory Apps tools".to_string(),
        );
    };
    ...
    let upload_auth = sory_model_provider::auth_provider_from_auth(auth);
