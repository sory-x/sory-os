...
use sory_otel::GOAL_BUDGET_LIMITED_METRIC;
use sory_otel::GOAL_COMPLETED_METRIC;
use sory_otel::GOAL_CREATED_METRIC;
use sory_otel::GOAL_DURATION_SECONDS_METRIC;
use sory_otel::GOAL_TOKEN_COUNT_METRIC;
...
pub struct ExternalGoalPreviousGoal {
    goal_id: String,
    status: sory_state::ThreadGoalStatus,
    objective: String,
}

impl From<&sory_state::ThreadGoal> for ExternalGoalPreviousStatus {
    fn from(goal: &sory_state::ThreadGoal) -> Self {
        Self::Existing(ExternalGoalPreviousGoal::from(goal))
    }
}

impl From<&sory_state::ThreadGoal> for ExternalGoalPreviousGoal {
    fn from(goal: &sory_state::ThreadGoal) -> Self {
        Self {
...
pub struct ExternalGoalSet {
    pub goal: sory_state::ThreadGoal,
...
impl Session {
    ...
    pub(crate) async fn set_thread_goal(
                ...
                state_db
                    .replace_thread_goal(
                        ...
                        status
                            ...
                            .unwrap_or(sory_state::ThreadGoalStatus::Active),
        ...
        *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        let newly_active_goal = goal_status == sory_state::ThreadGoalStatus::Active
            && (replacing_goal
                || previous_status
                    .is_some_and(|status| status != sory_state::ThreadGoalStatus::Active));
        if newly_active_goal {
            ...
            .await;
        } else if goal_status != sory_state::ThreadGoalStatus::Active {
    ...
    async fn apply_external_thread_goal_status(self: &Arc<Self>, external_set: ExternalGoalSet) {
        ...
        match status {
            ...
            sory_state::ThreadGoalStatus::BudgetLimited => {
                if self.active_turn_context().await.is_none() {
                    self.clear_stopped_thread_goal_runtime_state().await;
                }
            }
            sory_state::ThreadGoalStatus::Paused | sory_state::ThreadGoalStatus::Complete => {
                self.clear_stopped_thread_goal_runtime_state().await;
            }
        }
    }
    ...
    fn emit_goal_terminal_metrics_if_status_changed(
        &self,
        previous_status: Option<sory_state::ThreadGoalStatus>,
        goal: &sory_state::ThreadGoal,
    ) {
        ...
        let counter = match goal.status {
            sory_state::ThreadGoalStatus::BudgetLimited => GOAL_BUDGET_LIMITED_METRIC,
            sory_state::ThreadGoalStatus::Complete => GOAL_COMPLETED_METRIC,
            sory_state::ThreadGoalStatus::Active | sory_state::ThreadGoalStatus::Paused => {
                return;
            }
        };
    ...
    ) -> anyhow::Result<Option<sory_state::ThreadGoalStatus>> {
    ...
    async fn account_thread_goal_progress(
        ...
        let goal = match outcome {
            sory_state::ThreadGoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    sory_state::ThreadGoalStatus::Active => false,
                    sory_state::ThreadGoalStatus::BudgetLimited => {
                        matches!(budget_limit_steering, BudgetLimitSteering::Suppressed)
                    }
                    sory_state::ThreadGoalStatus::Paused
                    | sory_state::ThreadGoalStatus::Complete => true,
                };
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
            ...
            sory_state::ThreadGoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == sory_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != sory_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        &self,
        state_db: &StateDbHandle,
        mode: sory_state::ThreadGoalAccountingMode,
        terminal_metric_emission: TerminalMetricEmission,
            ...
            .await?
        {
            ...
            sory_state::ThreadGoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
    ...
    async fn pause_active_thread_goal_for_interrupt(&self) -> anyhow::Result<()> {
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            sory_state::ThreadGoalAccountingMode::ActiveStatusOnly,
            TerminalMetricEmission::Emit,
        )
        .await?;
            ...
            Ok(Some(state_db)) => match state_db.get_thread_goal(self.conversation_id).await {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == sory_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
    ...
    async fn goal_continuation_candidate_if_active(
        ...
        if goal.status != sory_state::ThreadGoalStatus::Active {
...
pub(crate) fn protocol_goal_from_state(goal: sory_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
pub(crate) fn protocol_goal_status_from_state(
    ...
    match status {
        sory_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        sory_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        sory_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        sory_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

pub(crate) fn state_goal_status_from_protocol(
    status: ThreadGoalStatus,
) -> sory_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => sory_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => sory_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::BudgetLimited => sory_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => sory_state::ThreadGoalStatus::Complete,
    }
}
