...
use sory_app_server_protocol::ConfigLayerSource;
use sory_app_server_protocol::McpElicitationObjectType;
...
use sory_analytics::AppInvocation;
use sory_analytics::InvocationType;
use sory_analytics::build_track_events_context;
use sory_config::types::AppToolApproval;
use sory_features::Feature;
use sory_hooks::PermissionRequestDecision;
use sory_mcp::SORY_APPS_MCP_SERVER_NAME;
use sory_mcp::MCP_TOOL_SORY_APPS_META_KEY;
use sory_mcp::McpPermissionPromptAutoApproveContext;
use sory_mcp::SandboxState;
use sory_mcp::auth_elicitation_completed_result;
use sory_mcp::build_auth_elicitation_plan;
use sory_mcp::declared_openai_file_input_param_names;
use sory_mcp::mcp_permission_prompt_is_auto_approved;
use sory_otel::sanitize_metric_tag_value;
use sory_protocol::items::McpToolCallError;
use sory_protocol::items::McpToolCallItem;
use sory_protocol::items::McpToolCallStatus;
use sory_protocol::items::TurnItem;
use sory_protocol::mcp::CallToolResult;
use sory_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use sory_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use sory_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use sory_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use sory_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use sory_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use sory_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use sory_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use sory_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use sory_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use sory_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use sory_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use sory_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use sory_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
use sory_protocol::openai_models::InputModality;
use sory_protocol::protocol::AskForApproval;
use sory_protocol::protocol::McpInvocation;
...
use sory_protocol::request_user_input::RequestUserInputQuestion;
use sory_protocol::request_user_input::RequestUserInputQuestionOption;
...
use sory_utils_output_truncation::truncate_text;
...
const MCP_CALL_COUNT_METRIC: &str = "sory.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "sory.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "sory/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "sory.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "sory.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == SORY_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == SORY_APPS_MCP_SERVER_NAME {
    ...
    if server == SORY_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_sory_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        sory.mcp.target.id = Empty,
        sory.mcp.server_user_flow.triggered = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_sory_apps_auth_elicitation(
        sess,
        turn_context,
        call_id,
        &invocation.server,
...
async fn maybe_request_sory_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    call_id: &str,
    server: &str,
    ...
    if !sess
        ...
        .read()
        .await
        .is_host_owned_sory_apps_server(server)
    {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        sory_connectors::metadata::connector_install_url(
    ...
    let params = McpServerElicitationRequestParams {
        ...
        server_name: SORY_APPS_MCP_SERVER_NAME.to_string(),
    ...
    refresh_sory_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

#[expect(
    clippy::await_holding_invalid_type,
    reason = "sory Apps cache refresh reads through the session-owned manager guard"
)]
async fn refresh_sory_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    let mcp_tools_result = {
        let manager = sess.services.mcp_connection_manager.read().await;
        manager.hard_refresh_sory_apps_tools_cache().await
    };
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh sory Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        sory_linux_sandbox_exe: turn_context.sory_linux_sandbox_exe.clone(),
        #[allow(deprecated)]
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                sory_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_sory_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != SORY_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    sory_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, sory_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_SORY_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == SORY_APPS_MCP_SERVER_NAME {
        let mut sory_apps_meta = metadata
            .and_then(|metadata| metadata.sory_apps_meta.clone())
            .unwrap_or_default();
        sory_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_SORY_APPS_META_KEY.to_string(),
            serde_json::Value::Object(sory_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__sory_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == SORY_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == SORY_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        sory_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_SORY_APPS_META_KEY))
...
fn openai_file_input_params_for_server(
    ...
    (server == SORY_APPS_MCP_SERVER_NAME)
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == SORY_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_sory_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_sory_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, sory_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
