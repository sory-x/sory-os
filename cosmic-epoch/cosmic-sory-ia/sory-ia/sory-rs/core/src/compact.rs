...
use sory_analytics::soryCompactionEvent;
use sory_analytics::CompactionImplementation;
use sory_analytics::CompactionPhase;
use sory_analytics::CompactionReason;
use sory_analytics::CompactionStatus;
use sory_analytics::CompactionStrategy;
use sory_analytics::CompactionTrigger;
use sory_analytics::now_unix_seconds;
use sory_protocol::error::SoryErr;
use sory_protocol::error::Result as soryResult;
use sory_protocol::items::ContextCompactionItem;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(SoryErr::Interrupted) => {
                return Err(SoryErr::Interrupted);
            }
            Err(e @ SoryErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(soryCompactionEvent {
...
pub(crate) fn compaction_status_from_result<T>(result: &soryResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(SoryErr::Interrupted | SoryErr::TurnAborted) => CompactionStatus::Interrupted,
