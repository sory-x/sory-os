...
use sory_execpolicy::AmendError;
use sory_execpolicy::Decision;
use sory_execpolicy::Error as ExecPolicyRuleError;
...
use sory_execpolicy::PolicyParser;
use sory_execpolicy::RuleMatch;
use sory_execpolicy::blocking_append_allow_prefix_rule;
use sory_execpolicy::blocking_append_network_rule;
...
use sory_shell_command::is_dangerous_command::command_might_be_dangerous;
use sory_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(sory_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(sory_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &sory_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            sory_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
    ...
    let command_is_dangerous = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            sory_shell_command::is_dangerous_command::is_dangerous_powershell_words(command)
        }
    };
...
fn default_policy_path(sory_home: &Path) -> PathBuf {
    sory_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            sory_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and sory prompts the user to run outside the sandbox
