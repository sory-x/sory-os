...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.sory_home = temp_dir.path().join("sory-home").abs();
    config.cwd = config.sory_home.abs();
    std::fs::create_dir_all(&config.sory_home).expect("create sory home");
...
#[tokio::test]
async fn start_thread_uses_all_default_environments_from_sory_home() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.sory_home = temp_dir.path().join("sory-home").abs();
    config.cwd = config.sory_home.abs();
    std::fs::create_dir_all(&config.sory_home).expect("create sory home");
    std::fs::write(
        config.sory_home.join("environments.toml"),
        r#"
default = "dev"
...
"#,
    ...
    .expect("runtime paths");
    let environment_manager = Arc::new(
        sory_exec_server::EnvironmentManager::from_sory_home(
    ...
    let prompt_items = crate::prompt_debug::build_prompt_input_from_session(
        thread.thread.sory.session.as_ref(),
        Vec::<UserInput>::new(),
    )
    .await
    .expect("prompt input");
...
#[tokio::test]
async fn explicit_installation_id_skips_sory_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.sory_home = temp_dir.path().join("sory-home").abs();
    config.cwd = config.sory_home.abs();
    ...
    assert!(!config.sory_home.join(INSTALLATION_ID_FILENAME).exists());
    assert_eq!(thread.thread.sory.session.installation_id, installation_id);
...
async fn resumed_thread_keeps_paused_goal_paused() -> anyhow::Result<()> {
    ...
    assert_eq!(sory_state::ThreadGoalStatus::Paused, goal.status);
    assert!(
        resumed
            .thread
            .sory
