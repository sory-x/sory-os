...
use sory_analytics::CompactionTrigger;
use sory_analytics::HookRunFact;
use sory_analytics::build_track_events_context;
use sory_hooks::PermissionRequestDecision;
use sory_hooks::PermissionRequestOutcome;
use sory_hooks::PermissionRequestRequest;
use sory_hooks::PostToolUseOutcome;
use sory_hooks::PostToolUseRequest;
use sory_hooks::PreToolUseOutcome;
use sory_hooks::PreToolUseRequest;
use sory_hooks::SessionStartOutcome;
use sory_hooks::UserPromptSubmitOutcome;
use sory_hooks::UserPromptSubmitRequest;
use sory_otel::HOOK_RUN_DURATION_METRIC;
use sory_otel::HOOK_RUN_METRIC;
...
use sory_protocol::protocol::HookCompletedEvent;
use sory_protocol::protocol::HookEventName;
use sory_protocol::protocol::HookRunStatus;
use sory_protocol::protocol::HookRunSummary;
use sory_protocol::protocol::HookSource;
use sory_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = sory_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = sory_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (sory_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use sory_protocol::protocol::HookEventName;
    use sory_protocol::protocol::HookExecutionMode;
    use sory_protocol::protocol::HookHandlerType;
    use sory_protocol::protocol::HookRunStatus;
    use sory_protocol::protocol::HookScope;
    use sory_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    sory_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
