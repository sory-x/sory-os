...
const SORY_ARC_MONITOR_ENDPOINT_OVERRIDE: &str = "sory_ARC_MONITOR_ENDPOINT_OVERRIDE";
const SORY_ARC_MONITOR_TOKEN: &str = "sory_ARC_MONITOR_TOKEN";
...
pub(crate) async fn monitor_action(
    ...
    let auth = match turn_context.auth_manager.as_ref() {
        Some(auth_manager) => match auth_manager.auth().await {
            Some(auth) if auth.uses_sory_backend() => Some(auth),
            _ => None,
        },
        None => None,
    };
    let env_token = read_non_empty_env_var(SORY_ARC_MONITOR_TOKEN);
    if env_token.is_none() && auth.is_none() {
        return ArcMonitorOutcome::Ok;
    }

    let url = read_non_empty_env_var(SORY_ARC_MONITOR_ENDPOINT_OVERRIDE).unwrap_or_else(|| {
        format!(
            "{}/sory/safety/arc",
    ...
    if let Some(token) = env_token {
        ...
        request =
            request.headers(sory_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
async fn build_arc_monitor_request(
    ...
    ArcMonitorRequest {
        metadata: ArcMonitorMetadata {
            sory_thread_id: conversation_id.clone(),
