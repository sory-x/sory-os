...
pub fn sandbox_setup_is_complete(sory_home: &Path) -> bool {
    sory_windows_sandbox::sandbox_setup_is_complete(sory_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_sory_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = sory_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = sory_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if sory_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            sory_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "sory.windows_sandbox.elevated_setup_canceled"
    } else {
        "sory.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    sory_windows_sandbox::run_elevated_setup(
        sory_windows_sandbox::SandboxSetupRequest {
            policy,
            policy_cwd,
            command_cwd,
            env_map,
        ...
        sory_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
pub fn run_legacy_setup_preflight(
    ...
    sory_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        sory_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    sory_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let sory_home = request.sory_home;
    let active_profile = request.active_profile;
    let setup_sory_home = sory_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_sory_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_sory_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(sory_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let _ = metrics.record_duration(
        "sory.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "sory.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "sory.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        ...
        let _ = metrics.counter(
            "sory.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("originator", originator_tag)],
        );
    }
