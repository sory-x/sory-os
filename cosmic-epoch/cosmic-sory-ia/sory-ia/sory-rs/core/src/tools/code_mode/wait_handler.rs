        ...
        match payload {
                ...
                if tool_name.namespace.is_none() && tool_name.name.as_str() == WAIT_TOOL_NAME =>
            {
                ...
                let wait_response = exec
                    ...
                    .services
                    .code_mode_service
                    .wait(sory_code_mode::WaitRequest {
                ...
                if let sory_code_mode::WaitOutcome::LiveCell(response) = &wait_response
                    && !matches!(response, sory_code_mode::RuntimeResponse::Yielded { .. })
                {
                    ...
                    let runtime_cell_id = match response {
                        sory_code_mode::RuntimeResponse::Yielded { cell_id, .. }
                        | sory_code_mode::RuntimeResponse::Terminated { cell_id, .. }
                        | sory_code_mode::RuntimeResponse::Result { cell_id, .. } => cell_id,
                    };
                    exec.session
                        .services
