...
impl CodeModeExecuteHandler {
    ...
    async fn execute(
        ...
        let args =
            sory_code_mode::parse_exec_source(&code).map_err(FunctionCallError::RespondToModel)?;
        let exec = ExecContext { session, turn };
        let enabled_tools =
            sory_tools::collect_code_mode_tool_definitions(&self.nested_tool_specs);
        ...
        let response = exec
            ...
            .services
            .code_mode_service
            .execute(sory_code_mode::ExecuteRequest {
        ...
        if !matches!(response, sory_code_mode::RuntimeResponse::Yielded { .. }) {
