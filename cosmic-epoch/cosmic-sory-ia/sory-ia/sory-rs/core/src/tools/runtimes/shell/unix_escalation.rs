...
use sory_execpolicy::MatchOptions;
...
use sory_protocol::protocol::NetworkPolicyRuleAction;
...
use sory_shell_command::bash::parse_shell_lc_plain_commands;
use sory_shell_command::bash::parse_shell_lc_single_command_prefix;
use sory_shell_escalation::EscalateServer;
use sory_shell_escalation::EscalationDecision;
use sory_shell_escalation::EscalationExecution;
use sory_shell_escalation::EscalationPermissions;
use sory_shell_escalation::EscalationPolicy;
use sory_shell_escalation::EscalationSession;
use sory_shell_escalation::ExecParams;
use sory_shell_escalation::ExecResult;
use sory_shell_escalation::PreparedExec;
use sory_shell_escalation::ResolvedPermissionProfile;
use sory_shell_escalation::ShellCommandExecutor;
use sory_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Sory(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        arg0,
        sandbox_policy_cwd,
        sory_linux_sandbox_exe: ctx.turn.sory_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    sory_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<sory_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["sory_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            sory_linux_sandbox_exe: self.sory_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Sory(SoryErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Sory(SoryErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
