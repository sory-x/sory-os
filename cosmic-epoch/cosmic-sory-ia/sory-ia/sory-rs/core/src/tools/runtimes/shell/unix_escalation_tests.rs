...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.sory_home)
        .context("recreate sory home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .sory_home
        .join(sory_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command =
        sory_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(sory_execpolicy::Policy::empty())),
        ...
        stopwatch: sory_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = tokio::time::timeout(
        Duration::from_secs(5),
        sory_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        sory_shell_escalation::EscalationDecision::Escalate(
            sory_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
