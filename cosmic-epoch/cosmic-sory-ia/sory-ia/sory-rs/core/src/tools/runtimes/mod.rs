...
*/
use crate::exec_env::SORY_THREAD_ID_ENV_VAR;
...
#[cfg(target_os = "macos")]
use sory_network_proxy::sory_PROXY_GIT_SSH_COMMAND_MARKER;
use sory_network_proxy::PROXY_ACTIVE_ENV_KEY;
use sory_network_proxy::PROXY_ENV_KEYS;
#[cfg(target_os = "macos")]
use sory_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
...
pub(crate) fn exec_env_for_sandbox_permissions(
        ...
        && env.contains_key(PROXY_ACTIVE_ENV_KEY)
    {
        ...
        // Only macOS injects a sory-owned SSH wrapper for the managed SOCKS proxy.
        #[cfg(target_os = "macos")]
        if env
            .get(PROXY_GIT_SSH_COMMAND_ENV_KEY)
            .is_some_and(|command| command.starts_with(sory_PROXY_GIT_SSH_COMMAND_MARKER))
        {
...
/// runtime-only vars like `sory_THREAD_ID` without pretending they came from
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if let Some(thread_id) = env.get(SORY_THREAD_ID_ENV_VAR) {
        override_env.insert(SORY_THREAD_ID_ENV_VAR.to_string(), thread_id.clone());
    }
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    build_override_exports_for_keys("__sory_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__sory_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__sory_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__sory_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_sory_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_sory_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", sory_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__sory_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
