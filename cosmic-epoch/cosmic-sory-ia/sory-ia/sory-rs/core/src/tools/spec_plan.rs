...
use sory_tools::ResponsesApiNamespace;
use sory_tools::ResponsesApiNamespaceTool;
use sory_tools::TOOL_SEARCH_TOOL_NAME;
use sory_tools::ToolCall as ExtensionToolCall;
use sory_tools::ToolEnvironmentMode;
use sory_tools::ToolExecutor;
use sory_tools::ToolName;
use sory_tools::ToolOutput;
use sory_tools::ToolSpec;
use sory_tools::ToolsConfig;
use sory_tools::collect_code_mode_exec_prompt_tool_definitions;
use sory_tools::default_namespace_description;
...
fn spec_for_model_request(
    ...
    if config.code_mode_enabled
        ...
        && sory_code_mode::is_code_mode_nested_tool(spec.name())
    {
        sory_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    config.code_mode_only_enabled
        && exposure != ToolExposure::DirectModelOnly
        && sory_code_mode::is_code_mode_nested_tool(&sory_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, sory_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| sory_code_mode::ToolNamespaceDescription {
...
fn append_extension_tool_executors(
    ...
    let mut reserved_tool_names = registered_executors
        ...
        .collect::<HashSet<_>>();
    if config.code_mode_enabled {
        reserved_tool_names.insert(ToolName::plain(sory_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(sory_code_mode::WAIT_TOOL_NAME));
    }
...
impl ToolExecutor<ToolInvocation> for MultiAgentV2NamespaceOverride {
    ...
    ) -> Result<Box<dyn ToolOutput>, sory_tools::FunctionCallError> {
...
fn compare_code_mode_tools(
    left: &sory_code_mode::ToolDefinition,
    right: &sory_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, sory_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &sory_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, sory_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
