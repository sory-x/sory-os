...
use sory_protocol::protocol::CollabAgentRef;
use sory_protocol::protocol::CollabAgentStatusEntry;
...
pub(crate) fn collab_spawn_error(err: SoryErr) -> FunctionCallError {
    match err {
        SoryErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        SoryErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: SoryErr) -> FunctionCallError {
    match err {
        SoryErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.sory_linux_sandbox_exe = turn.sory_linux_sandbox_exe.clone();
    #[allow(deprecated)]
...
fn find_spawn_agent_model_name(
    available_models: &[sory_protocol::openai_models::ModelPreset],
    requested_model: &str,
