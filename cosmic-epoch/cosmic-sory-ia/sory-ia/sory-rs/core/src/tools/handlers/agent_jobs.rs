...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<sory_state::StateRuntime>, FunctionCallError> {
...
async fn run_agent_job_loop(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    db: Arc<sory_state::StateRuntime>,
    job_id: String,
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(sory_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(sory_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, sory_state::AgentJobItemStatus::Running) {
...
fn render_instruction_template(instruction: &str, row_json: &Value) -> String {
    const OPEN_BRACE_SENTINEL: &str = "__sory_OPEN_BRACE__";
    const CLOSE_BRACE_SENTINEL: &str = "__sory_CLOSE_BRACE__";
...
fn job_runtime_timeout(job: &sory_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &sory_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &sory_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
