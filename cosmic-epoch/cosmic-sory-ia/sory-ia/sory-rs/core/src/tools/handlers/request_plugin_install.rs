...
use sory_tools::DiscoverableToolAction;
use sory_tools::DiscoverableToolType;
use sory_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use sory_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use sory_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use sory_tools::RequestPluginInstallArgs;
use sory_tools::RequestPluginInstallEntry;
use sory_tools::RequestPluginInstallResult;
use sory_tools::ToolName;
use sory_tools::ToolSpec;
use sory_tools::all_requested_connectors_picked_up;
use sory_tools::build_request_plugin_install_elicitation_request;
use sory_tools::collect_request_plugin_install_entries;
use sory_tools::filter_request_plugin_install_discoverable_tools_for_client;
use sory_tools::verified_connector_install_completed;
...
impl ToolExecutor<ToolInvocation> for RequestPluginInstallHandler {
    ...
    async fn handle(
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("sory-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in sory-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.sory_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    sory_home: &sory_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    turn: &crate::session::turn_context::TurnContext,
    tool: &DiscoverableTool,
    auth: Option<&sory_login::soryAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_sory_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh sory apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
