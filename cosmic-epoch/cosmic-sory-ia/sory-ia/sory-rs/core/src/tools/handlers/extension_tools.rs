...
pub(crate) struct ExtensionToolAdapter(Arc<dyn sory_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn sory_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl sory_extension_api::ToolExecutor<sory_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> sory_tools::ToolName {
            sory_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> Option<sory_tools::ToolSpec> {
            Some(sory_tools::ToolSpec::Function(
                sory_tools::ResponsesApiTool {
                    name: "extension_echo".to_string(),
                    description: "Echoes arguments.".to_string(),
                    strict: true,
                    parameters: sory_tools::parse_tool_input_schema(&json!({
                        "type": "object",
                        "properties": {
                            "message": { "type": "string" },
                        },
        ...
        async fn handle(
            &self,
            _call: sory_tools::ToolCall,
        ) -> Result<Box<dyn sory_tools::ToolOutput>, sory_tools::FunctionCallError> {
            Ok(Box::new(sory_tools::JsonToolOutput::new(
                json!({ "ok": true }),
            )))
        }
    }
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: sory_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = sory_tools::JsonToolOutput::new(json!({ "ok": true }));
