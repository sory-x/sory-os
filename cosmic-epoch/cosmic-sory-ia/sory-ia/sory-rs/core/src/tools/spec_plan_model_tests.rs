...
use sory_models_manager::model_info::with_config_overrides;
...
use sory_tools::ToolSpec;
use sory_tools::ToolsConfig;
use sory_tools::ToolsConfigParams;
use sory_tools::UnifiedExecShellMode;
use sory_tools::ZshForkConfig;
use sory_tools::mcp_call_tool_result_output_schema;
use sory_tools::mcp_tool_to_deferred_responses_api_tool;
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    let serialized = serde_json::to_value(ToolSpec::Function(
        mcp_tool_to_deferred_responses_api_tool(
            &ToolName::namespaced("mcp__sory_apps__", "lookup_order"),
            &tool,
        )
...
fn build_specs_with_inputs_for_test(
    ...
    extension_tool_executors: &[Arc<
        dyn sory_extension_api::ToolExecutor<sory_extension_api::ToolCall>,
    >],
    dynamic_tools: &[DynamicToolSpec],
) -> Vec<ToolSpec> {
...
#[tokio::test]
async fn test_build_specs_gpt5_sory_default() {
...
#[tokio::test]
async fn test_build_specs_gpt51_sory_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_build_specs_gpt5_sory_unified_exec_web_search() {
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt51_sory_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
...
#[tokio::test]
async fn test_gpt_5_1_sory_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_sory_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4-mini",
        &features,
...
#[tokio::test]
async fn test_gpt_5_1_sory_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
    ...
    assert_eq!(
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\sory\sory-execve-wrapper"
                } else {
                    "/opt/sory/sory-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshFork(ZshForkConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/sory/zsh").unwrap(),
...
async fn search_tool_description_falls_back_to_connector_name_without_description() {
    ...
    let tools = build_specs(
        ...
        Some(vec![ToolInfo {
            ...
            callable_namespace: "mcp__sory_apps__calendar".to_string(),
            namespace_description: None,
            tool: mcp_tool(
