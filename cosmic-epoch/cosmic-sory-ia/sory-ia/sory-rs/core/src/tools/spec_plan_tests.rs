...
use sory_extension_api::ToolCall as ExtensionToolCall;
use sory_extension_api::ToolExecutor;
...
use sory_protocol::models::VIEW_IMAGE_TOOL_NAME;
use sory_protocol::openai_models::InputModality;
use sory_protocol::openai_models::ModelInfo;
use sory_protocol::openai_models::WebSearchToolType;
use sory_protocol::protocol::SessionSource;
use sory_protocol::protocol::SubAgentSource;
use sory_tools::AdditionalProperties;
use sory_tools::DiscoverablePluginInfo;
use sory_tools::DiscoverableTool;
use sory_tools::FreeformTool;
use sory_tools::JsonSchema;
use sory_tools::JsonSchemaPrimitiveType;
use sory_tools::JsonSchemaType;
use sory_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use sory_tools::ResponsesApiNamespaceTool;
use sory_tools::ResponsesApiTool;
use sory_tools::ResponsesApiWebSearchFilters;
use sory_tools::ResponsesApiWebSearchUserLocation;
use sory_tools::TOOL_SEARCH_TOOL_NAME;
use sory_tools::ToolEnvironmentMode;
use sory_tools::ToolName;
use sory_tools::ToolsConfigParams;
use sory_tools::mcp_call_tool_result_output_schema;
use sory_tools::request_user_input_available_modes;
...
const sory_APPS_MCP_SERVER_NAME: &str = "sory_apps";
...
fn extension_tool_executor(
    ...
    impl ToolExecutor<ExtensionToolCall> for SpecOnlyExtensionExecutor {
        ...
        ) -> Result<Box<dyn sory_tools::ToolOutput>, sory_tools::FunctionCallError> {
...
#[test]
fn test_full_toolset_specs_for_gpt5_sory_unified_exec_web_search() {
    let model_info = model_info();
...
fn web_search_config_is_forwarded_to_tool_spec() {
    ...
    let web_search_config = WebSearchConfig {
        filters: Some(sory_protocol::config_types::WebSearchFilters {
            allowed_domains: Some(vec!["example.com".to_string()]),
        }),
        user_location: Some(sory_protocol::config_types::WebSearchUserLocation {
            r#type: sory_protocol::config_types::WebSearchUserLocationType::Approximate,
            country: Some("US".to_string()),
            region: Some("California".to_string()),
            city: Some("San Francisco".to_string()),
            timezone: Some("America/Los_Angeles".to_string()),
        }),
        search_context_size: Some(sory_protocol::config_types::WebSearchContextSize::High),
    };
...
fn namespaced_dynamic_specs_are_hidden_when_namespace_tools_are_disabled() {
    ...
    assert_lacks_tool_name(&tools, "sory_app");
...
fn search_tool_registers_for_deferred_dynamic_tools() {
    ...
    assert!(description.contains("- Dynamic tools: Tools provided by the current sory thread."));
    assert_contains_tool_names(&tools, &[TOOL_SEARCH_TOOL_NAME]);
    assert_lacks_tool_name(&tools, "sory_app");
    assert!(registry.has_tool(&ToolName::plain(TOOL_SEARCH_TOOL_NAME)));
    assert!(registry.has_tool(&ToolName::namespaced("sory_app", "automation_update")));
    assert!(registry.has_tool(&ToolName::namespaced("sory_app", "automation_list")));
}
