...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the sory sandbox network proxy.";
...
) -> sory_exec_server::ExecEnvPolicy {
    sory_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<sory_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Sory(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == sory_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let sory_home = crate::config::find_sory_home().map_err(|err| {
                UnifiedExecError::create_process(format!(
            ...
            let spawned = match request.windows_sandbox_level {
                sory_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    sory_windows_sandbox::spawn_windows_sandbox_session_elevated(
                        policy_json.as_str(),
                ...
                sory_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | sory_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    sory_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        policy_json.as_str(),
                        request.windows_sandbox_policy_cwd.as_path(),
                        sory_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            sory_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
                &request.arg0,
                sory_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            sory_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            SORY_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Sory(SoryErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;
