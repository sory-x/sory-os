...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("sory_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            "sory_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_server_params_use_env_policy_overlay_contract() {
    let cwd: sory_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        sory_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = sory_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = sory_protocol::models::PermissionProfile::Disabled;
    let request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: sory_exec_server::ExecEnvPolicy {
                inherit: sory_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: sory_sandboxing::SandboxType::None,
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let sory_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        sory_protocol::protocol::ExecCommandStatus::Failed
    );
