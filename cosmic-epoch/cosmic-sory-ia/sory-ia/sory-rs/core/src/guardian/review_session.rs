...
use anyhow::anyhow;
use sory_analytics::GuardianReviewAnalyticsResult;
use sory_analytics::GuardianReviewSessionKind;
...
use crate::sory_delegate::run_sory_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            sory_linux_sandbox_exe: spawn_config.sory_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.sory.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.sory.session).await {
...
impl GuardianReviewSessionManager {
    pub(crate) async fn trunk_rollout_path(&self) -> Option<PathBuf> {
        ...
        trunk.sory.session.ensure_rollout_materialized().await;
        match trunk.sory.session.current_rollout_path().await {
            Ok(path) => path,
            Err(err) => {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, sory: Sory) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            sory.session.get_config().await.as_ref(),
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, sory: Sory) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            sory.session.get_config().await.as_ref(),
        );
        self.state
    ...
    pub(crate) async fn send_trunk_event_raw_for_test(&self, event: Event) {
        ...
        trunk.sory.session.send_event_raw(event).await;
    }
...
async fn spawn_guardian_review_session(
    ...
    let sory = Box::pin(run_sory_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let mut analytics_result = GuardianReviewAnalyticsResult::from_session(
        review_session.sory.session.conversation_id.to_string(),
        guardian_session_kind,
        params.model.clone(),
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.sory.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.sory.submit(Op::UserTurn {
            environments: None,
    ...
    if matches!(outcome.0, GuardianReviewSessionOutcome::Completed(_)) {
        if outcome.2
            && let Some(total_token_usage) = review_session.sory.session.total_token_usage().await
        {
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.sory.session.new_default_turn().await;
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            ...
            event = review_session.sory.next_event() => {
                match event {
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<sory_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
async fn interrupt_and_drain_turn(sory: &Sory, expected_turn_id: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::soryHooks));
    }
    ...
    async fn interrupt_and_drain_turn_ignores_prior_turn_completion() {
        ...
        interrupt_and_drain_turn(&review_session.sory, "current-turn")
            .await
            .expect("drain current turn");

        assert!(review_session.sory.rx_event.try_recv().is_err());
    }
}
