...
use sory_config::ConfigLayerSource;
use sory_config::ConfigLayerStack;
use sory_config::ConfigLayerStackOrdering;
use sory_config::ConfigRequirements;
use sory_config::ConfigRequirementsToml;
use sory_config::ConstrainedWithSource;
use sory_config::FeatureRequirementsToml;
use sory_config::McpServerIdentity;
use sory_config::McpServerRequirement;
use sory_config::PluginRequirementsToml;
use sory_config::ProfileV2Name;
use sory_config::ResidencyRequirement;
use sory_config::SandboxModeRequirement;
use sory_config::Sourced;
use sory_config::ThreadConfigLoader;
use sory_config::config_toml::ConfigLockfileToml;
use sory_config::config_toml::ConfigToml;
use sory_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use sory_config::config_toml::ProjectConfig;
use sory_config::config_toml::RealtimeAudioConfig;
use sory_config::config_toml::RealtimeConfig;
use sory_config::config_toml::ThreadStoreToml;
use sory_config::config_toml::validate_model_providers;
use sory_config::loader::load_config_layers_state;
use sory_config::loader::project_trust_key;
use sory_config::profile_toml::ConfigProfile;
use sory_config::sandbox_mode_requirement_for_permission_profile;
use sory_config::types::ApprovalsReviewer;
use sory_config::types::AuthCredentialsStoreMode;
use sory_config::types::History;
use sory_config::types::McpServerConfig;
use sory_config::types::McpServerDisabledReason;
use sory_config::types::McpServerTransportConfig;
use sory_config::types::MemoriesConfig;
use sory_config::types::ModelAvailabilityNuxConfig;
use sory_config::types::Notice;
use sory_config::types::OAuthCredentialsStoreMode;
use sory_config::types::SessionPickerViewMode;
use sory_config::types::ToolSuggestConfig;
use sory_config::types::ToolSuggestDisabledTool;
use sory_config::types::ToolSuggestDiscoverable;
use sory_config::types::TuiKeymap;
use sory_config::types::TuiNotificationSettings;
use sory_config::types::TuiPetAnchor;
use sory_config::types::UriBasedFileOpener;
use sory_config::types::WindowsSandboxModeToml;
use sory_core_plugins::PluginsConfigInput;
use sory_exec_server::ExecutorFileSystem;
use sory_exec_server::LOCAL_FS;
use sory_features::AppsMcpPathOverrideConfigToml;
use sory_features::Feature;
use sory_features::FeatureConfigSource;
use sory_features::FeatureOverrides;
use sory_features::FeatureToml;
use sory_features::Features;
use sory_features::FeaturesToml;
use sory_features::MultiAgentV2ConfigToml;
use sory_features::NetworkProxyConfigToml;
use sory_git_utils::resolve_root_git_project_for_trust;
use sory_login::AuthManagerConfig;
use sory_mcp::McpConfig;
use sory_memories_read::memory_root;
use sory_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use sory_model_provider_info::ModelProviderInfo;
use sory_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use sory_model_provider_info::built_in_model_providers;
use sory_model_provider_info::merge_configured_model_providers;
use sory_models_manager::ModelsManagerConfig;
use sory_protocol::config_types::AltScreenMode;
...
use sory_protocol::config_types::ShellEnvironmentPolicy;
use sory_protocol::config_types::TrustLevel;
use sory_protocol::config_types::Verbosity;
use sory_protocol::config_types::WebSearchConfig;
use sory_protocol::config_types::WebSearchMode;
...
use sory_protocol::openai_models::ModelsResponse;
...
use sory_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_sory_runtime;
...
#[cfg(test)]
mod schema;
pub use sory_config::ConfigLoadOptions;
pub use sory_config::Constrained;
pub use sory_config::ConstraintError;
pub use sory_config::ConstraintResult;
pub use sory_config::LoaderOverrides;
pub use sory_network_proxy::NetworkProxyAuditMetadata;
use sory_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use sory_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(sory_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(sory_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal sory-only writable roots are intentionally excluded.
...
// when sory is still responsible for the network policy. `Disabled` means no
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, sory will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - sory
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.sory/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "sory"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send sory '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the sory home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that sory can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by sory unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/sory/blob/main/sory-rs/rmcp-client/src/oauth.rs#L2
    /// file: sory_HOME/.credentials.json
    ///       This file will be readable to sory and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, sory will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all sory state (defaults to `~/.sory` but can be
    /// overridden by the `sory_HOME` environment variable).
    pub sory_home: AbsolutePathBuf,

    /// Directory where sory stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where sory writes log files (defaults to `$sory_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where sory writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores sory version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_sory_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.sory/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current sory executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub sory_self_exe: Option<PathBuf>,

    /// Path to the `sory-linux-sandbox` executable. This must be set if
    /// [`sory_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `sory-linux-sandbox`.
    pub sory_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `sory-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for sory updates on startup and surfaces update prompts.
    /// Set to `false` only if your sory updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across sory product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across sory product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: sory_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn sory_home(&self) -> PathBuf {
        self.sory_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn sory_home(mut self, sory_home: PathBuf) -> Self {
        self.sory_home = Some(sory_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let sory_home = match sory_home {
            Some(sory_home) => AbsolutePathBuf::from_absolute_path(sory_home)?,
            None => find_sory_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&sory_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = sory_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    sory_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(sory_config::io_error_from_config_error(
        ...
        {
            let allow_sory_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_sory_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_sory_version_mismatch = allow_sory_version_mismatch;
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &sory_core_plugins::PluginsManager,
    ) -> McpConfig {
        ...
        McpConfig {
            ...
            sory_home: self.sory_home.to_path_buf(),
            ...
            sory_linux_sandbox_exe: self.sory_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.sory_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_sory_home(
            sory_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific sory home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_sory_home(
        sory_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = sory_config::build_cli_overrides_layer(&cli_overrides);
        sory_config::merge_toml_values(&mut merged, &cli_layer);
        let sory_home = AbsolutePathBuf::from_absolute_path_checked(sory_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &sory_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `sory exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `sory_self_exe`, `sory_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(sory_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &sory_config::NoopThreadConfigLoader,
    )
    .await?;

    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, sory_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .sory/ folders.
...
/// Patch `sory_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(sory_home: &Path, provider: &str) -> std::io::Result<()> {
    sory_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &sory_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            sory_protocol::permissions::FileSystemSandboxEntry {
                path: sory_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: sory_protocol::permissions::FileSystemAccessMode::None,
            }
        } else {
            ...
            sory_protocol::permissions::FileSystemSandboxEntry {
                path: sory_protocol::permissions::FileSystemPath::Path { path },
                access: sory_protocol::permissions::FileSystemAccessMode::None,
            }
        };
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let user_instructions = AgentsMdManager::load_global_instructions(Some(&sory_home))
        ...
        let memories_root = memory_root(&sory_home);
        ...
        let (
            ...
            {
                // Keep sory runtime write access while storing the runtime
        ...
        let model_provider_id = model_provider
            ...
            .unwrap_or_else(|| "soryos-zen".to_string());
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(sory_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| sory_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| sory_home.to_path_buf());
        ...
        let helper_readable_roots = get_readable_roots_required_for_sory_runtime(
            &sory_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_sory_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_sory_version_mismatch)
...
/// Returns the path to the sory configuration directory, which can be
/// specified by the `sory_HOME` environment variable. If not set, defaults to
/// `~/.sory`.
///
/// - If `sory_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `sory_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_sory_home() -> std::io::Result<AbsolutePathBuf> {
    sory_utils_home_dir::find_sory_home()
}

/// Returns the path to the folder where sory logs are stored. Does not verify
