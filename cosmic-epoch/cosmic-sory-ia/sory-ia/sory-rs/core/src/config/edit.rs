...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for sory-managed fast defaults.
...
/// `$sory_HOME/config.toml`; profile-v2 callers should resolve their target
...
impl ConfigEditsBuilder {
    pub fn new(sory_home: &Path) -> Self {
        Self::for_config_path(&sory_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(sory_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.sory_home.join(CONFIG_TOML_FILE).to_path_buf());
