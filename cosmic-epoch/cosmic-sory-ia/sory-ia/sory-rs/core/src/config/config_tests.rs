...
use sory_config::ConfigLayerEntry;
use sory_config::ProfileV2Name;
use sory_config::RequirementSource;
use sory_config::config_toml::AgentRoleToml;
use sory_config::config_toml::AgentsToml;
use sory_config::config_toml::AutoReviewToml;
...
use sory_config::config_toml::RealtimeToml;
use sory_config::config_toml::RealtimeTransport;
use sory_config::config_toml::RealtimeWsMode;
use sory_config::config_toml::RealtimeWsVersion;
use sory_config::config_toml::ToolsToml;
use sory_config::loader::project_trust_key;
use sory_config::permissions_toml::FilesystemPermissionToml;
use sory_config::permissions_toml::FilesystemPermissionsToml;
use sory_config::permissions_toml::NetworkDomainPermissionToml;
use sory_config::permissions_toml::NetworkDomainPermissionsToml;
use sory_config::permissions_toml::NetworkToml;
use sory_config::permissions_toml::PermissionProfileToml;
use sory_config::permissions_toml::PermissionsToml;
use sory_config::permissions_toml::WorkspaceRootsToml;
use sory_config::profile_toml::ConfigProfile;
use sory_config::types::AppToolApproval;
use sory_config::types::ApprovalsReviewer;
use sory_config::types::BundledSkillsConfig;
use sory_config::types::FeedbackConfigToml;
use sory_config::types::HistoryPersistence;
use sory_config::types::McpServerEnvVar;
use sory_config::types::McpServerOAuthConfig;
use sory_config::types::McpServerToolConfig;
use sory_config::types::McpServerTransportConfig;
use sory_config::types::MemoriesConfig;
use sory_config::types::MemoriesToml;
use sory_config::types::ModelAvailabilityNuxConfig;
use sory_config::types::Notice;
use sory_config::types::NotificationCondition;
use sory_config::types::NotificationMethod;
use sory_config::types::Notifications;
use sory_config::types::OtelConfig;
use sory_config::types::OtelConfigToml;
use sory_config::types::OtelExporterKind;
use sory_config::types::SandboxWorkspaceWrite;
use sory_config::types::SessionPickerViewMode;
use sory_config::types::SkillsConfig;
use sory_config::types::ToolSuggestDisabledTool;
use sory_config::types::ToolSuggestDiscoverableType;
use sory_config::types::Tui;
...
use sory_config::types::WindowsToml;
...
use sory_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use sory_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use sory_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use sory_protocol::models::ManagedFileSystemPermissions;
...
use sory_protocol::protocol::NetworkAccess;
...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let sory_home = tempdir()?;
    std::fs::write(
        sory_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = sory_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        sory_home.path().join(CONFIG_TOML_FILE),
        r#"
...
async fn experimental_network_requirements_enable_proxy_without_feature() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(sory_config::ConfigRequirementsToml {
                network: Some(sory_config::NetworkRequirementsToml {
                    enabled: Some(true),
                    ..Default::default()
                }),
                ..Default::default()
...
async fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = sory_home.path().join("memories").abs();
    assert_eq!(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        sory_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let sory_home = temp_dir.path().join("sory-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".sory"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".sory".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".sory"), cwd.as_path()),
            "expected .sory carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".sory"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".sory"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of sory and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of sory and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of sory."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            sory_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sory_home.path());
    let project_dot_sory =
        AbsolutePathBuf::resolve_path_against_base("project/.sory", sory_home.path());
    ...
    let requirements_toml = sory_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = sory_config::ConfigRequirements {
    ...
    let refreshed_layer_stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                sory_app_server_protocol::ConfigLayerSource::User {
            ...
            ConfigLayerEntry::new(
                sory_app_server_protocol::ConfigLayerSource::Project {
                    dot_sory_folder: project_dot_sory.clone(),
                },
                toml::toml! {
            ...
            ConfigLayerEntry::new(
                sory_app_server_protocol::ConfigLayerSource::LegacyManagedConfigTomlFromMdm,
                toml::toml! {
                    [mcp_servers.managed_overrides_session]
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(sory_home.path().to_path_buf()),
            ..Default::default()
        },
        sory_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                sory_app_server_protocol::ConfigLayerSource::Project {
                    dot_sory_folder: project_dot_sory,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
            ...
            ConfigLayerEntry::new(
                sory_app_server_protocol::ConfigLayerSource::SessionFlags,
                toml::toml! {
                    [mcp_servers.session_overrides_user]
...
-> anyhow::Result<()> {
    let sory_home = TempDir::new()?;
    let plugin_root = sory_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".sory-plugin"))?;
    std::fs::write(
        plugin_root.join(".sory-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
  ...
  "mcpServers": {
    ...
    let config = thread_config
        .rebuild_preserving_session_layers(&refreshed_config)
        .await?;
    let plugins_manager = PluginsManager::new(sory_home.path().to_path_buf());
...
"#,
    ...
    let requirements = sory_config::ConfigRequirementsToml {
...
#[tokio::test]
async fn sqlite_home_defaults_to_sory_home_for_workspace_write() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, sory_home.path().to_path_buf());
...
async fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let memories_root = sory_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".sory");
...
async fn cli_override_takes_precedence_over_profile_sandbox_mode() -> std::io::Result<()> {
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, sory_home.abs()).await?;
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let sory_home = TempDir::new()?;
    let managed_path = sory_home.path().join("managed_config.toml");
    let config_path = sory_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), sory_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(sory_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(sory_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(sory_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/sory-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/sory-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/sory-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-sory-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-sory-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-sory-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(sory_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(sory_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let sory_home = TempDir::new()?;
    let base_config = sory_home.path().join(CONFIG_TOML_FILE);
    let selected_config = sory_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(sory_home.path(), &profile_name),
        sory_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn sory_home(&self) -> AbsolutePathBuf {
        self.sory_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let workspace = sory_home.path().join("workspace");
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        sory_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let missing_path = sory_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let role_config_path = sory_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = sory_config::ConfigLayerStack::new(
        vec![sory_config::ConfigLayerEntry::new(
            sory_app_server_protocol::ConfigLayerSource::User {
                file: sory_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        sory_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".sory").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let researcher_path = sory_home.path().join("agents").join("researcher.toml");
    let reviewer_path = sory_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = sory_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let catalog_path = sory_home.path().join("catalog.json");
...
"#;
    ...
    let sory_home_temp_dir = TempDir::new().unwrap();
...
async fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            sory_home: fixture.sory_home(),
            sqlite_home: fixture.sory_home().to_path_buf(),
            log_dir: fixture.sory_home().join("log").to_path_buf(),
            config_lock_export_dir: None,
            config_lock_allow_sory_version_mismatch: false,
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: sory_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let sory_home = fixture.sory_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = sory_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![sory_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = sory_config::RequirementSource::Unknown;
    ...
    let requirements = sory_config::ConfigRequirements {
        web_search_mode: sory_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        sory_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/sory4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/sory3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/sory2");
...
[projects."/Users/mbolin/code/sory4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/sory3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let sory_home = temp_dir.path();
    let config_path = sory_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(sory_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(sory_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(sory_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(sory_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
-> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(sory_config::ConfigRequirementsToml {
                allowed_sandbox_modes: Some(vec![sory_config::SandboxModeRequirement::ReadOnly]),
                ..Default::default()
            }))
        }))
        .build()
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let cwd = sory_home.path().join("workspace");
    let outside_root = sory_home.path().join("outside-write");
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(sory_config::ConfigRequirementsToml {
                allowed_web_search_modes: Some(vec![
                    sory_config::WebSearchModeRequirement::Cached,
                ]),
                ..Default::default()
            }))
        }))
...
async fn feature_requirements_normalize_effective_feature_values() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(sory_config::ConfigRequirementsToml {
                feature_requirements: Some(sory_config::FeatureRequirementsToml {
                    entries: BTreeMap::from([
...
export_dir = "locks"
allow_sory_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_sory_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let sory_home = TempDir::new()?;
    let lock_path = sory_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
sory_version = "older-version"
