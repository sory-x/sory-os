...
use sory_config::permissions_toml::NetworkUnixSocketPermissionToml;
use sory_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use sory_features::NetworkProxyDomainPermissionToml;
use sory_features::NetworkProxyModeToml;
use sory_features::NetworkProxyUnixSocketPermissionToml;
use sory_network_proxy::NetworkMode;
use sory_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use sory_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use sory_protocol::permissions::project_roots_glob_pattern;
...
/// for sory to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_sory_runtime(
    sory_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(sory_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older sory versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of sory. Filesystem access will remain restricted. Upgrade sory if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of sory and will be ignored. Upgrade sory if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of sory and will be ignored. Upgrade sory if this path is required."
            ),
        },
    );
}
