...
use sory_config::ConfigError;
use sory_config::ConfigLayerEntry;
use sory_config::ConfigLayerStackOrdering;
use sory_config::ConfigLoadError;
use sory_config::ConfigRequirements;
use sory_config::ConfigRequirementsToml;
use sory_config::ConfigRequirementsWithSources;
use sory_config::FilesystemDenyReadPattern;
use sory_config::LoaderOverrides;
use sory_config::RequirementSource;
use sory_config::SessionThreadConfig;
use sory_config::StaticThreadConfigLoader;
use sory_config::ThreadConfigSource;
use sory_config::config_error_from_ignored_toml_fields;
use sory_config::config_error_from_toml;
...
use sory_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_sory_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_sory_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_sory_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let sory_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when sory_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .sory_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = sory_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        sory_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = sory_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = sory_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when sory_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![sory_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(sory_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(sory_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sory_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sory_home).await?;
...
async fn load_config_layers_includes_cloud_hook_requirements() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirementsToml {
        hooks: Some(sory_config::ManagedHooksRequirementsToml {
            managed_dir: Some(managed_dir.clone()),
            windows_managed_dir: None,
            hooks: sory_config::HookEventsToml {
                pre_tool_use: vec![sory_config::MatcherGroup {
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            sory_config::SandboxModeRequirement::ReadOnly,
            sory_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                sory_config::CloudRequirementsLoadErrorCode::RequestFailed,
                /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".sory")).await?;
    tokio::fs::create_dir_all(project_root.join(".sory")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".sory").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".sory").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_sory_folder } => Some(dot_sory_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".sory").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".sory").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".sory")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".sory")).await?;
    ...
    .await?;
    write_project_hook_config(
        &repo_child.join(".sory"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".sory"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".sory"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".sory"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_sory_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".sory").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".sory").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".sory").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".sory").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let sory_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sory_home).await?;
    tokio::fs::write(sory_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_sory_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_sory_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".sory"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn sory_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let sory_home = home_dir.join(".sory");
    tokio::fs::create_dir_all(&sory_home).await?;
    tokio::fs::write(
        sory_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn sory_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_sory = project_root.join(".sory");
    let nested_dot_sory = nested.join(".sory");

    tokio::fs::create_dir_all(&nested_dot_sory).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_sory.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_sory).await?;
    make_config_for_test(
        &project_dot_sory,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_sory.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_sory_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_sory)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let sory_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&sory_home_untrusted).await?;
    make_config_for_test(
        &sory_home_untrusted,
        &project_root,
    ...
    .await?;
    let untrusted_config_path = sory_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let sory_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&sory_home_unknown).await?;
    tokio::fs::write(
        sory_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_sory).await?;
    ...
    // `.sory` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_sory.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_sory.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.sory` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".sory")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".sory").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let sory_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&sory_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let sory_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&sory_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &sory_home,
...
mod requirements_exec_policy_tests {
    ...
    use sory_config::ConfigRequirementsWithSources;
    use sory_config::RequirementSource;
    use sory_config::RequirementsExecPolicyDecisionToml;
    use sory_config::RequirementsExecPolicyParseError;
    use sory_config::RequirementsExecPolicyPatternTokenToml;
    use sory_config::RequirementsExecPolicyPrefixRuleToml;
    use sory_config::RequirementsExecPolicyToml;
    use sory_execpolicy::Decision;
    use sory_execpolicy::Evaluation;
    use sory_execpolicy::RuleMatch;
    use sory_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_sory_folder_with_requirements(
        dot_sory_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_sory_folder = AbsolutePathBuf::from_absolute_path(dot_sory_folder)
            .expect("absolute dot_sory_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_sory_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_sory_folder_with_requirements(temp_dir.path(), requirements);
