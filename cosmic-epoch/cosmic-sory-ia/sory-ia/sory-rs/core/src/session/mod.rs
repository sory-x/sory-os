...
use sory_analytics::SubAgentThreadStartedInput;
use sory_app_server_protocol::McpServerElicitationRequest;
use sory_app_server_protocol::McpServerElicitationRequestParams;
use sory_config::types::OAuthCredentialsStoreMode;
use sory_exec_server::Environment;
use sory_exec_server::EnvironmentManager;
use sory_exec_server::FileSystemSandboxContext;
use sory_extension_api::PromptSlot;
use sory_features::FEATURES;
use sory_features::Feature;
use sory_features::unstable_features_warning_event;
use sory_hooks::Hooks;
use sory_hooks::HooksConfig;
use sory_login::AuthManager;
use sory_login::soryAuth;
use sory_login::auth_env_telemetry::collect_auth_env_telemetry;
use sory_login::default_client::originator;
use sory_mcp::McpConnectionManager;
use sory_mcp::McpRuntimeEnvironment;
use sory_mcp::sory_apps_tools_cache_key;
use sory_models_manager::manager::RefreshStrategy;
use sory_models_manager::manager::SharedModelsManager;
use sory_network_proxy::NetworkProxy;
use sory_network_proxy::NetworkProxyAuditMetadata;
use sory_network_proxy::normalize_host;
use sory_otel::current_span_trace_id;
use sory_otel::current_span_w3c_trace_context;
use sory_otel::set_parent_from_w3c_trace_context;
use sory_protocol::ThreadId;
use sory_protocol::account::PlanType as AccountPlanType;
use sory_protocol::approvals::ElicitationRequestEvent;
use sory_protocol::approvals::ExecPolicyAmendment;
use sory_protocol::approvals::NetworkPolicyAmendment;
use sory_protocol::approvals::NetworkPolicyRuleAction;
use sory_protocol::config_types::ApprovalsReviewer;
...
use sory_protocol::dynamic_tools::DynamicToolResponse;
use sory_protocol::dynamic_tools::DynamicToolSpec;
use sory_protocol::items::TurnItem;
use sory_protocol::items::UserMessageItem;
use sory_protocol::mcp::CallToolResult;
use sory_protocol::models::ActivePermissionProfile;
use sory_protocol::models::AdditionalPermissionProfile;
...
use sory_protocol::models::format_allow_prefixes;
...
use sory_protocol::protocol::FileChange;
use sory_protocol::protocol::HasLegacyEvent;
use sory_protocol::protocol::InterAgentCommunication;
use sory_protocol::protocol::ItemCompletedEvent;
use sory_protocol::protocol::ItemStartedEvent;
use sory_protocol::protocol::RawResponseItemEvent;
use sory_protocol::protocol::ReviewRequest;
use sory_protocol::protocol::RolloutItem;
use sory_protocol::protocol::SessionSource;
use sory_protocol::protocol::SubAgentSource;
use sory_protocol::protocol::ThreadSource;
use sory_protocol::protocol::TurnAbortReason;
use sory_protocol::protocol::TurnContextItem;
use sory_protocol::protocol::TurnContextNetworkItem;
...
use sory_protocol::request_permissions::RequestPermissionsArgs;
use sory_protocol::request_permissions::RequestPermissionsEvent;
use sory_protocol::request_permissions::RequestPermissionsResponse;
use sory_protocol::request_user_input::RequestUserInputArgs;
use sory_protocol::request_user_input::RequestUserInputResponse;
use sory_rmcp_client::ElicitationResponse;
use sory_rollout::state_db;
use sory_rollout_trace::AgentResultTracePayload;
use sory_rollout_trace::ThreadStartedTraceMetadata;
use sory_rollout_trace::ThreadTraceContext;
use sory_sandboxing::policy_transforms::intersect_permission_profiles;
use sory_shell_command::parse_command::parse_command;
use sory_terminal_detection::user_agent;
use sory_thread_store::CreateThreadParams;
use sory_thread_store::LiveThread;
use sory_thread_store::LiveThreadInitGuard;
use sory_thread_store::LocalThreadStore;
use sory_thread_store::ReadThreadParams;
use sory_thread_store::ResumeThreadParams;
use sory_thread_store::ThreadEventPersistenceMode;
use sory_thread_store::ThreadPersistenceMetadata;
use sory_thread_store::ThreadStore;
use sory_utils_output_truncation::TruncationPolicy;
...
use crate::sory_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use sory_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                sory_error_info: Some(SoryErrorInfo::BadRequest),
            },
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                ...
                ErrorEvent {
                    ...
                    sory_error_info: Some(SoryErrorInfo::ActiveTurnNotSteerable {
                        turn_kind: *turn_kind,
                    }),
                }
            }
...
use sory_git_utils::get_git_repo_root;
use sory_mcp::compute_auth_statuses;
use sory_mcp::effective_mcp_servers_from_configured;
use sory_mcp::host_owned_sory_apps_enabled;
use sory_otel::SessionTelemetry;
use sory_otel::THREAD_STARTED_METRIC;
...
use sory_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
...
use sory_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use sory_protocol::protocol::ApplyPatchApprovalRequestEvent;
use sory_protocol::protocol::AskForApproval;
use sory_protocol::protocol::SoryErrorInfo;
use sory_protocol::protocol::CompactedItem;
use sory_protocol::protocol::DeprecationNoticeEvent;
use sory_protocol::protocol::ErrorEvent;
...
use sory_protocol::protocol::McpServerRefreshConfig;
use sory_protocol::protocol::ModelRerouteEvent;
use sory_protocol::protocol::ModelRerouteReason;
use sory_protocol::protocol::ModelVerification;
use sory_protocol::protocol::ModelVerificationEvent;
use sory_protocol::protocol::NetworkApprovalContext;
use sory_protocol::protocol::NonSteerableTurnKind;
use sory_protocol::protocol::Op;
use sory_protocol::protocol::RateLimitSnapshot;
use sory_protocol::protocol::RequestUserInputEvent;
...
use sory_protocol::protocol::SessionNetworkProxyRuntime;
use sory_protocol::protocol::StreamErrorEvent;
...
use sory_protocol::protocol::WarningEvent;
use sory_protocol::user_input::UserInput;
use sory_tools::ToolEnvironmentMode;
use sory_tools::ToolsConfig;
use sory_tools::ToolsConfigParams;
use sory_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use sory_utils_stream_parser::ProposedPlanSegment;

/// The high-level interface to the sory system.
...
/// Wrapper returned by [`sory::spawn`] containing the spawned [`sory`] and
/// the unique session id.
pub struct SorySpawnOk {
    pub sory: Sory,
    pub thread_id: ThreadId,
}

pub(crate) struct SorySpawnArgs {
    ...
    pub(crate) extensions: Arc<sory_extension_api::ExtensionRegistry<crate::config::Config>>,
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/sory/concepts/cyber-safety";

impl Sory {
    /// Spawn a new [`sory`] and initialize the session.
    pub(crate) async fn spawn(args: SorySpawnArgs) -> soryResult<SorySpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if sory_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(SorySpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: SorySpawnArgs) -> soryResult<SorySpawnOk> {
        let SorySpawnArgs {
            mut config,
            installation_id,
            auth_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| SoryErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            sory_models_manager::manager::RefreshStrategy::Offline
        } else {
            sory_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "sory_spawn")
                        .await
                }
                None => None,
            }
        ...
        // Generate a unique ID for the lifetime of this sory session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.sory_home)
        })?;
        ...
        Ok(SorySpawnOk { sory, thread_id })
    }

    /// Submit the `op` wrapped in a `Submission` with a unique ID.
    pub async fn submit(&self, op: Op) -> soryResult<String> {
    ...
    /// Use sparingly: prefer `submit()` so sory is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> soryResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| SoryErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: sory_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
        handlers::persist_thread_memory_mode_update(&self.session, mode).await
    }

    pub async fn shutdown_and_wait(&self) -> soryResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(SoryErr::InternalAgentDied) => {}
    ...
    pub async fn next_event(&self) -> soryResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> soryResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-sory-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &sory_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn sory_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn sory_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn sory_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.sory_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect sory performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let (
            ...
            let sory_home = updated.sory_home.clone();
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_sory_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &sory_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| sory_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&sory_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_cwd(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                sory_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-sory, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = sory_error.to_string();
        let sory_error_info = SoryErrorInfo::ResponseStreamDisconnected {
            http_status_code: sory_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            sory_error_info: Some(sory_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<sory_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::soryHooks),
