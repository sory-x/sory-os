...
use sory_config::ConfigLayerStackOrdering;
use sory_config::LoaderOverrides;
use sory_config::NetworkConstraints;
use sory_config::NetworkDomainPermissionToml;
use sory_config::NetworkDomainPermissionsToml;
use sory_config::RequirementSource;
use sory_config::Sourced;
use sory_config::loader::project_trust_key;
use sory_config::types::ToolSuggestDisabledTool;

use sory_features::Feature;
use sory_features::Features;
use sory_login::soryAuth;
use sory_model_provider_info::ModelProviderInfo;
use sory_models_manager::bundled_models_response;
use sory_models_manager::model_info;
use sory_models_manager::test_support::construct_model_info_offline_for_tests;
use sory_models_manager::test_support::get_model_offline_for_tests;
use sory_protocol::AgentPath;
use sory_protocol::SessionId;
use sory_protocol::ThreadId;
use sory_protocol::account::PlanType as AccountPlanType;
use sory_protocol::config_types::ServiceTier;
use sory_protocol::config_types::TrustLevel;
use sory_protocol::exec_output::ExecToolCallOutput;
use sory_protocol::models::ActivePermissionProfile;
use sory_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use sory_protocol::models::FileSystemPermissions;
use sory_protocol::models::FunctionCallOutputBody;
use sory_protocol::models::FunctionCallOutputPayload;
use sory_protocol::models::PermissionProfile;
use sory_protocol::models::SandboxEnforcement;
...
use sory_protocol::protocol::NonSteerableTurnKind;
use sory_protocol::protocol::SandboxPolicy;
use sory_protocol::protocol::TurnEnvironmentSelection;
use sory_protocol::request_permissions::PermissionGrantScope;
use sory_protocol::request_permissions::RequestPermissionProfile;
use tracing::Span;
...
use sory_app_server_protocol::McpElicitationSchema;
use sory_config::config_toml::ConfigToml;
use sory_config::config_toml::ProjectConfig;
use sory_execpolicy::Decision;
use sory_execpolicy::NetworkRuleProtocol;
use sory_execpolicy::Policy;
use sory_network_proxy::NetworkProxyConfig;
use sory_otel::MetricsClient;
use sory_otel::MetricsConfig;
use sory_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use sory_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use sory_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use sory_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use sory_otel::TelemetryAuthMode;
...
use sory_protocol::models::BaseInstructions;
...
use sory_protocol::protocol::CompactedItem;
use sory_protocol::protocol::ConversationAudioParams;
use sory_protocol::protocol::CreditsSnapshot;
use sory_protocol::protocol::GranularApprovalConfig;
use sory_protocol::protocol::InitialHistory;
use sory_protocol::protocol::InterAgentCommunication;
use sory_protocol::protocol::NetworkApprovalProtocol;
use sory_protocol::protocol::RateLimitSnapshot;
use sory_protocol::protocol::RateLimitWindow;
use sory_protocol::protocol::RealtimeAudioFrame;
use sory_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use sory_protocol::protocol::RealtimeVoice;
...
use sory_protocol::protocol::SkillScope;
use sory_protocol::protocol::Submission;
use sory_protocol::protocol::ThreadGoalStatus;
use sory_protocol::protocol::ThreadRolledBackEvent;
use sory_protocol::protocol::TokenCountEvent;
use sory_protocol::protocol::TokenUsage;
use sory_protocol::protocol::TokenUsageInfo;
use sory_protocol::protocol::TurnAbortedEvent;
use sory_protocol::protocol::TurnCompleteEvent;
use sory_protocol::protocol::TurnStartedEvent;
use sory_protocol::protocol::UserMessageEvent;
use sory_protocol::protocol::W3cTraceContext;
use sory_protocol::request_user_input::RequestUserInputAnswer;
use sory_protocol::request_user_input::RequestUserInputResponse;
use sory_rmcp_client::ElicitationAction;
...
use sory_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "sory-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
    ...
    let response = session
        .request_mcp_server_elicitation(
            ...
            McpServerElicitationRequestParams {
                ...
                server_name: "sory_apps".to_string(),
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        sory_protocol::protocol::SessionSource::Exec,
...
fn write_project_hooks(dot_sory: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_sory)?;
    std::fs::write(
        dot_sory.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        sory_home.join(sory_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<sory_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&sory_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            source: sory_hooks::SessionStartSource::Startup,
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn sory_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { sory_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> sory_sandboxing::SandboxablePreference {
            sory_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(sory_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: sory_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let sory_home = session.sory_home().await;
    std::fs::create_dir_all(&sory_home).expect("create sory home");
    let config_toml_path = sory_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = sory_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let sory_home = session.sory_home().await;
    std::fs::create_dir_all(&sory_home).expect("create sory home");
    let base_config_path = sory_home.join(CONFIG_TOML_FILE);
    let profile_config_path = sory_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(sory_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            .features
            .enable(Feature::soryHooks)
            .expect("enable sory hooks");
    })
    .await?;
    let sory_home = session.sory_home().await;
    std::fs::create_dir_all(&sory_home)?;
    let config_toml_path = sory_home.join(CONFIG_TOML_FILE);
    let user_config: sory_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = sory_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = sory_hooks::list_hooks(sory_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        sory_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: sory_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: sory_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: sory_config::MatcherGroup {
                matcher: None,
                hooks: vec![sory_config::HookHandlerConfig::Command {
        ...
        let identity = sory_config::TomlValue::try_from(identity)?;
        sory_config::version_for_toml(&identity)
    };
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl sory_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage(
            &self,
            session_store: &sory_extension_api::ExtensionData,
            thread_store: &sory_extension_api::ExtensionData,
            turn_store: &sory_extension_api::ExtensionData,
            token_usage: &TokenUsageInfo,
        ) {
            self.records
                .lock()
    ...
    let mut builder = sory_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl sory_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let sory_home = tempfile::TempDir::new()?;
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_sory().with_config(|config| {
        config.permissions.approval_policy =
            sory_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.sory.ensure_rollout_materialized().await;
    initial
        .sory
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        sory_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "sory_core__sory_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            sory_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            sory_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            sory_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.sory_error_info,
        Some(SoryErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    let sory_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(sory_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(sory_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("sory_other".to_string()),
        limit_name: Some("sory_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(sory_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.sory_error_info == Some(SoryErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(sory_home: &Path) -> Config {
...
fn turn_environments_for_tests(
    environment: &Arc<sory_exec_server::Environment>,
    cwd: &sory_utils_absolute_path::AbsolutePathBuf,
) -> crate::environment_selection::ResolvedTurnEnvironments {
    crate::environment_selection::ResolvedTurnEnvironments {
        turn_environments: vec![TurnEnvironment {
            environment_id: sory_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = sory_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        sory_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let sory_home = parent_config.sory_home.clone();
    let skill_dir = sory_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&sory_exec_server::LOCAL_FS));
    ...
    let role_path = sory_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let sory_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(sory_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(soryAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.sory_home.to_path_buf(),
        auth_manager.clone(),
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.sory_home.to_path_buf()));
    let mcp_manager = Arc::new(McpManager::new(Arc::clone(&plugins_manager)));
    let skills_manager = Arc::new(SkillsManager::new(
        config.sory_home.clone(),
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(sory_extension_api::ExtensionRegistryBuilder::new().build()),
        AgentControl::default(),
        Arc::new(sory_exec_server::EnvironmentManager::default_for_tests()),
        /*analytics_events_client*/ None,
        Arc::new(sory_thread_store::LocalThreadStore::new(
            sory_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        sory_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        sory_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: sory_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(sory_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: sory_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: sory_extension_api::ExtensionData::new(thread_id.to_string()),
        agent_control,
        network_proxy: None,
        network_approval: Arc::clone(&network_approval),
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(sory_thread_store::LocalThreadStore::new(
            sory_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
        ...
        environment_manager: Arc::new(sory_exec_server::EnvironmentManager::default_for_tests()),
    };
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            sory_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(sory_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = sory_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(sory_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    &turn_context,
                    call_id,
                    sory_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("sory-core-tests");
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserTurn {
            ...
            approvals_reviewer: Some(sory_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        sory_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, SoryErr::InvalidRequest(_)));
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        sory_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = sory_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(sory_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn sory_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        sory_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_emits_thread_stop_lifecycle() {
    ...
    impl sory_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop(&self, input: sory_extension_api::ThreadStopInput<'_>) {
            assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl sory_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop(&self, input: sory_extension_api::ThreadStopInput<'_>) {
            assert_eq!(
                self.expected_thread_id.to_string(),
                input.thread_store.level_id()
    ...
    impl sory_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort(&self, input: sory_extension_api::TurnAbortInput<'_>) {
            assert_eq!(
                self.expected_thread_id.to_string(),
                input.thread_store.level_id()
            );
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let sory = Arc::clone(&sory);
        tokio::spawn(async move { sory.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let sory = Arc::clone(&sory);
        tokio::spawn(async move { sory.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_sory)
        .await;
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(sory_home).await;
...
pub(crate) async fn make_session_and_context_with_dynamic_tools_and_rx(
    ...
    make_session_and_context_with_auth_and_config_and_rx(
        soryAuth::from_api_key("Test API Key"),
        dynamic_tools,
        |_config| {},
    )
    .await
...
async fn make_goal_session_and_context_with_rx() -> (
    ...
    (session, turn_context, rx, sory_home)
}
...
async fn upsert_goal_test_thread(session: &Session) {
    ...
    let mut builder = sory_state::ThreadMetadataBuilder::new(
...
impl sory_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute<'a>(
        &'a self,
        _session_store: &'a sory_extension_api::ExtensionData,
        thread_store: &'a sory_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<sory_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    sory_extension_api::PromptFragment::developer_policy(
...
-> Arc<sory_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = sory_extension_api::ExtensionRegistryBuilder::new();
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(sory_extension_api::ExtensionData::new(
...
async fn interrupt_accounts_active_goal_before_pausing() -> anyhow::Result<()> {
    let (sess, tc, _rx, _sory_home) = make_goal_session_and_context_with_rx().await;
    ...
    assert_eq!(
        sory_protocol::protocol::ThreadGoalStatus::Paused,
        goal.status
    );
...
async fn active_goal_continuation_runs_again_after_no_tool_turn() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(8), async {
        loop {
            let event = test.sory.next_event().await?;
...
async fn pending_request_user_input_does_not_spawn_extra_goal_continuation() -> anyhow::Result<()> {
    ...
    let request_user_input_event = wait_for_event_match(&test.sory, |event| match event {
    ...
    assert!(
        timeout(Duration::from_millis(200), test.sory.next_event())
            .await
            .is_err(),
...
async fn goal_test_state_db(sess: &Session) -> anyhow::Result<crate::StateDbHandle> {
    ...
    sory_state::StateRuntime::init(config.sqlite_home.clone(), config.model_provider_id.clone())
        .await
}

#[tokio::test]
async fn budget_limited_accounting_steers_active_turn_without_aborting() -> anyhow::Result<()> {
    let (sess, tc, rx, _sory_home) = make_goal_session_and_context_with_rx().await;
    sess.set_thread_goal(
        tc.as_ref(),
        SetGoalRequest {
            objective: Some("Keep improving the benchmark".to_string()),
    ...
    assert_eq!(sory_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn external_goal_mutation_accounts_active_turn_before_status_change() -> anyhow::Result<()> {
    ...
    let updated_goal = state_db
        .update_thread_goal(
            ...
            sory_state::ThreadGoalUpdate {
                objective: None,
                status: Some(sory_state::ThreadGoalStatus::Complete),
                token_budget: None,
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(sory_state::ThreadGoalStatus::Complete, goal.status);
    assert_eq!(70, goal.tokens_used);
...
async fn external_objective_change_steers_active_turn() -> anyhow::Result<()> {
    ...
    let old_goal = state_db
        .replace_thread_goal(
            ...
            sory_state::ThreadGoalStatus::Active,
...
async fn external_active_goal_set_marks_current_turn_for_accounting() -> anyhow::Result<()> {
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(sory_state::ThreadGoalStatus::Active, goal.status);
    assert_eq!(25, goal.tokens_used);
...
async fn completed_goal_accounts_current_turn_tokens_before_tool_response() -> anyhow::Result<()> {
    ...
    let state_db = sory_state::StateRuntime::init(
    ...
    assert_eq!(
        sory_state::ThreadGoalStatus::Complete,
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(sory_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn create_goal_tool_rejects_existing_goal() {
    let (session, turn_context, _rx, _sory_home) = make_goal_session_and_context_with_rx().await;
    let tracker = Arc::new(tokio::sync::Mutex::new(TurnDiffTracker::new()));
    ...
    handler
        .handle(ToolInvocation {
            ...
            tool_name: sory_tools::ToolName::plain("create_goal"),
...
async fn update_goal_tool_rejects_pausing_goal() {
    ...
    let response = update_handler
        .handle(ToolInvocation {
            ...
            tool_name: sory_tools::ToolName::plain("update_goal"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "status": "paused",
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use sory_protocol::protocol::AskForApproval;
    use sory_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: sory_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: sory_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let sory_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_sory = project_root.join(".sory");
    let nested_dot_sory = nested.join(".sory");

    std::fs::create_dir_all(&sory_home)?;
    std::fs::create_dir_all(&nested_dot_sory)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_sory)?;
    write_project_hooks(&nested_dot_sory)?;
    write_project_trust_config(&sory_home, &[(&nested, TrustLevel::Trusted)]).await?;
    ...
    let expected_source_path = sory_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_sory.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_sory = project_root.join(".sory");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_sory)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let sory_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&sory_home)?;
        write_project_trust_config(&sory_home, &trust_entries).await?;
