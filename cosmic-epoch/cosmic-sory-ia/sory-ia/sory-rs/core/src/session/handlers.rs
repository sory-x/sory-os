...
use sory_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn resolve_elicitation(
    ...
    decision: sory_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        sory_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        sory_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        sory_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. sory will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub async fn set_thread_memory_mode(sess: &Arc<Session>, sub_id: String, mode: ThreadMemoryMode) {
    if let Err(err) = persist_thread_memory_mode_update(sess, mode).await {
        ...
        let event = Event {
            ...
            msg: EventMsg::Error(ErrorEvent {
                message: err.to_string(),
                sory_error_info: Some(SoryErrorInfo::Other),
            }),
        };
...
fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor.on_thread_stop(sory_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down sory instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(sory_rollout_trace::RolloutStatus::Completed);
    true
}
