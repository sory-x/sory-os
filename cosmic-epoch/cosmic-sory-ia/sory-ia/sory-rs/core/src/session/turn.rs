...
use sory_analytics::CompactionPhase;
use sory_analytics::CompactionReason;
use sory_analytics::InvocationType;
use sory_analytics::TurnResolvedConfigFact;
use sory_analytics::build_track_events_context;
use sory_async_utils::OrCancelExt;
use sory_features::Feature;
use sory_git_utils::get_git_repo_root;
use sory_hooks::HookEvent;
use sory_hooks::HookEventAfterAgent;
use sory_hooks::HookPayload;
use sory_hooks::HookResult;
...
use sory_protocol::items::PlanItem;
...
use sory_protocol::protocol::AgentMessageContentDeltaEvent;
use sory_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use sory_protocol::protocol::PlanDeltaEvent;
use sory_protocol::protocol::ReasoningContentDeltaEvent;
use sory_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use sory_tools::filter_request_plugin_install_discoverable_tools_for_client;
use sory_utils_stream_parser::AssistantTextChunk;
use sory_utils_stream_parser::AssistantTextStreamParser;
use sory_utils_stream_parser::ProposedPlanSegment;
use sory_utils_stream_parser::extract_proposed_plan_text;
use sory_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<sory_extension_api::ExtensionData>,
    input: Vec<UserInput>,
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = sory_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
    ...
    // Although from the perspective of sory.rs, TurnDiffTracker has the lifecycle of a Task which contains
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = sory_hooks::StopRequest {
                    ...
                    for run in hooks.preview_stop(&stop_request) {
                        sess.send_event(
                            &turn_context,
                            EventMsg::HookStarted(sory_protocol::protocol::HookStartedEvent {
            ...
            Err(SoryErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(SoryErr::InvalidImageRequest()) => {
                {
...
) -> soryResult<PreSamplingCompactResult> {
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = sory_connectors::metadata::connector_mention_slug(connector);
...
fn connector_inserted_in_messages(
    ...
    let mention_slug = sory_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    turn_store: Arc<sory_extension_api::ExtensionData>,
    ...
    cancellation_token: CancellationToken,
) -> soryResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(SoryErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(SoryErr::ContextWindowExceeded);
            }
            Err(SoryErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(SoryErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                SoryErr::Stream(_, requested_delay) => {
...
fn agent_message_text(item: &sory_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            sory_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: sory_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(sory_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, soryResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> soryResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, soryResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: soryResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            sory.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            sory.usage.reasoning_output_tokens = field::Empty,
            sory.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(sory_async_utils::CancelErr::Cancelled) => break Err(SoryErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![sory_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(SoryErr::TurnAborted);
    }
