...
use sory_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use sory_sandboxing::policy_transforms::effective_network_sandbox_policy;
use std::sync::atomic::AtomicBool;
use std::sync::atomic::Ordering;

pub(super) fn image_generation_tool_auth_allowed(auth_manager: Option<&AuthManager>) -> bool {
    auth_manager.is_some_and(AuthManager::current_auth_uses_sory_backend)
}
...
pub struct TurnContext {
    ...
    pub(crate) sory_self_exe: Option<PathBuf>,
    pub(crate) sory_linux_sandbox_exe: Option<PathBuf>,
    pub(crate) truncation_policy: TruncationPolicy,
    pub(crate) dynamic_tools: Vec<DynamicToolSpec>,
    pub(crate) turn_metadata_state: Arc<TurnMetadataState>,
    pub(crate) extension_data: Arc<sory_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_sory_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_sory_backend);
        self.features.apps_enabled_for_auth(uses_sory_backend)
    }
    ...
    pub(crate) async fn with_model(
        ...
        Self {
            ...
            sory_self_exe: self.sory_self_exe.clone(),
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let extension_data = Arc::new(sory_extension_api::ExtensionData::new(sub_id.clone()));
        TurnContext {
            sub_id,
            ...
            sory_self_exe: per_turn_config.sory_self_exe.clone(),
            sory_linux_sandbox_exe: per_turn_config.sory_linux_sandbox_exe.clone(),
    ...
    pub(crate) async fn new_turn_with_sub_id(
    ...
    ) -> soryResult<Arc<TurnContext>> {
        let notify_config_contributors = !self.services.extensions.config_contributors().is_empty();
        let update_result: soryResult<_> = {
            ...
            match state.session_configuration.clone().apply(&updates) {
                Ok(next) => {
                    ...
                    let sory_home = next.sory_home.clone();
                ...
                Err(err) => Err(SoryErr::InvalidRequest(err.to_string())),
            }
        };
        ...
        let (
            ...
            Err(err) => {
                ...
                .await;
                return Err(SoryErr::InvalidRequest(message));
            }
        };
    ...
    ) -> soryResult<ResolvedTurnEnvironments> {
