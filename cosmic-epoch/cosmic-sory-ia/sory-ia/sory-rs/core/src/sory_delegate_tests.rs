...
use sory_protocol::protocol::GuardianAssessmentAction;
...
use sory_protocol::request_user_input::RequestUserInputEvent;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let cancel = CancellationToken::new();
    let forward = tokio::spawn(forward_ops(Arc::clone(&sory), rx_ops, cancel));

    let submission = Submission {
        id: "sub-1".to_string(),
        op: Op::Interrupt,
        trace: Some(sory_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
#[tokio::test]
async fn run_sory_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    assert!(matches!(result, Err(SoryErr::TurnAborted)));
}
