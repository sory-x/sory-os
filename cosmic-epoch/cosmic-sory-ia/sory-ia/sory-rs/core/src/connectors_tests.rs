...
use sory_config::AppRequirementToml;
use sory_config::AppToolRequirementToml;
use sory_config::AppToolsRequirementsToml;
...
use sory_config::types::AppsDefaultConfig;
use sory_connectors::merge::plugin_connector_to_app_info;
use sory_connectors::metadata::connector_install_url;
use sory_connectors::metadata::sanitize_name;
...
fn sory_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{SORY_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| SORY_APPS_MCP_SERVER_NAME.to_string());
...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    let sory_home = tempdir().expect("tempdir should succeed");
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let sory_home = tempdir().expect("tempdir should succeed");
    std::fs::write(sory_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(sory_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .sory_home(sory_home.path().to_path_buf())
        .fallback_cwd(Some(sory_home.path().to_path_buf()))
        .build()
