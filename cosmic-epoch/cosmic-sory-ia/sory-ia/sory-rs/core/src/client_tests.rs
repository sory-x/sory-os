...
use super::X_SORY_INSTALLATION_ID_HEADER;
use super::X_SORY_PARENT_THREAD_ID_HEADER;
use super::X_SORY_TURN_METADATA_HEADER;
use super::X_SORY_WINDOW_ID_HEADER;
...
use sory_model_provider::BearerAuthProvider;
use sory_model_provider_info::CHATGPT_SORY_BASE_URL;
use sory_model_provider_info::ModelProviderInfo;
use sory_model_provider_info::WireApi;
use sory_model_provider_info::create_oss_provider_with_base_url;
...
use sory_rollout_trace::ExecutionStatus;
use sory_rollout_trace::InferenceTraceAttempt;
use sory_rollout_trace::InferenceTraceContext;
use sory_rollout_trace::RawTraceEventPayload;
use sory_rollout_trace::RolloutTrace;
use sory_rollout_trace::TraceWriter;
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::soryTurnStarted {
        sory_turn_id: "turn-1".to_string(),
        thread_id: "thread-root".to_string(),
    })?;
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(CHATGPT_SORY_BASE_URL.to_string())),
        )
    } else {
        (
            None,
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_sory_responses() {
...
#[tokio::test]
async fn non_chatgpt_sory_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
