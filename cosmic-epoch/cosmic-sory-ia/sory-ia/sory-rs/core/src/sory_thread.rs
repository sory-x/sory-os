...
use sory_thread_store::ThreadStoreResult;
...
impl ThreadConfigSnapshot {
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        sory_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct SoryThreadTurnContextOverrides {
...
/// (formerly called a conversation) in sory.
impl SoryThread {
    pub(crate) fn new(
        sory: Sory,
        session_configured: SessionConfiguredEvent,
    ...
    pub async fn submit(&self, op: Op) -> soryResult<String> {
        self.sory.submit(op).await
    }

    /// Returns the session telemetry handle for thread-scoped production instrumentation.
    pub fn session_telemetry(&self) -> SessionTelemetry {
        self.sory.session.services.session_telemetry.clone()
    }

    pub async fn shutdown_and_wait(&self) -> soryResult<()> {
        self.sory.shutdown_and_wait().await
    }

    /// Wait until the underlying session loop has terminated.
    pub async fn wait_until_terminated(&self) {
        self.sory.session_loop_termination.clone().await;
    }
    ...
    pub(crate) fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor.on_thread_resume(sory_extension_api::ThreadResumeInput {
                session_store: &self.sory.session.services.session_extension_data,
                thread_store: &self.sory.session.services.thread_extension_data,
            });
        }
    }
    ...
    pub async fn ensure_rollout_materialized(&self) {
        self.sory.session.ensure_rollout_materialized().await;
    }

    #[doc(hidden)]
    pub async fn flush_rollout(&self) -> std::io::Result<()> {
        self.sory.session.flush_rollout().await
    }
    ...
    pub async fn validate_turn_context_overrides(
        &self,
        overrides: SoryThreadTurnContextOverrides,
    ) -> ConstraintResult<()> {
        let SoryThreadTurnContextOverrides {
            cwd,
            workspace_roots,
        ...
        self.sory.session.validate_settings(&updates).await
    }

    /// Use sparingly: this is intended to be removed soon.
    pub async fn submit_with_id(&self, sub: Submission) -> soryResult<()> {
        self.sory.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> soryResult<Event> {
        self.sory.next_event().await
    }

    pub async fn agent_status(&self) -> AgentStatus {
        self.sory.agent_status().await
    }

    pub(crate) fn subscribe_status(&self) -> watch::Receiver<AgentStatus> {
        self.sory.agent_status.clone()
    }
    ...
    pub async fn token_usage_info(&self) -> Option<TokenUsageInfo> {
        self.sory.session.token_usage_info().await
    }
    ...
    pub(crate) async fn inject_user_message_without_turn(&self, message: String) {
        ...
        if self
            ...
            .is_err()
        {
            let turn_context = self.sory.session.new_default_turn().await;
            self.sory
                .session
    ...
    #[cfg(test)]
    pub(crate) async fn append_message(&self, message: ResponseItem) -> soryResult<String> {
            ...
            .await
        {
            self.sory
                ...
                .await;
            self.sory.session.maybe_start_turn_for_pending_work().await;
        }
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> soryResult<()> {
        if items.is_empty() {
            return Err(SoryErr::InvalidRequest(
                "items must not be empty".to_string(),
            ));
        }

        let turn_context = self.sory.session.new_default_turn().await;
        if self.sory.session.reference_context_item().await.is_none() {
            self.sory
                .session
    ...
    pub async fn config_snapshot(&self) -> ThreadConfigSnapshot {
        self.sory.thread_config_snapshot().await
    }

    pub async fn config(&self) -> Arc<crate::config::Config> {
        self.sory.session.get_config().await
    }
    ...
    pub async fn refresh_runtime_config(&self, next_config: crate::config::Config) {
        self.sory.session.refresh_runtime_config(next_config).await;
    }

    pub async fn environment_selections(&self) -> Vec<TurnEnvironmentSelection> {
        self.sory.thread_environment_selections().await
    }
    ...
    pub fn enabled(&self, feature: Feature) -> bool {
        self.sory.enabled(feature)
    }

    pub async fn increment_out_of_band_elicitation_count(&self) -> soryResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        let was_zero = *guard == 0;
        *guard = guard.checked_add(1).ok_or_else(|| {
            SoryErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> soryResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        if *guard == 0 {
            return Err(SoryErr::InvalidRequest(
...
fn pending_message_input_item(message: &ResponseItem) -> soryResult<ResponseInputItem> {
    match message {
