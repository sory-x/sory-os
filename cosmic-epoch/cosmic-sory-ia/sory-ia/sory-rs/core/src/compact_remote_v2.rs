...
use sory_rollout_trace::CompactionCheckpointTracePayload;
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped { reason } => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    sory_analytics::CompactionStatus::Interrupted,
                    Some(error),
                )
                .await;
...
) -> soryResult<(ResponseItem, String)> {
...
mod tests {
    ...
    fn response_stream(events: Vec<soryResult<ResponseEvent>>) -> ResponseStream {
