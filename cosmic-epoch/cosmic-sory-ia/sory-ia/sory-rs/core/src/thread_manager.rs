...
use crate::sory_thread::SoryThread;
...
use crate::session::Sory;
use crate::session::SorySpawnArgs;
use crate::session::SorySpawnOk;
...
use sory_extension_api::empty_extension_registry;
...
use sory_model_provider_info::OPENAI_PROVIDER_ID;
...
use sory_protocol::config_types::CollaborationModeMask;
...
use sory_state::DirectionalThreadSpawnEdgeStatus;
use sory_thread_store::InMemoryThreadStore;
use sory_thread_store::LocalThreadStore;
use sory_thread_store::LocalThreadStoreConfig;
use sory_thread_store::ReadThreadByRolloutPathParams;
use sory_thread_store::ReadThreadParams;
use sory_thread_store::StoredThread;
use sory_thread_store::ThreadMetadataPatch;
use sory_thread_store::ThreadStore;
use sory_thread_store::ThreadStoreError;
use sory_thread_store::UpdateThreadMetadataParams;
...
struct TempsoryHomeGuard {
    path: PathBuf,
}

impl Drop for TempsoryHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created sory thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<SoryThread>,
...
pub struct ThreadManager {
    ...
    _test_sory_home_guard: Option<TempsoryHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<sory_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<SoryThread>>>>,
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        let sory_home = config.sory_home.clone();
        ...
        Self {
            ...
            _test_sory_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided soryAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let sory_home = std::env::temp_dir().join(format!(
            "sory-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&sory_home)
            .unwrap_or_else(|err| panic!("temp sory home dir create failed: {err}"));
        ...
        manager._test_sory_home_guard = Some(TempsoryHomeGuard { path: sory_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided soryAuth and sory home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_sory_home = match AbsolutePathBuf::from_absolute_path_checked(&sory_home) {
            Ok(sory_home) => sory_home,
            Err(err) => panic!("test sory_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                sory_home: sory_home.clone(),
                sqlite_home: sory_home.clone(),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(sory_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_manager,
                plugins_manager,
                mcp_manager,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> soryResult<Arc<SoryThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `SoryThread`/`LiveThread`, so metadata changes stay ordered
    ...
    ) -> soryResult<StoredThread> {
        if let Ok(thread) = self.get_thread(thread_id).await {
            if thread.config_snapshot().await.ephemeral {
                return Err(SoryErr::InvalidRequest(format!(
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    SoryErr::ThreadNotFound(thread_id)
                }
        ...
        if let Some(state_db_ctx) = thread.state_db() {
            for status in [
                ...
                for descendant_id in state_db_ctx
                    ...
                    .await
                    .map_err(|err| {
                        SoryErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                    })?
                {
    ...
    pub async fn start_thread(&self, config: Config) -> soryResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<sory_protocol::dynamic_tools::DynamicToolSpec>,
        persist_extended_history: bool,
    ) -> soryResult<NewThread> {
    ...
    /// as `Arc<SoryThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<SoryThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> soryResult<Arc<SoryThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(SoryErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => SoryErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> soryResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<SoryThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(sory, thread_id, tracked_session_source)
            .await?;
    ...
    async fn finalize_thread_spawn(
        ...
        let event = sory.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(SoryErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(SoryThread::new(
                    sory,
        ...
        if let Err(err) = sory.shutdown_and_wait().await {
            warn!("failed to shut down duplicate thread {thread_id}: {err}");
        }
        Err(SoryErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> sory_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return sory_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            .await
            .ok()
            .map(|thread| thread.sory.session.services.rollout_thread_trace.clone())
            .unwrap_or_else(sory_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> SoryErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => SoryErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => SoryErr::InvalidRequest(message),
        err => SoryErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> SoryErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => SoryErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => SoryErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => SoryErr::UnsupportedOperation(format!(
            "thread metadata update is not supported by this store: {operation}"
        )),
        err => SoryErr::Fatal(format!(
