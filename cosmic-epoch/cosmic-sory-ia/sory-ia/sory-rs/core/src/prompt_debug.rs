...
pub async fn build_prompt_input(
    ...
    let thread_manager = ThreadManager::new(
        ...
        SessionSource::Exec,
        Arc::new(
            EnvironmentManager::from_sory_home(config.sory_home.clone(), local_runtime_paths)
                .await
                .map_err(|err| SoryErr::Fatal(err.to_string()))?,
        ),
    ...
    let output = build_prompt_input_from_session(thread.thread.sory.session.as_ref(), input).await;
