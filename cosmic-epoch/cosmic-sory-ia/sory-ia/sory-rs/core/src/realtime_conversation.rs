...
use sory_api::RealtimeAudioFrame;
use sory_api::RealtimeEvent;
use sory_api::RealtimeEventParser;
use sory_api::RealtimeSessionConfig;
use sory_api::RealtimeSessionMode;
use sory_api::RealtimeWebsocketClient;
use sory_api::RealtimeWebsocketEvents;
use sory_api::RealtimeWebsocketWriter;
...
use sory_login::read_openai_api_key_from_env;
...
use sory_protocol::protocol::RealtimeConversationClosedEvent;
use sory_protocol::protocol::RealtimeConversationRealtimeEvent;
use sory_protocol::protocol::RealtimeConversationSdpEvent;
use sory_protocol::protocol::RealtimeConversationStartedEvent;
use sory_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> soryResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> soryResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> soryResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(SoryErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> soryResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| SoryErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> soryResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> soryResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> soryResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> soryResult<PreparedRealtimeConversationStart> {
...
) -> soryResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> soryResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), SoryErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&soryAuth>, provider: &ModelProviderInfo) -> soryResult<String> {
    ...
    if let Some(api_key) = auth.and_then(soryAuth::api_key) {
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            SoryErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
