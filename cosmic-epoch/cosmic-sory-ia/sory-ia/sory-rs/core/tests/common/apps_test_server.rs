use crate::test_sory::TestSoryBuilder;
use crate::test_sory::test_sory;
use anyhow::Result;
...
const sory_APPS_META_KEY: &str = "_sory_apps";
const PROTOCOL_VERSION: &str = "2025-11-25";
const SERVER_NAME: &str = "sory-apps-test";
...
pub const DIRECT_CALENDAR_CREATE_EVENT_TOOL: &str = "mcp__sory_apps__calendar_create_event";
pub const DIRECT_CALENDAR_LIST_EVENTS_TOOL: &str = "mcp__sory_apps__calendar_list_events";
pub const DIRECT_CALENDAR_EXTRACT_TEXT_TOOL: &str = "mcp__sory_apps__calendar_extract_text";
pub const SEARCH_CALENDAR_NAMESPACE: &str = "mcp__sory_apps__calendar";
...
pub fn apps_enabled_builder(apps_base_url: impl Into<String>) -> TestSoryBuilder {
...
pub fn search_capable_apps_builder(apps_base_url: impl Into<String>) -> TestSoryBuilder {
    let apps_base_url = apps_base_url.into();
    test_sory()
        .with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing())
...
async fn recorded_apps_tool_calls(server: &MockServer) -> Vec<Value> {
    server
        ...
        .filter_map(|request| {
            ...
            (request.url.path() == "/api/sory/apps"
...
async fn mount_streamable_http_json_rpc(
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/sory/apps/?$"))
        .respond_with(soryAppsJsonRpcResponder {
            connector_name,
...
struct soryAppsJsonRpcResponder {
    connector_name: String,
    connector_description: String,
    searchable: bool,
}

impl Respond for soryAppsJsonRpcResponder {
        ...
        match method {
            ...
            "tools/call" => {
                ...
                let file_id = body
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or_default();
                let sory_apps_meta = body.pointer("/params/_meta/_sory_apps").cloned();
