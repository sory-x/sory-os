...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = sory_utils_cargo_bin::cargo_bin("sory-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `sory-execve-wrapper` binary");
        return Ok(None);
    };
