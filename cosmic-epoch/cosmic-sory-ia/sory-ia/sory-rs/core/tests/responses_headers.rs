...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let sory_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-sory-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-sory-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-sory-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(request.header("x-sory-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_sory().build(&server).await.expect("build test sory");
    ...
    let initial_header = first_request
        ...
        .expect("x-sory-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-sory-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-sory-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/sory.git",
    ]);
