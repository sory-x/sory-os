...
const REMOTE_MODEL: &str = "sory-test-ttl";
const VERSIONED_MODEL: &str = "sory-test-versioned";
const MISSING_VERSION_MODEL: &str = "sory-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "sory-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.sory_home.join(CACHE_FILE);
    ...
    sory
        .submit(Op::UserTurn {
            ...
            approval_policy: sory_protocol::protocol::AskForApproval::Never,
    ...
    let _ = wait_for_event(&sory, |event| matches!(event, EventMsg::TurnComplete(_))).await;
