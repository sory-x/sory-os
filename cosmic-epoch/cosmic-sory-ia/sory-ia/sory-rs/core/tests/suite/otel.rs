...
fn extract_log_field_handles_empty_bare_values() {
    let line = "event.name=\"sory.tool_result\" mcp_server= mcp_server_origin=";
...
fn extract_log_field_does_not_confuse_similar_keys() {
    let line = "event.name=\"sory.tool_result\" mcp_server_origin=stdio";
...
async fn responses_api_emits_api_request_event() {
    ...
    let TestSory { sory, .. } = test_sory().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("sory.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected sory.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("sory.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected sory.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing sory.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("sory.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing sory.sse_event".to_string()))
    });
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("handle_responses{")
                ...
                && line.contains("sory.request.reasoning_effort=high")
                ...
                && line.contains("sory.usage.reasoning_output_tokens=2")
                && line.contains("sory.usage.total_tokens=9")
        }),
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("turn{otel.name=\"session_task.turn\"")
                && line.contains("sory.turn.reasoning_effort=high")
                && line.contains("sory.turn.token_usage.input_tokens=3")
                && line.contains("sory.turn.token_usage.cached_input_tokens=1")
                && line.contains("sory.turn.token_usage.non_cached_input_tokens=2")
                && line.contains("sory.turn.token_usage.output_tokens=5")
                && line.contains("sory.turn.token_usage.reasoning_output_tokens=2")
                && line.contains("sory.turn.token_usage.total_tokens=9")
        }),
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("sory.tool_result") && line.contains("call_id=custom-tool-call")
            })
            .ok_or_else(|| "missing sory.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    wait_for_event(&sory, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("sory.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing sory.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_shell_command_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| line.contains("sory.tool_result") && line.contains("call_id=shell-call"))
            .ok_or_else(|| "missing sory.tool_result event".to_string())?;
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("sory.tool_decision") && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing sory.tool_decision event for {call_id}"))?;
...
async fn handle_shell_command_user_approved_records_tool_decision() {
    let server = start_mock_server().await;
    let command = touch_command("sory-otel-approval-test");
    mount_sse_once(
        &server,
        sse(vec![
    ...
    let approval_event =
        wait_for_event(&sory, |ev| matches!(ev, EventMsg::ExecApprovalRequest(_))).await;
