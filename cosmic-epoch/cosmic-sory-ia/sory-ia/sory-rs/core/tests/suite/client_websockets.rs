...
use serde_json::json;
use sory_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use sory_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use sory_core::ModelClient;
use sory_core::ModelClientSession;
use sory_core::Prompt;
use sory_core::ResponseEvent;
use sory_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(sory_login::default_client::get_sory_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-sory-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_SORY_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_sends_response_processed_after_remote_compaction_v2() {
    ...
    wait_for_event(&test.sory, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-sory-primary-used-percent": "100.0",
            "x-sory-secondary-used-percent": "87.5",
            "x-sory-primary-over-secondary-limit-percent": "95.0",
            "x-sory-primary-window-minutes": "15",
            "x-sory-secondary-window-minutes": "60"
        }
    });
    ...
    let token_event =
        wait_for_event(&test.sory, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.sory, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let enriched_turn_metadata = r#"{"turn_id":"turn-123","thread_source":"user","sandbox":"workspace-write","workspaces":[{"root_path":"/tmp/repo","latest_git_commit_hash":"abc123","associated_remote_urls":["git@github.com:openai/sory.git"],"has_changes":true}]}"#;
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![
        message_item("hello"),
        assistant_message_item("msg-1", "assistant output"),
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-sory-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-sory-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn responses_websocket_preserves_custom_turn_metadata_fields() {
    ...
    assert_eq!(body["type"].as_str(), Some("response.create"));
    assert_eq!(
        body["client_metadata"]["x-sory-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = sory_core::test_support::construct_model_info_offline(MODEL, &config);
