...
use sory_core::REVIEW_PROMPT;
use sory_core::config::Config;
use sory_core::review_format::render_review_output_text;
use sory_protocol::models::ContentItem;
use sory_protocol::models::ResponseItem;
use sory_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use sory_protocol::protocol::EventMsg;
use sory_protocol::protocol::ExitedReviewModeEvent;
use sory_protocol::protocol::Op;
use sory_protocol::protocol::ReviewCodeLocation;
use sory_protocol::protocol::ReviewFinding;
use sory_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under sory sandbox network restrictions.
    skip_if_no_network!();
    ...
    let sory = new_conversation_for_server(&server, sory_home.clone(), |_| {}).await;
    ...
    let _entered = wait_for_event(&sory, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&sory, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = sory.rollout_path().expect("rollout path");
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let sory = new_conversation_for_server(&server, sory_home.clone(), |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&sory, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&sory, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = sory_home.path().join("resume.jsonl");
    {
        ...
        let user = sory_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = sory_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let sory =
        resume_conversation_for_server(&server, sory_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let sory = new_conversation_for_server(&server, sory_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, sory_home, resume_path)
        .await
