...
const REQUESTED_MODEL: &str = "gpt-5.3-sory";
const TRUSTED_ACCESS_FOR_CYBER_VERIFICATION: &str = "trusted_access_for_cyber";

const CYBER_POLICY_MESSAGE: &str =
    "This request has been flagged for potentially high-risk cyber activity.";

fn disabled_text_turn(test: &TestSory, text: &str) -> Op {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_sory().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.sory, |event| {
    ...
    let warning = wait_for_event(&test.sory, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.sory, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.sory, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.sory_error_info, Some(SoryErrorInfo::CyberPolicy));
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let warning = wait_for_event(&test.sory, |event| {
        matches!(
            event,
...
async fn openai_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&test.sory, |_| true).await;
        match event {
