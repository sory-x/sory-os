...
async fn build_sory(server: &StreamingSseServer) -> Arc<SoryThread> {
    test_sory()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming sory test session: {err}"))
        .sory
}

async fn submit_user_input(sory: &SoryThread, text: &str) {
    sory
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_danger_full_access_user_turn(test: &TestSory, text: &str) {
...
async fn steer_user_input(sory: &SoryThread, text: &str) {
    sory
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(sory: &SoryThread, text: &str) {
    sory
    ...
    wait_for_event(sory, |event| {
        matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(sory: &SoryThread) {
    wait_for_event(sory, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(sory: &SoryThread, text: &str) {
...
async fn wait_for_turn_complete(sory: &SoryThread) {
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let sory = build_sory(&server).await;

    submit_user_input(&sory, "first prompt").await;

    wait_for_reasoning_item_started(&sory).await;

    submit_queue_only_agent_mail(&sory, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&sory, "first answer").await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&sory, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    submit_user_input(&sory, "first prompt").await;
    submit_user_input(&sory, "second prompt").await;

    wait_for_agent_message(&sory, "resumed old task").await;
    wait_for_turn_complete(&sory).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&sory, "processed steered prompt").await;
    wait_for_turn_complete(&sory).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&sory, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&sory, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
