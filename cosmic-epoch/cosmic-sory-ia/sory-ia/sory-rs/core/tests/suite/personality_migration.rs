...
use sory_core::SESSIONS_SUBDIR;
use sory_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use sory_core::personality_migration::PersonalityMigrationStatus;
use sory_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(sory_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(sory_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(sory_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(sory_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = sory_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(sory_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = sory_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
