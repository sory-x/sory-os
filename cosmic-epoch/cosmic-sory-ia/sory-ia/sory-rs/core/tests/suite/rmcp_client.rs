...
use sory_exec_server::HttpRequestParams;
use sory_login::soryAuth;
use sory_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use sory_utils_cargo_bin::cargo_bin;
...
fn read_only_user_turn(fixture: &TestSory, text: impl Into<String>) -> Op {
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/sory-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn wait_for_mcp_server(fixture: &TestSory, server_name: &str) -> anyhow::Result<()> {
...
async fn call_cwd_tool(
    ...
    wait_for_event(&fixture.sory, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.sory, |ev| {
    ...
    wait_for_event(&fixture.sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.sory, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.sory, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: sory_protocol::openai_models::ReasoningEffort::Medium,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that sory should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: sory can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so sory will call the MCP echo tool
    ...
    // Phase 3: configure sory with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert sory begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.sory, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in sory_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire sory_HOME through the code.
#[test]
#[serial(sory_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so sory will call the OAuth-backed
    ...
    // Phase 3: seed an isolated sory_HOME with fallback OAuth tokens for this
    ...
    // Phase 4: configure sory with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert sory begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.sory, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the sory fixture.
