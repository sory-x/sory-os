...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.sory);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.sory, "after resume").await?;
    shutdown_thread(&resumed.sory).await?;
...
async fn submit_user_turn(sory: &Arc<SoryThread>, text: &str) -> Result<()> {
    sory
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_compact_turn(sory: &Arc<SoryThread>) -> Result<()> {
    sory.submit(Op::Compact).await?;
    let warning_event = wait_for_event(sory, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(sory: &Arc<SoryThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-sory-window-id")
        .expect("missing x-sory-window-id header");
