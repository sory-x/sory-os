...
use sory_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestSory) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured sory helper binary
...
impl Expectation {
    fn verify(&self, test: &TestSory, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(test: &TestSory, expected_command: &str) -> ExecApprovalRequestEvent {
    let event = wait_for_event(&test.sory, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestSory) {
    let event = wait_for_event(&test.sory, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestSory) {
...
async fn wait_for_spawned_thread(test: &TestSory) -> Result<Arc<SoryThread>> {
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_sory().with_model(model).with_config(move |config| {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/sory-fallback-rule-test.txt && echo hello > /tmp/sory-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://sory-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
