...
async fn run_turn(test: &TestSory, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestSory, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_sory_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestSory> {
    let mut builder = test_sory().with_model("test-gpt-5.1-sory");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_sory_with_test_tool(&server).await?;
