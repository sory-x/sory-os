...
//! request payload that sory would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(sory_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<SoryThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.sory)
}

async fn user_turn(conversation: &Arc<SoryThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<SoryThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<SoryThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<SoryThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = sory_core::test_support::auth_manager_from_auth(
        sory_login::soryAuth::from_api_key("dummy"),
    );
