...
async fn request_body_is_zstd_compressed_for_sory_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/sory/v1", server.uri());
    ...
    let sory = builder.build(&server).await?.sory;
