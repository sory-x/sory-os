...
use sory_core::ModelClient;
use sory_core::NewThread;
use sory_core::Prompt;
use sory_core::ResponseEvent;
use sory_core::ThreadManager;
use sory_core::resolve_installation_id;
use sory_core::thread_store_from_config;
...
use sory_protocol::models::LocalShellAction;
use sory_protocol::models::LocalShellExecAction;
use sory_protocol::models::LocalShellStatus;
...
use sory_protocol::protocol::SessionMeta;
...
/// Writes an `auth.json` into the provided `sory_home` with the specified parameters.
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: match sory_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path()) {
                Ok(cwd) => cwd,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = sory_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = sory_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![sory_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = sory_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure sory to resume from our file
    let sory_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_sory()
        .with_home(sory_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, sory_home, session_path.clone())
        .await
        .expect("resume conversation");
    let sory = test.sory.clone();
    ...
    sory
        ...
        .await
        .unwrap();
    wait_for_event(&sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let sory_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_sory().with_model("gpt-5.4");
    let test = builder
        .resume(&server, sory_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_sory().with_auth(soryAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.sory_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_eq!(
        request_body["client_metadata"]["x-sory-installation-id"].as_str(),
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let sory_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&sory_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort;
    let summary = config.model_reasoning_summary;
    let model = sory_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info = sory_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(soryAuth::from_api_key(
            "unused-api-key",
        ))),
        thread_id.into(),
        thread_id,
    ...
    let mut stream = client_session
        .stream(
            ...
            &sory_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_sory()
        .with_auth(soryAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/sory", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_sory()
        .with_auth(create_dummy_sory_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/sory/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match soryAuth::from_auth_storage(
        sory_home.path(),
    ...
    .await
    {
        Ok(Some(auth)) => sory_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No soryAuth found in sory_home"),
        Err(e) => panic!("Failed to load soryAuth: {e}"),
    };
    let installation_id = resolve_installation_id(&config.sory_home)
        .await
    ...
    let NewThread { thread: sory, .. } = thread_manager
...
async fn skills_append_to_developer_message() {
    ...
    let sory_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = sory_home.path().join("skills/demo");
    ...
    let sory_home_path = sory_home.path().to_path_buf();
    let mut builder = test_sory()
        .with_home(sory_home.clone())
        .with_auth(soryAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = sory_home_path.abs();
        });
    let sory = builder
        .build(&server)
        .await
    ...
    let _sory_home_guard = sory_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let sory_home_parent = TempDir::new().unwrap();
    let long_home_parent = sory_home_parent
        .path()
        .join("sory-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let sory_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = sory_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_sory()
        ...
        .with_config(move |config| {
            config.cwd = sory_home_path.abs();
            let user_config_path = sory_home_path.join("config.toml").abs();
    ...
    let _sory_home_guard = sory_home;
    let _sory_home_parent_guard = sory_home_parent;
}
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestSory { sory, .. } = test_sory().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    let TestSory {
        ...
        session_configured,
        ..
    } = test_sory().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        sory_core::test_support::auth_manager_from_auth(soryAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-sory-primary-used-percent", "12.5")
        .insert_header("x-sory-secondary-used-percent", "40.0")
        .insert_header("x-sory-primary-window-minutes", "10")
        .insert_header("x-sory-secondary-window-minutes", "60")
        .insert_header("x-sory-primary-reset-at", "1704069000")
        .insert_header("x-sory-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_sory()
        .with_auth(soryAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    wait_for_event(&sory, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-sory-primary-used-percent", "100.0")
        .insert_header("x-sory-secondary-used-percent", "87.5")
        .insert_header("x-sory-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-sory-primary-window-minutes", "15")
        .insert_header("x-sory-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_sory();
    let sory_fixture = builder.build(&server).await?;
    let sory = sory_fixture.sory.clone();
    ...
    let token_event = wait_for_event(&sory, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&sory, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&sory, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&sory, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = SoryErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_sory_auth() -> soryAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under sory sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
