...
const FIXED_CWD: &str = "/tmp/sory_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> soryAuth {
        match self {
            AuthCase::ChatGpt => soryAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => soryAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&sory, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().sory_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestSoryHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestSoryHarness> {
...
) -> Result<TestSoryHarness> {
    fs::create_dir_all(FIXED_CWD)?;
    let mut builder = test_sory().with_auth(settings.auth.build());
    if hooks {
        builder = builder.with_pre_build_hook(write_manual_compact_hooks);
    }
    TestSoryHarness::with_builder(builder.with_config(move |config| {
        config.cwd =
            sory_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(FIXED_CWD))
...
fn rollout_path(harness: &TestSoryHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn submit_user_input(sory: &sory_core::SoryThread, items: Vec<UserInput>) -> Result<()> {
    sory
        .submit(Op::UserInput {
            environments: None,
            items,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<sory_HOME>");
            search_start = start_index + "<sory_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <sory_HOME>/skills/.system/imagegen/SKILL.md and \
         <sory_HOME>/skills/custom/SKILL.md"
    );
}
