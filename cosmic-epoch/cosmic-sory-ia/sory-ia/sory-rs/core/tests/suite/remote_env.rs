...
use core_test_support::test_sory::test_env;
...
use sory_exec_server::CopyOptions;
use sory_exec_server::CreateDirectoryOptions;
use sory_exec_server::FileSystemSandboxContext;
use sory_exec_server::LOCAL_ENVIRONMENT_ID;
use sory_exec_server::REMOTE_ENVIRONMENT_ID;
...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestSory> {
...
async fn wait_for_completion_without_patch_approval(test: &TestSory) {
    let event = wait_for_event(&test.sory, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sory-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    test.fs()
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sory-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    test.fs()
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/sory-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sory-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    test.fs()
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/sory-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/sory-remote-dotdot-{}", std::process::id()));
...
fn remote_test_file_path() -> PathBuf {
    ...
    PathBuf::from(format!(
        "/tmp/sory-remote-test-env-{}-{nanos}.txt",
        std::process::id()
    ))
}
