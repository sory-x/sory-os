...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let sory_home = home.path().join(".sory");
    ...
    let mut cmd = Command::new(sory_utils_cargo_bin::cargo_bin("sory-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn sory-rs");
