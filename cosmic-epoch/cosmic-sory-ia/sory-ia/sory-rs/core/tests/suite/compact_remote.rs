...
use core_test_support::test_sory::TestSoryBuilder;
use core_test_support::test_sory::TestSoryHarness;
...
use serde_json::json;
use sory_core::compact::SUMMARY_PREFIX;
...
use sory_protocol::protocol::RealtimeConversationRealtimeEvent;
use sory_protocol::protocol::RealtimeEvent;
use sory_protocol::protocol::RealtimeOutputModality;
...
fn remote_realtime_test_sory_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestSoryBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_sory()
        .with_auth(soryAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(sory: &sory_core::SoryThread) -> Result<()> {
    sory
    ...
    wait_for_event_match(sory, |msg| match msg {
...
async fn close_realtime_conversation(sory: &sory_core::SoryThread) -> Result<()> {
    sory.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(sory, |msg| match msg {
...
async fn wait_for_turn_complete(sory: &sory_core::SoryThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestSoryHarness::with_builder(
        test_sory().with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let sory = harness.test().sory.clone();
    ...
    sory
        ...
        .await?;
    wait_for_turn_complete(&sory).await;

    sory.submit(Op::Compact).await?;
    wait_for_turn_complete(&sory).await;
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_sory().with_auth(auth);
    ...
    let harness = TestSoryHarness::with_builder(builder).await?;
    let sory = harness.test().sory.clone();
    let image_url =
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    let harness = TestSoryHarness::with_builder(
        test_sory()
            .with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_sory().with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let dynamic_tools = vec![
        DynamicToolSpec {
            namespace: Some("sory_app".to_string()),
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "sory_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "sory_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&sory, |event| match event {
        EventMsg::ContextCompacted(_) => Some(true),
        _ => None,
    })
    .await;
    wait_for_event(&sory, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    assert!(message);
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&sory, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestSoryHarness::with_builder(
        test_sory()
            .with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_sory = baseline_harness.test().sory.clone();
    ...
    baseline_sory
        ...
        .await?;
    wait_for_event(&baseline_sory, |event| {
    ...
    baseline_sory.submit(Op::Compact).await?;
    wait_for_event(&baseline_sory, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestSoryHarness::with_builder(
        test_sory()
            .with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_sory = override_harness.test().sory.clone();
    ...
    override_sory
        ...
        .await?;
    wait_for_event(&override_sory, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_sory.submit(Op::Compact).await?;
    wait_for_event(&override_sory, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = sory.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    sory.submit(Op::Shutdown).await?;
    wait_for_event(&sory, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.sory.submit(Op::Compact).await?;
    wait_for_event(&initial.sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    initial.sory.submit(Op::Shutdown).await?;
    wait_for_event(&initial.sory, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.sory
        ...
        .await?;
    wait_for_event(&test.sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.sory.submit(Op::Compact).await?;
    wait_for_event(&test.sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_sory_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.sory.as_ref()).await?;
    ...
    close_realtime_conversation(test.sory.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_sory_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_sory_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.sory.as_ref()).await?;
    ...
    close_realtime_conversation(initial.sory.as_ref()).await?;
