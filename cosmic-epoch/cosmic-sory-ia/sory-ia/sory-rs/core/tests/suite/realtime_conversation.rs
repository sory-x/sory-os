...
use sory_core::test_support::auth_manager_from_auth;
...
use sory_protocol::protocol::RealtimeConversationVersion;
use sory_protocol::protocol::RealtimeEvent;
use sory_protocol::protocol::RealtimeNoopRequested;
...
use sory_utils_output_truncation::approx_token_count;
use std::fs;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from sory.";
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "sory_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in sory_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let db = test.sory.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = sory_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.sory, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.sory, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.sory, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.sory.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.sory, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.sory, |msg| match msg {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=sory-realtime-call-boundary")
    );
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.sory, |msg| match msg {
    ...
    let stale_event = timeout(Duration::from_millis(700), async {
        wait_for_event_match(&test.sory, |msg| match msg {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.sory, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.sory.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.sory, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.sory_error_info, Some(SoryErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.sory
        ...
        .await?;
    let _ = wait_for_event_match(&test.sory, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_experimental_realtime_ws_base_url_override() -> Result<()> {
    ...
    let mut builder = test_sory().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_sory().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_rejects_voice_for_wrong_realtime_version() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.sory, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let history = user_turns
        ...
        .collect::<Vec<_>>();
    test.sory.shutdown_and_wait().await?;
    let resumed_thread = test
        .thread_manager
        .resume_thread_with_history(
            test.config.clone(),
            InitialHistory::Forked(history),
            auth_manager_from_auth(soryAuth::from_api_key("dummy")),
            /*persist_extended_history*/ false,
            /*parent_trace*/ None,
        )
        .await?;
    let sory = resumed_thread.thread;
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("sory-rs/core"))?;
    ...
    assert!(startup_context.contains("sory-rs/"));
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_sory().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
