...
async fn build_analytics_plugin_test_sory(
    server: &MockServer,
    sory_home: Arc<TempDir>,
) -> Result<Arc<sory_core::SoryThread>> {
...
async fn build_apps_enabled_plugin_test_sory(
    server: &MockServer,
    sory_home: Arc<TempDir>,
...
async fn wait_for_sample_mcp_ready(sory: &sory_core::SoryThread) -> Result<()> {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let sory_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(sory_home.as_ref());
    write_plugin_app_plugin(sory_home.as_ref());
    let sory = build_apps_enabled_plugin_test_sory(
        &server,
        Arc::clone(&sory_home),
    ...
    sory
        .submit(Op::UserInput {
            environments: None,
            items: vec![sory_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(sory_home.as_ref());
    write_plugin_mcp_plugin(sory_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(sory_home.as_ref());

    let sory =
        build_apps_enabled_plugin_test_sory(&server, sory_home, apps_server.chatgpt_base_url)
            .await?;
    wait_for_sample_mcp_ready(&sory).await?;

    sory
        .submit(Op::UserInput {
            environments: None,
            items: vec![sory_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__sory_apps__google_calendar"),
    ...
    let calendar_tool = request
        .tool_by_name("mcp__sory_apps__google_calendar", "_create_event")
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let sory_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(sory_home.as_ref());
    let sory = build_analytics_plugin_test_sory(&server, sory_home).await?;
    ...
    assert_eq!(
        ...
        serde_json::json!(sory_login::default_client::originator().value)
    );
