...
use sory_core::RolloutRecorder;
use sory_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(sory_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = sory_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(sory_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(sory_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(sory_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = sory_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_sory_home_directory() {
    let repo = TempDir::new().unwrap();
    let sory_home = repo.path().join(".sory");
    std::fs::create_dir_all(&sory_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".sory/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&sory_home, id);

    let found =
        find_thread_path_by_id_str(&sory_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
