...
use core_test_support::test_sory::ShellModelOutput;
...
fn configure_shell_model(
    builder: TestSoryBuilder,
    output_type: ShellModelOutput,
) -> TestSoryBuilder {
    match output_type {
        ShellModelOutput::ShellCommand => builder.with_model("test-gpt-5-sory"),
    }
}
...
async fn shell_output_is_structured_with_freeform_apply_patch(
    ...
    let mut builder = configure_shell_model(test_sory(), output_type);
