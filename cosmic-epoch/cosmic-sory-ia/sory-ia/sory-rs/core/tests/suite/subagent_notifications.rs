...
use sory_core::config::AgentRoleConfig;
...
const INHERITED_MODEL: &str = "gpt-5.3-sory";
...
fn write_home_skill(sory_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = sory_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestSory) -> Result<String> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_sory::TestSoryBuilder,
    ) -> core_test_support::test_sory::TestSoryBuilder,
) -> Result<(TestSory, String)> {
    ...
    let mut builder = configure_test(test_sory().with_config(|config| {
        config
            .features
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.sory_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
