...
use sory_core::compact::SUMMARIZATION_PROMPT;
...
async fn assert_compaction_uses_turn_lifecycle_id(sory: &std::sync::Arc<sory_core::SoryThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = sory.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn sory.
    ...
    sory.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&sory, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down sory to flush rollout entries before inspecting the file.
    sory.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    sory.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&sory, |ev| match ev {
    ...
    wait_for_event(&sory, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.sory_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.sory_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let sory = builder.build(&server).await.unwrap().sory;
    ...
    let first = wait_for_event_match(&sory, |ev| match ev {
    ...
    let last = wait_for_event_match(&sory, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        sory_protocol::models::ResponseItem::Message {
            id: None,
        ...
        sory_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_sory().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.sory, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let previous_model = "gpt-5.3-sory";
    ...
    let test = builder.build(&server).await.expect("build test sory");
    ...
    test.sory
        ...
        .await
        .expect("submit first user turn");
    wait_for_event(&test.sory, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.sory).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut initial_builder = test_sory()
        .with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test sory");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.sory, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut resumed_builder = test_sory()
        .with_auth(soryAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.sory).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&sory, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&sory, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let sory = builder.build(&server).await.unwrap();

    sory.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    sory.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    sory.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&sory, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
