...
fn repo_root() -> std::path::PathBuf {
    ...
    sory_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = sory_utils_cargo_bin::cargo_bin("sory").unwrap();
    ...
    //     10,
    //     None,
    //     sory_core::ThreadSortKey::UpdatedAt,
    //     &[],
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // sory to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `sory exec --profile ...` preserves the active profile when it
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the sory CLI and invoke `exec`, which is what records a session.
    let bin = sory_utils_cargo_bin::cargo_bin("sory").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.timeout(Duration::from_secs(30));
    cmd.arg("exec")
    ...
    let bin2 = sory_utils_cargo_bin::cargo_bin("sory").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("sory_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume sory-cli run failed");
