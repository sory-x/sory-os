...
use core_test_support::test_sory::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestSoryHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestSoryBuilder) -> TestSoryBuilder,
) -> Result<TestSoryHarness> {
    let builder = configure(test_sory());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestSoryHarness::with_remote_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestSoryHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_sory_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let sory_linux_sandbox_exe = harness
        .test()
        .config
        .sory_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include sory-linux-sandbox helper");
    assert_eq!(
        sory_linux_sandbox_exe
            .file_name()
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&sory_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
