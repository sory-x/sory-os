...
#[ignore = "requires tmux and a locally built sory binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let sory = sory_binary(&repo_root)?;
    ...
    write_auth(sory_home.path())?;

    let session_name = format!("sory-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("sory_HOME={}", sory_home.path().display()))
    ...
    let sory_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!sory_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&sory_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&sory_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("sory-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&sory_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let session_name = format!("sory-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn sory_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = sory_utils_cargo_bin::cargo_bin("sory") {
        return Ok(path);
    }

    let fallback = repo_root.join("sory-rs/target/debug/sory");
    anyhow::ensure!(
        fallback.is_file(),
        "sory binary is unavailable; run `cargo build -p sory-cli` first"
    );
    Ok(fallback)
}
...
"#
    ...
    std::fs::write(sory_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(sory_home: &Path) -> Result<()> {
    std::fs::write(
