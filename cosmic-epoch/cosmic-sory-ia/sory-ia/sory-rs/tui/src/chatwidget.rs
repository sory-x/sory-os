//! The main sory TUI chat surface.
//!
...
use crate::version::SORY_CLI_VERSION;
...
use sory_app_server_protocol::SoryErrorInfo as AppServersoryErrorInfo;
...
use sory_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use sory_app_server_protocol::ConfigLayerSource;
use sory_app_server_protocol::CreditsSnapshot;
...
use sory_app_server_protocol::ModelVerification as AppServerModelVerification;
use sory_app_server_protocol::RateLimitReachedType;
...
use sory_app_server_protocol::SkillMetadata as ProtocolSkillMetadata;
use sory_app_server_protocol::SkillsListResponse;
use sory_app_server_protocol::ThreadGoal as AppThreadGoal;
use sory_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use sory_chatgpt::connectors as chatgpt_connectors;
...
use sory_core_skills::model::SkillMetadata;
use sory_exec_server::EnvironmentManager;
use sory_features::FEATURES;
use sory_features::Feature;
#[cfg(test)]
use sory_git_utils::CommitLogEntry;
use sory_git_utils::current_branch_name;
use sory_git_utils::get_git_repo_root;
use sory_git_utils::local_git_branches;
use sory_git_utils::recent_commits;
use sory_otel::RuntimeMetricsSummary;
use sory_otel::SessionTelemetry;
use sory_plugin::PluginCapabilitySummary;
use sory_protocol::ThreadId;
use sory_protocol::account::PlanType;
use sory_protocol::approvals::GuardianAssessmentAction;
use sory_protocol::approvals::GuardianAssessmentDecisionSource;
use sory_protocol::approvals::GuardianAssessmentEvent;
use sory_protocol::approvals::GuardianAssessmentStatus;
...
use sory_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use sory_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
use sory_protocol::request_permissions::RequestPermissionsEvent;
use sory_protocol::user_input::ByteRange;
...
const MEMORIES_DOC_URL: &str = "https://developers.openai.com/sory/memories";
...
use sory_file_search::FileMatch;
...
use sory_utils_approval_presets::ApprovalPreset;
use sory_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: sory_feedback::soryFeedback,
...
/// and by sending requests back to sory-core.
///
...
pub(crate) struct ChatWidget {
    ...
    sory_rate_limit_reached_type: Option<RateLimitReachedType>,
    ...
    feedback: sory_feedback::soryFeedback,
...
fn request_permissions_from_params(
    params: sory_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd =
            resume_command(Some(name), thread_id).unwrap_or_else(|| format!("sory resume {name}"));
    ...
    /// Forward a command directly to sory.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.sory_op_target {
            soryOpTarget::Direct(sory_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = sory_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            soryOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::soryOp(op));
            }
        }
        true
    }
