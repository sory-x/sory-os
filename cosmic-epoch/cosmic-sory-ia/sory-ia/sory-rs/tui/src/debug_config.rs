...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_sory_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_sory_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use sory_config::FilesystemConstraints;
    use sory_config::HookEventsToml;
    use sory_config::HookHandlerConfig;
    use sory_config::ManagedHooksRequirementsToml;
    use sory_config::MatcherGroup;
    ...
    use sory_config::NetworkUnixSocketPermissionToml;
    use sory_config::NetworkUnixSocketPermissionsToml;
    use sory_config::RequirementSource;
    use sory_config::ResidencyRequirement;
    use sory_config::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\sory\\config.toml")
        } else {
            absolute_path("/etc/sory/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.sory")
        } else {
            absolute_path("/repo/.sory")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_sory_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\sory\\requirements.toml")
        } else {
            absolute_path("/etc/sory/requirements.toml")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.sory\\config.toml")
        } else {
            absolute_path("/home/alice/.sory/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "experimental_network: unix_sockets={/tmp/blocked.sock=none, /tmp/sory.sock=allow} (source: cloud requirements)"
        ));
    }
