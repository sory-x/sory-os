//! Ambient terminal rendering for the sory companion.
//!
...
impl AmbientPet {
    ...
    /// sory_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_sory_home(
            ...
            /*sory_home*/ Some(sory_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = sory_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_sory_app_vocabulary() {
