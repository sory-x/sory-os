...
use sory_login::detect_provider_api_key_from_env;
...
fn onboarding_request_id() -> sory_app_server_protocol::RequestId {
    sory_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<sory_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Choose a login method to get started with sory".into(),
            ]),
            Line::from(vec![
                "  ".into(),
                "or use your own API key with any supported provider".into(),
            ]),
            "".into(),
        ];
        ...
        let chatgpt_description = if !self.is_chatgpt_login_allowed() {
            "Sory account login is disabled"
        } else {
            ...
            match option {
                ...
                SignInOption::ApiKey => {
                    lines.extend(create_mode_item(
                        ...
                        "OpenAI, Anthropic, Mistral, Google, and more",
                    ));
                }
            }
            lines.push("".into());
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  API key login is disabled by this workspace. Sign in with Sory to continue."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in to your Sory account".fg(Color::Green).into(),
            "".into(),
            ...
            .dim(),
            "".into(),
            "  sory can make mistakes".into(),
            "  Review the code it writes and commands it runs".dim().into(),
            "".into(),
            "  Powered by your Sory account".into(),
            "".into(),
            Line::from(vec![
                "  Press ".fg(Color::Cyan),
                self.confirm_binding().into(),
    ...
    fn render_chatgpt_success(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in to your Sory account"
                .fg(Color::Green)
                .into(),
        ];
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  sory is ready to use with your API key.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "Enter your API key for any supported provider".bold(),
            ]),
            "".into(),
        ...
        if state.prepopulated_from_env {
            let provider_msg = match &state.detected_provider {
                Some(name) => format!("  Detected {name} API key from environment variable."),
                None => "  Detected API key from environment variable.".into(),
            };
            intro_lines.push(provider_msg.into());
            intro_lines.push(
                "  Paste a different key if you prefer to use another provider."
                    .dim()
                    .into(),
            );
    ...
    fn start_api_key_entry(&mut self) {
        ...
        self.set_error(/*message*/ None);
        let prefill = detect_provider_api_key_from_env();
        let mut guard = self.sign_in_state.write().unwrap();
        match &mut *guard {
            SignInState::ApiKeyEntry(state) => {
                if state.value.is_empty() {
                    if let Some((provider, key)) = prefill {
                        state.value = key;
                        state.prepopulated_from_env = true;
                        state.detected_provider = Some(provider);
                    } else {
                        state.prepopulated_from_env = false;
                        state.detected_provider = None;
                    }
                }
            }
            _ => {
                *guard = SignInState::ApiKeyEntry(ApiKeyInputState {
                    value: prefill.as_ref().map(|(_, k)| k.clone()).unwrap_or_default(),
                    prepopulated_from_env: prefill.is_some(),
                    detected_provider: prefill.map(|(p, _)| p),
                });
            }
        }
        drop(guard);
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .sory_home(sory_home_path.clone())
            .build()
            .await
            .unwrap();
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                sory_app_server_client::EnvironmentManager::default_for_tests(),
            ),
