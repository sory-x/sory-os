...
pub(super) async fn test_config() -> Config {
    ...
    let sory_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_sory_home(sory_home.clone(), Vec::new())
            .await
            .expect("config");
    config.sory_home = sory_home.abs();
    config.sqlite_home = sory_home.clone();
    config.log_dir = sory_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::soryOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask sory to do anything".to_string();
...
fn token_usage_breakdown(usage: TokenUsage) -> sory_app_server_protocol::TokenUsageBreakdown {
    sory_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    sory_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: sory_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    sory_error_info: Option<SoryErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = sory_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            sory_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(sory_app_server_protocol::HookEventName, usize)],
    apps: &[(&str, bool)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    sory_app_server_protocol::PluginHookSummary {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                sory_app_server_protocol::HookOutputEntry {
                    kind: sory_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: sory_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        sory_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        sory_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        sory_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        sory_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        sory_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        sory_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        sory_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        sory_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
