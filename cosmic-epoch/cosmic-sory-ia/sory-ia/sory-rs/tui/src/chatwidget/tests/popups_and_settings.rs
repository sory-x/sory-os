...
use sory_app_server_protocol::HookErrorInfo;
    ...
    chat.on_plugin_detail_loaded(
        ...
        Ok(PluginReadResponse {
            plugin: plugins_test_detail(
                ...
                &[
                    (sory_app_server_protocol::HookEventName::PreToolUse, 1),
                    (sory_app_server_protocol::HookEventName::Stop, 2),
                ],
...
async fn realtime_audio_picker_emits_persist_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3-sory")).await;
    chat.open_realtime_audio_device_selection_with_names(
...
async fn server_overloaded_error_does_not_switch_models() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5.3-sory")).await;
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(SoryErrorInfo::ServerOverloaded),
    );
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &sory_feedback::FeedbackDiagnostics::new(vec![sory_feedback::FeedbackDiagnostic {
