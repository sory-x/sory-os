...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        sory_error_info: &AppServersoryErrorInfo,
    ) -> bool {
        matches!(
            sory_error_info,
            AppServersoryErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "sory is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        let rate_limit_reached_type = self.sory_rate_limit_reached_type.map(|kind| {
        ...
        self.sory_rate_limit_reached_type = rate_limit_reached_type;
