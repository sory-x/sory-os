...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServersoryErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServersoryErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServersoryErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServersoryErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServersoryErrorInfo) -> bool {
    matches!(info, AppServersoryErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_sory_limit = limit_id.eq_ignore_ascii_case("sory");
            if is_sory_limit
                && let Some(rate_limit_reached_type) = snapshot.rate_limit_reached_type
            {
                self.sory_rate_limit_reached_type = Some(rate_limit_reached_type);
            }
            let warnings = if is_sory_limit {
            ...
            self.sory_rate_limit_reached_type = None;
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::soryOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using sory. Request increase?",
            ),
        };
