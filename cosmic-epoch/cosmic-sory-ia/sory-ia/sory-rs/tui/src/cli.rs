...
use clap::Parser;
use sory_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `sory resume` subcommand.
    // These are not exposed as user flags on the base `sory` command.
    #[clap(skip)]
    ...
    /// top-level `sory resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `sory fork` subcommand.
    // These are not exposed as user flags on the base `sory` command.
    #[clap(skip)]
    ...
    /// top-level `sory fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
