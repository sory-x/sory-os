...
// The standalone `sory-tui` binary prints a short help message before the
...
use sory_app_server_client::RemoteAppServerClient;
use sory_app_server_client::RemoteAppServerConnectArgs;
pub use sory_app_server_client::RemoteAppServerEndpoint;
use sory_app_server_protocol::Account as AppServerAccount;
use sory_app_server_protocol::AskForApproval;
use sory_app_server_protocol::AuthMode as AppServerAuthMode;
...
use sory_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use sory_exec_server::ExecServerRuntimePaths;
...
use sory_rollout::StateDbHandle;
use sory_rollout::state_db;
use sory_state::log_db;
...
pub(crate) use sory_app_server_client::legacy_core;
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            std::io::Error::other(LocalStateDbStartupError::new(
                sory_state::state_db_path(config.sqlite_home.as_path()),
                err.to_string(),
            ))
        }),
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let sory_home = find_sory_home().wrap_err("failed to resolve sory_HOME")?;
            sory_app_server_client::app_server_control_socket_path(&sory_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(sory_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = sory_app_server_client::app_server_control_socket_path(sory_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_sory_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        sory_feedback::soryFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = sory_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let sory_home = match find_sory_home() {
        Ok(sory_home) => sory_home.to_path_buf(),
        Err(err) => {
    ...
    let remote_endpoint = match explicit_remote_endpoint {
        ...
        None => maybe_probe_default_daemon_socket(&sory_home)
            .await
    ...
    let environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::from_sory_home(sory_home.clone(), local_runtime_paths).await
        } else {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&sory_home, profile_v2);
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&sory_home).await?;
    ...
    // sory CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("sory-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for sory crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("sory_core=info,sory_tui=info,sory_rmcp_client=info")
        })
    };
    ...
    let feedback = sory_feedback::soryFeedback::new();
...
async fn run_ratatui_app(
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !remote_mode {
            cloud_requirements = cloud_requirements_loader_for_storage(
                initial_config.sory_home.to_path_buf(),
                /*enable_sory_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `sory {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_sory_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
...
mod tests {
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: sory_app_server_client::app_server_control_socket_path(&sory_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://sory.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("sory.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        ...
        let socket_path = temp_dir.path().join("sory.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(sory_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let sory_home = TempDir::new()?;
        let socket_path =
            sory_app_server_client::app_server_control_socket_path(sory_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(sory_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        fn write_session_rollout(
            ...
            let session_meta = sory_protocol::protocol::SessionMeta {
                id: thread_id,
                timestamp: meta_rfc3339.to_string(),
                cwd: cwd.to_path_buf(),
                originator: "sory".to_string(),
                cli_version: "0.0.0".to_string(),
                source: sory_protocol::protocol::SessionSource::Cli,
                model_provider: Some(model_provider.to_string()),
                ..Default::default()
            };
            let session_meta = serde_json::to_value(sory_protocol::protocol::SessionMetaLine {
                meta: session_meta,
                git: None,
            })?;
            let lines = [
        ...
        let mut app_server =
            AppServerSession::new(sory_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = sory_state::StateRuntime::init(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            sory_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let sory_home = temp_dir.path().to_path_buf();
