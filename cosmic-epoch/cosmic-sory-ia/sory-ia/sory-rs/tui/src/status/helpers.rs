...
mod tests {
    ...
    async fn test_config(sory_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let sory_home = TempDir::new().expect("temp sory home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = sory_home.path().join(DEFAULT_AGENTS_MD_FILENAME);
        let config = test_config(&sory_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let sory_home = TempDir::new().expect("temp sory home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = sory_home.path().join(LOCAL_AGENTS_MD_FILENAME);
        let config = test_config(&sory_home, &cwd).await;
