...
impl App {
    ...
    pub(super) async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        if windows_sandbox_changed {
            #[cfg(target_os = "windows")]
            {
                ...
                self.app_event_tx
                    .send(AppEvent::soryOp(AppCommand::override_turn_context(
                        /*cwd*/ None,
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("sory-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.sory_home))
        {
...
mod tests {
    ...
    -> Result<()> {
        ...
        let sory_home = tempdir()?;
        app.config.sory_home = sory_home.path().to_path_buf().abs();
        std::fs::write(sory_home.path().join("config.toml"), "[broken")?;
