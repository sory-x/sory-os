    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt { preset } => {
                self.session_telemetry.counter(
                    "sory.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "sory.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let sory_home = self.config.sory_home.clone();
                    let tx = self.app_event_tx.clone();
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "sory.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    match builder.apply().await {
                        Ok(()) => {
                            ...
                            {
                                self.app_event_tx.send(AppEvent::soryOp(
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "sory can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            }
            ...
            AppEvent::PersistProviderSelection { provider_id } => {
                ...
                let edits = vec![crate::config_update::replace_config_value(
                    crate::config_update::profile_scoped_key_path(profile, "model_provider_id"),
                ...
                {
                    Ok(()) => {
                        let mut message = format!("Provider changed to {provider_id}");
                        if let Some(profile) = profile {
                            message.push_str(" for ");
                            message.push_str(profile);
                            message.push_str(" profile");
                        }
                        message.push_str(". Start a new thread to use the new provider.");
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.sory_home.clone();
