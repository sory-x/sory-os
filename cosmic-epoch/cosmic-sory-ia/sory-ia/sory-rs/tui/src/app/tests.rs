...
use sory_app_server_protocol::AdditionalFileSystemPermissions;
use sory_app_server_protocol::AdditionalNetworkPermissions;
use sory_app_server_protocol::AdditionalPermissionProfile;
use sory_app_server_protocol::AgentMessageDeltaNotification;
...
use sory_app_server_protocol::FileUpdateChange;
...
use sory_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use sory_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use sory_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use sory_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use sory_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use sory_app_server_protocol::PatchChangeKind;
use sory_app_server_protocol::PermissionsRequestApprovalParams;
use sory_app_server_protocol::RequestId as AppServerRequestId;
use sory_app_server_protocol::ServerNotification;
use sory_app_server_protocol::ServerRequest;
use sory_app_server_protocol::SessionSource;
...
use sory_app_server_protocol::TokenUsageBreakdown;
...
use sory_app_server_protocol::UserInput as AppServerUserInput;
...
async fn handle_mcp_inventory_result_clears_committed_loading_cell() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: sory_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        feedback: sory_feedback::soryFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::soryOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: sory_app_server_protocol::CollabAgentTool::Wait,
                status: sory_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(sory_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: sory_app_server_protocol::CollabAgentTool::SendInput,
                status: sory_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    sory_app_server_protocol::CollabAgentState {
                        status: sory_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.sory_home = sory_home.path().to_path_buf().abs();
    ...
    let config = std::fs::read_to_string(sory_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let sory_home = tempdir()?;
        app.config.sory_home = sory_home.path().to_path_buf().abs();
        app.config.sqlite_home = sory_home.path().to_path_buf();
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = sory_home.path().join("memories");
...
-> Result<()> {
    ...
    let sory_home = tempdir()?;
    app.config.sory_home = sory_home.path().to_path_buf().abs();
    let config_toml_path = sory_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            sory_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            sory_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            sory_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            sory_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: sory_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: sory_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: sory_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                created_at: 1,
                updated_at: 2,
                status: sory_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: sory_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            sory_app_server_protocol::ServerRequestResolvedNotification {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        sory_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            sory_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        sory_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        sory_error_info: Some(AppServersoryErrorInfo::ActiveTurnNotSteerable {
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    sory_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: sory_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                sory_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
