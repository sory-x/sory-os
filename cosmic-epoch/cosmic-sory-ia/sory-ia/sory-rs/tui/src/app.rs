...
use sory_ansi_escape::ansi_escape_line;
use sory_app_server_client::AppServerRequestHandle;
use sory_app_server_client::RemoteAppServerEndpoint;
...
use sory_app_server_protocol::SkillErrorInfo;
...
use sory_models_manager::model_presets::HIDE_GPT_5_1_sory_MAX_MIGRATION_PROMPT_CONFIG;
use sory_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use sory_protocol::permissions::FileSystemSandboxKind;
use sory_rollout::StateDbHandle;
use sory_terminal_detection::user_agent;
use sory_utils_absolute_path::AbsolutePathBuf;
use sory_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        sory_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&sory_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&sory_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[sory_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&sory_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<sory_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn session_summary(
    ...
    let resume_command = sory_utils_cli::resume_command(/*thread_name*/ None, thread_id);
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.sory_error_info,
        Some(AppServersoryErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    pub async fn run(
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            sory_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("sory.status_line", /*inc*/ 1, &[]);
        }
...
See the sory keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.sory_home.clone();
