...
use sha2::Sha512;
use sory_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "sory-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/sory-backend/agent-identity";
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "sory-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "sory-cli".to_string(),
        },
...
mod tests {
    ...
    use sory_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    #[test]
    fn agent_identity_jwks_url_uses_sory_api_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/sory"),
            "http://localhost:8080/api/sory/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/sory/"),
            "http://localhost:8080/api/sory/agent-identities/jwks"
