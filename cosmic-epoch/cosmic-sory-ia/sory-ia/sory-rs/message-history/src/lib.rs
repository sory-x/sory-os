...
//!
//! The history is stored at `~/.sory/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.sory`.
...
impl HistoryConfig {
    pub fn new(sory_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            sory_home: sory_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.sory_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.sory/history.jsonl` and ensure the parent directory exists.
