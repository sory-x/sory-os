...
//!
//! If the keyring is not available or fails, we fall back to sory_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "sory MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_sory_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    impl TempsoryHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create sory_HOME temp dir");
            unsafe {
                std::env::set_var("sory_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
