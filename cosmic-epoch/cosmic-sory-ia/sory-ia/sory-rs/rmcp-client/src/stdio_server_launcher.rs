...
use sory_exec_server::ExecBackend;
use sory_exec_server::ExecEnvPolicy;
use sory_exec_server::ExecParams;
use sory_exec_server::ExecProcess;
use sory_protocol::config_types::ShellEnvironmentPolicyInherit;
#[cfg(unix)]
use sory_utils_pty::process_group::kill_process_group;
#[cfg(unix)]
use sory_utils_pty::process_group::terminate_process_group;
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from sory. Start from `All` only so the
...
mod tests {
    ...
    use sory_protocol::shell_environment;
