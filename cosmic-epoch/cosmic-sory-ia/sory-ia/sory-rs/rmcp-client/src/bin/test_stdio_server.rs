...
const MEMO_URI: &str = "memo://sory/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "sory/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of sory TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (sory TUI):
    /// - Build this binary: `cargo build -p sory-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `sory mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in sory TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://sory/{slug}".to_string(),
            name: "sory-memo".to_string(),
            title: Some("sory Memo".to_string()),
            description: Some(
                "Template for memo://sory/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise sory TUI handling of MCP image outputs.
///
