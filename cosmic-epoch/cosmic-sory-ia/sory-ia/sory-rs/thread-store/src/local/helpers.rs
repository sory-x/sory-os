...
pub(super) fn rollout_path_is_archived(sory_home: &Path, path: &Path) -> bool {
    path.starts_with(sory_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
