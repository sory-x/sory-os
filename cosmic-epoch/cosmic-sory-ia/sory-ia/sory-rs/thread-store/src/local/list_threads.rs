...
use sory_rollout::find_thread_names_by_ids;
use sory_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => sory_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => sory_rollout::ThreadSortKey::UpdatedAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => sory_rollout::SortDirection::Asc,
        SortDirection::Desc => sory_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        sory_home: store.config.sory_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.sory_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.sory_home.as_path(), &thread_ids).await
    {
...
async fn list_rollout_threads(
    state_db: Option<sory_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&sory_rollout::Cursor>,
    sort_key: sory_rollout::ThreadSortKey,
    sort_direction: sory_rollout::SortDirection,
) -> ThreadStoreResult<sory_rollout::ThreadsPage> {
