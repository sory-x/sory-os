...
pub fn prepare_pending_session_imports(
    ...
    for session in requested_sessions {
        let has_been_imported = match has_current_session_been_imported(sory_home, &session.path) {
...
pub fn prepare_validated_session_imports(
    ...
    requested_sessions
        .into_iter()
        .filter_map(|session| pending_session_import(sory_home, session))
        .collect()
}
...
mod tests {
    ...
    fn rejects_session_that_was_not_detected() {
        ...
        let sory_home = root.path().join("sory-home");
    ...
    fn skips_session_that_was_already_imported() {
        ...
        let source_path = root.path().join("session.jsonl");
        std::fs::write(&source_path, "{}\n").expect("session");
        record_imported_session(&sory_home, &source_path, ThreadId::new()).expect("record import");
