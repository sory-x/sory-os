...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_sory_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_sory_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: sory_protocol::protocol::HookEventName,
...
mod tests {
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.sory/config.toml").abs();
        let dot_sory_folder = test_path_buf("/tmp/worktree/.sory").abs();
        ...
        assert_eq!(
            super::hook_metadata_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.openai.sory".to_string(),
