...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "sory-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(sory_home: &Path) -> PathBuf {
    sandbox_bin_dir(sory_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, sory_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(sory_home: &Path, fallback_executable: &str) -> PathBuf {
    ...
    let destination = helper_bin_dir(sory_home).join(file_name);
    match copy_from_source_if_needed(&source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(sory_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), sory_home.display());
    ...
    let destination = helper_destination_for_source(kind, sory_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(sory_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let sory_home = Path::new(r"C:\Users\example\.sory");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.sory\.sandbox-bin"),
            helper_bin_dir(sory_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("sory-command-runner.exe");
        fs::write(&runner_source, b"runner").expect("runner");
        let runner_suffix = helper_version_suffix(&runner_source).expect("runner suffix");
        let runner_destination = helper_bin_dir(&sory_home).join(materialized_file_name(
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("sory.exe");
        let helper = resources_dir.join("sory-command-runner.exe");
        fs::write(&exe, b"sory").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved = source_path_for_exe(&exe, /*file_name*/ "sory-command-runner.exe")
            .expect("helper path");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("sory.exe");
        let sibling_helper = release_dir.join("sory-command-runner.exe");
        let resource_helper = resources_dir.join("sory-command-runner.exe");
        fs::write(&exe, b"sory").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "sory-command-runner-test-suffix.exe");
    }
}
