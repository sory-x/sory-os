...
pub const OFFLINE_USERNAME: &str = "sorySandboxOffline";
pub const ONLINE_USERNAME: &str = "sorySandboxOnline";
...
pub fn sandbox_dir(sory_home: &Path) -> PathBuf {
    sory_home.join(".sandbox")
}

pub fn sandbox_bin_dir(sory_home: &Path) -> PathBuf {
    sory_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(sory_home: &Path) -> PathBuf {
    sory_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(sory_home: &Path) -> PathBuf {
    sandbox_dir(sory_home).join("setup_marker.json")
}

pub fn sandbox_users_path(sory_home: &Path) -> PathBuf {
    sandbox_secrets_dir(sory_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, sory_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        sory_home: request.sory_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.sory_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.sory_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(sory_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(sory_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(sory_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    gather_legacy_full_read_roots(command_cwd, policy, sory_home)
}
...
pub(crate) fn effective_write_roots_for_setup(
    ...
    filter_sensitive_write_roots(write_roots, sory_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<sory_otel::StatsigMetricsSettings>,
    real_user: String,
    #[serde(default)]
    refresh_only: bool,
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "sory_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    ...
    {
        let candidate = dir.join("sory-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `sory-resources/` next to `sory.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("sory-resources")
            .join("sory-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("sory-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(sory_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(sory_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(sory_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.sory_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: sory_otel::global_statsig_metrics_settings(),
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.sory_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.sory_home);
        ...
        gather_read_roots(request.command_cwd, request.policy, request.sory_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, sory_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to sory_HOME or anything under sory_HOME/.sandbox,
    // sory_HOME/.sandbox-bin, or sory_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let sory_home_key = canonical_path_key(sory_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(sory_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(sory_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(sory_home));
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "sory_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_sory_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let sory_home = user_profile.join("soryHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&sory_home).expect("create sory home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &sory_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let sory_home = tmp.path().join("sory-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &sory_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&sory_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&sory_home);
        fs::create_dir_all(&sory_home).expect("create sory home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_sory_home = dunce::canonicalize(&sory_home).expect("canonical sory home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_sory_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_sory).expect("canonical extra .sory"),
                explicit_deny,
            ]
            .into_iter()
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &sory_home);
