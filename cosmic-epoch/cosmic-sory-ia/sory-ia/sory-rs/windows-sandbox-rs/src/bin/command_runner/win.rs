...
use anyhow::Context;
use anyhow::Result;
use sory_windows_sandbox::ErrorPayload;
use sory_windows_sandbox::ExitPayload;
use sory_windows_sandbox::FramedMessage;
use sory_windows_sandbox::LocalSid;
use sory_windows_sandbox::Message;
use sory_windows_sandbox::OutputPayload;
use sory_windows_sandbox::OutputStream;
use sory_windows_sandbox::PipeSpawnHandles;
use sory_windows_sandbox::ResizePayload;
use sory_windows_sandbox::SandboxPolicy;
use sory_windows_sandbox::SpawnReady;
use sory_windows_sandbox::SpawnRequest;
use sory_windows_sandbox::StderrMode;
use sory_windows_sandbox::StdinMode;
use sory_windows_sandbox::allow_null_device;
use sory_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use sory_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use sory_windows_sandbox::decode_bytes;
use sory_windows_sandbox::encode_bytes;
use sory_windows_sandbox::get_current_token_for_restriction;
use sory_windows_sandbox::hide_current_user_profile_dir;
use sory_windows_sandbox::log_note;
use sory_windows_sandbox::parse_policy;
use sory_windows_sandbox::read_frame;
use sory_windows_sandbox::read_handle_loop;
use sory_windows_sandbox::spawn_process_with_pipes;
use sory_windows_sandbox::to_wide;
use sory_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
const READ_ACL_MUTEX_NAME: &str = "Local\\sorySandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<sory_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.sory_home.clone();
    hide_current_user_profile_dir(req.sory_home.as_path());
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = sory_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
