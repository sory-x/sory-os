...
pub(super) fn ensure_sory_app_runtime_bin_readable(
    ...
    // sory desktop copies bundled Windows binaries out of WindowsApps to this
    // fixed LocalAppData cache before launching sory.exe.
    let runtime_bin_dir = local_app_data.join("OpenAI").join("sory").join("bin");
