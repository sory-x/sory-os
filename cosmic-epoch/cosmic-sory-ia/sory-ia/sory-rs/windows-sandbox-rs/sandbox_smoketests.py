...
# Run a suite of smoke tests against the Windows sandbox via the sory CLI
...
def _resolve_sory_cmd() -> List[str]:
    """Resolve the sory CLI to invoke `sory sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run sory.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "sory.exe",
        ws_root / "target" / "release" / "sory.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "sory.exe",
            cargo_base / "release" / "sory.exe",
        ])
    ...
    if shutil.which("sory"):
        return ["sory"]

    raise FileNotFoundError(
        "sory CLI not found. Build it first, e.g.\n"
        "  cargo build -p sory-cli --release\n"
        "or for debug:\n"
        "  cargo build -p sory-cli\n"
    )

sory_CMD = _resolve_sory_cmd()
print(sory_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to sory CLI overrides.
    ...
    argv = [*sory_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"sory_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\sory_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.sory artifacts) denied
    sory_home = Path(os.environ["USERPROFILE"]) / ".sory"
    cap_sid_target = sory_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .sory\\policy.json"], WS_ROOT)
    add("WS: .sory cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .sory policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > sory_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://sory-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
