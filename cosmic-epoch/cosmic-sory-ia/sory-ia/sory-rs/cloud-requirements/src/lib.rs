//! Cloud-hosted config requirements for sory.
//!
...
//! requirements cannot be loaded for those accounts, sory fails configuration loading rather than
...
const CLOUD_REQUIREMENTS_FETCH_ATTEMPT_METRIC: &str = "sory.cloud_requirements.fetch_attempt";
const CLOUD_REQUIREMENTS_FETCH_FINAL_METRIC: &str = "sory.cloud_requirements.fetch_final";
const CLOUD_REQUIREMENTS_LOAD_METRIC: &str = "sory.cloud_requirements.load";
...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"sory-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
fn auth_identity(auth: &soryAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_requirements_eligible_auth(auth: &soryAuth) -> bool {
    let Some(plan_type) = auth.account_plan_type() else {
        return false;
    };
    auth.uses_sory_backend()
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: sory_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
    ...
    async fn fetch_with_timeout(
        ...
        let _timer =
            sory_otel::start_global_timer("sory.cloud_requirements.fetch.duration_ms", &[]);
...
pub async fn cloud_requirements_loader_for_storage(
    ...
    .await;
    cloud_requirements_loader(auth_manager, chatgpt_base_url, sory_home)
}
...
fn emit_metric(metric_name: &str, tags: Vec<(&str, String)>) {
    if let Some(metrics) = sory_otel::global() {
        let tag_refs = tags
            .iter()
...
mod tests {
    ...
    use sory_config::types::AuthCredentialsStoreMode;
    use sory_login::auth::AgentIdentityAuth;
    use sory_login::auth::AgentIdentityAuthRecord;
    ...
    fn write_auth_json(sory_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(sory_home.join("auth.json"), serde_json::to_string(&value)?)?;
        Ok(())
    }
    ...
    async fn cloud_requirements_eligible_auth_allows_agent_identity_business_plan() {
        ...
        let original_authapi_base_url = std::env::var_os("sory_AGENT_IDENTITY_AUTHAPI_BASE_URL");
        unsafe {
            std::env::set_var("sory_AGENT_IDENTITY_AUTHAPI_BASE_URL", &authapi_base_url);
        }
        let _authapi_guard = EnvVarGuard {
            key: "sory_AGENT_IDENTITY_AUTHAPI_BASE_URL",
            original: original_authapi_base_url,
        };
        let auth = AgentIdentityAuth::load(record)
            .await
            .map(soryAuth::AgentIdentity)
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(sory_config::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        sory_config::AppRequirementToml {
                            enabled: Some(false),
                            tools: None,
                        },
                    )]),
            ...
            Some(ConfigRequirementsToml {
                apps: Some(sory_config::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        ...
                        sory_config::AppRequirementToml {
                            enabled: None,
                            tools: Some(sory_config::AppToolsRequirementsToml {
                                tools: BTreeMap::from([(
                                    "calendar/list_events".to_string(),
                                    sory_config::AppToolRequirementToml {
...
"#,
                ...
                plugins: Some(BTreeMap::from([(
                    ...
                    sory_config::PluginRequirementsToml {
                        mcp_servers: Some(BTreeMap::from([(
                            "sample".to_string(),
                            sory_config::McpServerRequirement {
                                identity: sory_config::McpServerIdentity::Command {
    ...
    async fn fetch_cloud_requirements_recovers_after_unauthorized_reload_updates_cache_identity() {
        ...
        let path = sory_home.path().join(CLOUD_REQUIREMENTS_CACHE_FILENAME);
