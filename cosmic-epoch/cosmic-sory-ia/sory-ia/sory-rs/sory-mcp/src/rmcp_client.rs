...
use crate::sory_apps::CachedsoryAppsToolsLoad;
use crate::sory_apps::soryAppsToolsCacheContext;
use crate::sory_apps::filter_disallowed_sory_apps_tools;
use crate::sory_apps::load_cached_sory_apps_tools;
use crate::sory_apps::load_startup_cached_sory_apps_tools_snapshot;
use crate::sory_apps::normalize_sory_apps_callable_name;
use crate::sory_apps::normalize_sory_apps_callable_namespace;
use crate::sory_apps::normalize_sory_apps_tool_title;
use crate::sory_apps::write_cached_sory_apps_tools_if_needed;
use crate::elicitation::ElicitationRequestManager;
use crate::mcp::SORY_APPS_MCP_SERVER_NAME;
...
use sory_config::McpServerTransportConfig;
use sory_config::types::OAuthCredentialsStoreMode;
use sory_exec_server::HttpClient;
use sory_exec_server::ReqwestHttpClient;
use sory_protocol::protocol::Event;
use sory_rmcp_client::ExecutorStdioServerLauncher;
use sory_rmcp_client::LocalStdioServerLauncher;
use sory_rmcp_client::RmcpClient;
use sory_rmcp_client::StdioServerLauncher;
...
/// MCP server capability indicating that sory should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "sory/sandbox-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "sory.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "sory.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) sory_apps_tools_cache_context: Option<soryAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.sory_apps_tools_cache_context.as_ref()
            && let CachedsoryAppsToolsLoad::Hit(tools) =
                load_cached_sory_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.sory_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        sory_apps_tools_cache_context: Option<soryAppsToolsCacheContext>,
        ...
        let startup_snapshot = load_startup_cached_sory_apps_tools_snapshot(
            &server_name,
            sory_apps_tools_cache_context.as_ref(),
        )
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_sory_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_sory_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_sory_apps_tool_title(server_name, connector_name.as_deref(), title);
        ...
        .collect();
    if server_name == SORY_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_sory_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "sory-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("sory".into()),
            description: None,
            icons: None,
            website_url: None,
        },
    ...
    write_cached_sory_apps_tools_if_needed(
        &server_name,
        sory_apps_tools_cache_context.as_ref(),
        &tools,
    );
...
mod tests {
    ...
    #[test]
    fn sory_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
