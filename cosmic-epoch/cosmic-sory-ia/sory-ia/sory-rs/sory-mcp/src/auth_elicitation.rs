...
//! Session orchestration stays in `sory-core`.

use sory_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_SORY_APPS_META_KEY: &str = "_sory_apps";
...
pub struct soryAppsConnectorAuthFailure {
...
pub struct soryAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct soryAppsAuthElicitationPlan {
    pub auth_failure: soryAppsConnectorAuthFailure,
    pub elicitation: soryAppsAuthElicitation,
}

#[derive(Serialize)]
struct soryAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<soryAppsConnectorAuthFailure> {
    ...
    Some(soryAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<soryAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(soryAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &soryAppsConnectorAuthFailure,
) -> soryAppsAuthElicitation {
    soryAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_SORY_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: soryAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("sory_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &soryAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in sory.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "sory_apps_auth_call_123");
    }
}
