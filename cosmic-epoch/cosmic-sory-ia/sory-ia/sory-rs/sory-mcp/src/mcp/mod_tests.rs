...
fn test_mcp_config(sory_home: PathBuf) -> McpConfig {
    McpConfig {
...
#[test]
fn sory_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
    ...
    assert_eq!(
        sory_apps_mcp_url_for_base_url(
            "http://localhost:8080/api/sory",
            /*apps_mcp_path_override*/ None,
        ),
        "http://localhost:8080/api/sory/apps"
    );
    assert_eq!(
        sory_apps_mcp_url_for_base_url(
            "http://localhost:8080",
...
#[test]
fn sory_apps_mcp_url_uses_legacy_sory_apps_path() {
    let config = test_mcp_config(PathBuf::from("/tmp"));

    assert_eq!(
        sory_apps_mcp_url(&config),
        "https://chatgpt.com/backend-api/wham/apps"
    );
}

#[test]
fn sory_apps_server_config_uses_legacy_sory_apps_path() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    let auth = soryAuth::create_dummy_chatgpt_auth_for_testing();

    let mut servers = with_sory_apps_mcp(HashMap::new(), /*auth*/ None, &config);
    assert!(!servers.contains_key(SORY_APPS_MCP_SERVER_NAME));

    config.apps_enabled = true;

    servers = with_sory_apps_mcp(servers, Some(&auth), &config);
    let server = servers
        .get(SORY_APPS_MCP_SERVER_NAME)
        .expect("sory apps should be present when apps is enabled");
    let config = server
        .configured_config()
        .expect("sory apps should use configured transport");
    let url = match &config.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for sory apps"),
    };
...
#[test]
fn sory_apps_server_config_uses_configured_apps_mcp_path_override() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    config.apps_mcp_path_override = Some("/custom/mcp".to_string());
    config.apps_enabled = true;
    let auth = soryAuth::create_dummy_chatgpt_auth_for_testing();

    let servers = with_sory_apps_mcp(HashMap::new(), Some(&auth), &config);
    let server = servers
        .get(SORY_APPS_MCP_SERVER_NAME)
        .expect("sory apps should be present when apps is enabled");
    let config = server
...
#[test]
fn sory_apps_server_config_forwards_configured_product_sku_header() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
...
#[tokio::test]
async fn effective_mcp_servers_preserve_user_servers_and_add_sory_apps() {
    let sory_home = tempfile::tempdir().expect("tempdir");
    let mut config = test_mcp_config(sory_home.path().to_path_buf());
    config.apps_enabled = true;
    let auth = soryAuth::create_dummy_chatgpt_auth_for_testing();
    ...
    let sory_apps = effective
        .get(SORY_APPS_MCP_SERVER_NAME)
        .expect("sory apps server should exist");
