...
use sory_protocol::mcp::Resource;
use sory_protocol::mcp::ResourceTemplate;
use sory_protocol::mcp::Tool;
...
use crate::sory_apps::sory_apps_tools_cache_key;
use crate::connection_manager::McpConnectionManager;
use crate::runtime::McpRuntimeEnvironment;
use crate::server::EffectiveMcpServer;

pub const SORY_APPS_MCP_SERVER_NAME: &str = "sory_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const SORY_CONNECTORS_TOKEN_ENV_VAR: &str = "sory_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `sory_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `sory-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// sory home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `sory-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn with_sory_apps_mcp(
    mut servers: HashMap<String, EffectiveMcpServer>,
    auth: Option<&soryAuth>,
    config: &McpConfig,
) -> HashMap<String, EffectiveMcpServer> {
    if host_owned_sory_apps_enabled(config, auth) {
        servers.insert(
            SORY_APPS_MCP_SERVER_NAME.to_string(),
            EffectiveMcpServer::configured(sory_apps_mcp_server_config(config)),
        );
    } else {
        servers.remove(SORY_APPS_MCP_SERVER_NAME);
    }
    servers
}

pub fn host_owned_sory_apps_enabled(config: &McpConfig, auth: Option<&soryAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(soryAuth::uses_sory_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    with_sory_apps_mcp(servers, auth, config)
}
...
pub async fn read_mcp_resource(
    ...
    let host_owned_sory_apps_enabled = host_owned_sory_apps_enabled(config, auth);
    ...
    let (manager, cancel_token) = McpConnectionManager::new(
        ...
        sory_apps_tools_cache_key(auth),
...
pub(crate) fn sory_apps_mcp_url(config: &McpConfig) -> String {
    sory_apps_mcp_url_for_base_url(
...
fn sory_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(SORY_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(SORY_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(SORY_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_sory_apps_base_url(base_url: &str) -> String {
...
fn sory_apps_mcp_url_for_base_url(base_url: &str, apps_mcp_path_override: Option<&str>) -> String {
    let base_url = normalize_sory_apps_base_url(base_url);
    let (base_url, default_path) = if base_url.contains("/backend-api") {
        (base_url, "wham/apps")
    } else if base_url.contains("/api/sory") {
        (base_url, "apps")
    } else {
        (format!("{base_url}/api/sory"), "apps")
    };
...
fn sory_apps_mcp_server_config(config: &McpConfig) -> McpServerConfig {
    let url = sory_apps_mcp_url(config);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: sory_apps_mcp_bearer_token_env_var(),
            http_headers,
