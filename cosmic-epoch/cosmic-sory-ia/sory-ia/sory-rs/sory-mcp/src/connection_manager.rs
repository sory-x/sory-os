//! Aggregates MCP server connections for sory.
//!
...
use crate::sory_apps::soryAppsToolsCacheKey;
...
use sory_protocol::protocol::McpStartupCompleteEvent;
use sory_protocol::protocol::McpStartupFailure;
use sory_protocol::protocol::McpStartupStatus;
use sory_protocol::protocol::McpStartupUpdateEvent;
...
pub struct McpConnectionManager {
    ...
    host_owned_sory_apps_enabled: bool,
...
impl McpConnectionManager {
    ...
    pub fn new_uninitialized_with_permission_profile(
        ...
        Self {
            ...
            host_owned_sory_apps_enabled: false,
    ...
    pub fn is_host_owned_sory_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_sory_apps_enabled && server_name == SORY_APPS_MCP_SERVER_NAME
    }
    ...
    pub async fn new(
        ...
        sory_home: PathBuf,
        sory_apps_tools_cache_key: soryAppsToolsCacheKey,
        host_owned_sory_apps_enabled: bool,
        ...
        let sory_apps_auth_provider = auth
            .filter(|auth| auth.uses_sory_backend())
            .map(sory_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            .await;
            let sory_apps_tools_cache_context = if server_name == SORY_APPS_MCP_SERVER_NAME {
                Some(soryAppsToolsCacheContext {
                    sory_home: sory_home.clone(),
                    user_key: sory_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == SORY_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    sory_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    /// Force-refresh sory apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_sory_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(SORY_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{SORY_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{SORY_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_sory_apps_tools_if_needed(
            SORY_APPS_MCP_SERVER_NAME,
            managed_client.sory_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
