...
Set a stable path to the skill CLI (default `sory_HOME` is `~/.sory`):

```
export sory_HOME="${sory_HOME:-$HOME/.sory}"
export IMAGE_GEN="$sory_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/sory-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$sory_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
