#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $sory_HOME/skills."""
...
def _sory_home() -> str:
    return os.environ.get("sory_HOME", os.path.expanduser("~/.sory"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "sory")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "sory-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_sory_home(), "skills")
