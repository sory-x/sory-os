//! Defines the protocol for a sory session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for sory:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::sory_thread::SoryThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Request sory to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down sory instance.
    Shutdown,
...
/// running the command proposed by sory.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.sory`, `.git`, notably `.git/hooks`) under
...
/// sory errors that we expose to clients.
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub sory_error_info: Option<SoryErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.sory_error_info
            .as_ref()
            .is_none_or(SoryErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Sory),
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(sory_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                sory_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_sory() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("sory".to_string()).restriction_product(),
            Some(Product::Sory)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Sory])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Sory]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = sory_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            sory_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".sory")).expect("create .sory");
        let canonical_cwd = sory_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_sory = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".sory"))
            .expect("canonical .sory");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_sory.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_sory = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".sory"))
            .expect("canonical .sory");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
