...
use crate::protocol::SoryErrorInfo;
...
use chrono::Local;
use chrono::Utc;
use sory_async_utils::CancelErr;
use sory_utils_string::truncate_middle_chars;
use sory_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, SoryErr>;
...
pub enum SoryErr {
    ...
    #[error(
        "sory ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `SoryError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use sory with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("sory-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for SoryErr {
...
impl SoryErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | SoryErr::EnvVar(_)
            | SoryErr::Fatal(_)
            | SoryErr::UsageNotIncluded
            | SoryErr::QuotaExceeded
            | SoryErr::InvalidImageRequest()
            | SoryErr::InvalidRequest(_)
            | SoryErr::RefreshTokenFailed(_)
            | SoryErr::UnsupportedOperation(_)
            | SoryErr::Sandbox(_)
            | SoryErr::LandlockSandboxExecutableNotProvided
            | SoryErr::RetryLimit(_)
            | SoryErr::ContextWindowExceeded
            | SoryErr::ThreadNotFound(_)
            | SoryErr::AgentLimitReached { .. }
            | SoryErr::Spawn
            | SoryErr::SessionConfiguredNotFirstEvent
            | SoryErr::UsageLimitReached(_)
            | SoryErr::ServerOverloaded
            | SoryErr::CyberPolicy { .. } => false,
            SoryErr::Stream(..)
            | SoryErr::Timeout
            | SoryErr::UnexpectedStatus(_)
            | SoryErr::ResponseStreamFailed(_)
            | SoryErr::ConnectionFailed(_)
            | SoryErr::InternalServerError
            | SoryErr::InternalAgentDied
            | SoryErr::Io(_)
            | SoryErr::Json(_)
            | SoryErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            SoryErr::LandlockRuleset(_) | SoryErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<SoryErr>()` checks continue to compile
    ...
    pub fn to_sory_protocol_error(&self) -> SoryErrorInfo {
        match self {
            SoryErr::ContextWindowExceeded => SoryErrorInfo::ContextWindowExceeded,
            SoryErr::UsageLimitReached(_)
            | SoryErr::QuotaExceeded
            | SoryErr::UsageNotIncluded => SoryErrorInfo::UsageLimitExceeded,
            SoryErr::ServerOverloaded => SoryErrorInfo::ServerOverloaded,
            SoryErr::CyberPolicy { .. } => SoryErrorInfo::CyberPolicy,
            SoryErr::RetryLimit(_) => SoryErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            SoryErr::ConnectionFailed(_) => SoryErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            SoryErr::ResponseStreamFailed(_) => SoryErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            SoryErr::RefreshTokenFailed(_) => SoryErrorInfo::Unauthorized,
            SoryErr::SessionConfiguredNotFirstEvent
            | SoryErr::InternalServerError
            | SoryErr::InternalAgentDied => SoryErrorInfo::InternalServerError,
            SoryErr::UnsupportedOperation(_)
            | SoryErr::ThreadNotFound(_)
            | SoryErr::AgentLimitReached { .. } => SoryErrorInfo::BadRequest,
            SoryErr::Sandbox(_) => SoryErrorInfo::SandboxError,
            _ => SoryErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            sory_error_info: Some(self.to_sory_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            SoryErr::RetryLimit(err) => Some(err.status),
            SoryErr::UnexpectedStatus(err) => Some(err.status),
            SoryErr::ConnectionFailed(err) => err.source.status(),
            SoryErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("sory")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/sory/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using sory (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/sory/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &SoryErr) -> String {
    let message = match e {
        SoryErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        SoryErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
