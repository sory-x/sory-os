...
fn listen_unix_socket_accepts_absolute_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix:///tmp/sory.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: absolute_path("/tmp/sory.sock")
        })
    );
}
...
fn listen_unix_socket_accepts_relative_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix://sory.sock"),
        Ok(AppServerTransport::UnixSocket {
...
fn default_control_socket_path() -> AbsolutePathBuf {
    ...
    app_server_control_socket_path(&sory_home).expect("default control socket path")
}
