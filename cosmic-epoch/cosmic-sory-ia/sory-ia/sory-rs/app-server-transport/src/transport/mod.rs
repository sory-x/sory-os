...
pub fn app_server_control_socket_path(sory_home: &Path) -> std::io::Result<AbsolutePathBuf> {
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let sory_home = find_sory_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve sory_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&sory_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),
