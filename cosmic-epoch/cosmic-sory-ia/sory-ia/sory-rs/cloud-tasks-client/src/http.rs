...
use sory_backend_client as backend;
use sory_backend_client::CodeTaskDetailsResponseExt;
use sory_git_utils::ApplyGitRequest;
use sory_git_utils::apply_git_patch;
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/sory/tasks/{}", self.base_url, id.0),
            };
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("sory_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
