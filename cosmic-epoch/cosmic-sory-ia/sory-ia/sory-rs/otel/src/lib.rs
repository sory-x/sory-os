...
pub use sory_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on sory-core.
...
impl From<sory_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: sory_app_server_protocol::AuthMode) -> Self {
        match mode {
            sory_app_server_protocol::AuthMode::ApiKey => Self::ApiKey,
            sory_app_server_protocol::AuthMode::Chatgpt
            | sory_app_server_protocol::AuthMode::ChatgptAuthTokens
            | sory_app_server_protocol::AuthMode::AgentIdentity => Self::Chatgpt,
        }
    }
}
