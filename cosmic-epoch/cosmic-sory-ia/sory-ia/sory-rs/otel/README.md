...
`sory-otel` is the OpenTelemetry integration crate for sory. It provides:

- Provider wiring for log/trace/metric exporters (`sory_otel::OtelProvider`
  and `sory_otel::provider`).
- Session-scoped business event emission via `sory_otel::SessionTelemetry`.
- Low-level metrics APIs via `sory_otel::metrics`.
- Trace-context helpers via `sory_otel::trace_context` and crate-root re-exports.
...
```rust
use sory_otel::config::OtelExporter;
use sory_otel::config::OtelHttpProtocol;
use sory_otel::config::OtelSettings;
use sory_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "sory-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    sory_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, sory
...
sory-specific session events. Rich session/business events should go through
...
`sory-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using sory-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use sory_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("sory.session_started", 1, &[("source", "tui")])?;
metrics.histogram("sory.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("sory.turns", 1, &[("model", "gpt-5.1")])?;
