...
pub type StateDbHandle = Arc<sory_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        sory_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .map_err(|err| {
                anyhow::anyhow!(
    ...
    .await;
    sory_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    runtime: &sory_state::StateRuntime,
    sory_home: &Path,
    ...
    loop {
        ...
        if backfill_state.status == sory_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, sory_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = sory_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match sory_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: sory_otel::MetricsClient,
    originator: &str,
) -> sory_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == sory_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<sory_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&sory_state::StateRuntime>,
    sory_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.sory_home() != sory_home {
        warn!(
            "state db sory_home mismatch: expected {}, got {}",
            ctx.sory_home().display(),
            sory_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => sory_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => sory_state::SortKey::UpdatedAt,
            },
...
pub async fn list_threads_db(
    ...
    match ctx
        .list_threads(
            page_size,
            sory_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => sory_state::SortDirection::Asc,
                    SortDirection::Desc => sory_state::SortDirection::Desc,
                },
                search_term,
            },
        )
