...
pub async fn append_thread_name(
    ...
    append_session_index_entry(sory_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(sory_home);
...
fn session_index_path(sory_home: &Path) -> PathBuf {
    sory_home.join(SESSION_INDEX_FILE)
}
