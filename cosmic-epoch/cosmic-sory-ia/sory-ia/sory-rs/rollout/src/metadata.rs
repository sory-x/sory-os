...
use sory_state::BackfillState;
use sory_state::BackfillStats;
use sory_state::BackfillStatus;
use sory_state::DB_ERROR_METRIC;
use sory_state::DB_METRIC_BACKFILL;
use sory_state::DB_METRIC_BACKFILL_DURATION_MS;
use sory_state::ExtractionOutcome;
use sory_state::ThreadMetadataBuilder;
use sory_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = sory_otel::global();
    ...
    let sessions_root = sory_home.join(SESSIONS_SUBDIR);
    let archived_root = sory_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(sory_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(sory_home: &Path, path: &Path) -> String {
    path.strip_prefix(sory_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
