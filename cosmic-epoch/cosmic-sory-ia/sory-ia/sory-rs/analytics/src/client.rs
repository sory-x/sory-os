...
use sory_login::default_client::create_client;
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::soryCompactionEvent) {
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/sory/analytics-events/events");
...
async fn send_track_events_request(auth: &soryAuth, url: &str, events: Vec<TrackEventRequest>) {
    if events.is_empty() {
        return;
    }
