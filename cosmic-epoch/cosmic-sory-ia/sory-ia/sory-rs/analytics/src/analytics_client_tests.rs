...
use crate::events::soryAppMentionedEventRequest;
use crate::events::soryAppServerClientMetadata;
use crate::events::soryAppUsedEventRequest;
use crate::events::soryCommandExecutionEventParams;
use crate::events::soryCommandExecutionEventRequest;
use crate::events::soryCompactionEventRequest;
use crate::events::soryHookRunEventRequest;
use crate::events::soryPluginEventRequest;
use crate::events::soryPluginUsedEventRequest;
use crate::events::soryReviewEventParams;
use crate::events::soryReviewEventRequest;
use crate::events::soryRuntimeMetadata;
use crate::events::soryToolItemEventBase;
use crate::events::soryTurnEventRequest;
...
use crate::events::sory_app_metadata;
use crate::events::sory_hook_run_metadata;
use crate::events::sory_plugin_metadata;
use crate::events::sory_plugin_used_metadata;
...
use crate::facts::soryCompactionEvent;
...
use sory_app_server_protocol::CollabAgentTool;
use sory_app_server_protocol::CollabAgentToolCallStatus;
use sory_app_server_protocol::CommandAction;
...
use sory_app_server_protocol::GuardianApprovalReview;
use sory_app_server_protocol::GuardianApprovalReviewAction;
use sory_app_server_protocol::GuardianApprovalReviewStatus;
use sory_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use sory_app_server_protocol::InitializeCapabilities;
use sory_app_server_protocol::InitializeParams;
use sory_app_server_protocol::ItemCompletedNotification;
use sory_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use sory_app_server_protocol::ItemStartedNotification;
use sory_app_server_protocol::JSONRPCErrorError;
use sory_app_server_protocol::McpToolCallStatus;
use sory_app_server_protocol::NonSteerableTurnKind;
use sory_app_server_protocol::PatchApplyStatus;
use sory_app_server_protocol::PermissionsRequestApprovalParams;
use sory_app_server_protocol::RequestId;
use sory_app_server_protocol::RequestPermissionProfile;
...
use sory_app_server_protocol::ServerResponse;
...
use sory_app_server_protocol::ThreadSource as AppServerThreadSource;
...
use sory_app_server_protocol::TurnError as AppServerTurnError;
...
use sory_app_server_protocol::UserInput;
use sory_login::default_client::DEFAULT_ORIGINATOR;
use sory_login::default_client::originator;
use sory_plugin::AppConnectorId;
use sory_plugin::PluginCapabilitySummary;
use sory_plugin::PluginId;
use sory_plugin::PluginTelemetryMetadata;
use sory_protocol::approvals::NetworkApprovalProtocol;
...
use sory_protocol::models::PermissionProfile as CorePermissionProfile;
use sory_protocol::protocol::AskForApproval;
use sory_protocol::protocol::HookEventName;
use sory_protocol::protocol::HookRunStatus;
use sory_protocol::protocol::HookSource;
...
fn sample_app_server_client_metadata() -> soryAppServerClientMetadata {
    soryAppServerClientMetadata {
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        client_name: Some("sory-tui".to_string()),
...
fn sample_runtime_metadata() -> soryRuntimeMetadata {
    soryRuntimeMetadata {
        sory_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(sory_app_server_protocol::TurnStartResponse {
        turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    sory_error_info: Option<sory_app_server_protocol::SoryErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: sory_error_info.map(|sory_error_info| AppServerTurnError {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "sory-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "sory-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "sory-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "sory-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        runtime: soryRuntimeMetadata {
            sory_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(sory_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: sory_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.sory/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".sory/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.sory/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/sory/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.sory/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(soryAppMentionedEventRequest {
        event_type: "sory_app_mentioned",
        event_params: sory_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(soryAppUsedEventRequest {
        event_type: "sory_app_used",
        event_params: sory_app_metadata(
            &tracking,
            AppInvocation {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "sory",
                "model_slug": "gpt-5.1-sory",
                "completed_at": 1710000000,
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(soryCompactionEventRequest {
        event_type: "sory_compaction_event",
        event_params: crate::events::sory_compaction_event_params(
            soryCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "sory-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "sory_rs_version": "0.1.0",
                    "runtime_os": "macos",
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "sory_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            app_server_client: soryAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("sory-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(soryCommandExecutionEventRequest {
        event_type: "sory_command_execution_event",
        event_params: soryCommandExecutionEventParams {
            base: soryToolItemEventBase {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: soryAppServerClientMetadata {
                    product_client_id: "sory_tui".to_string(),
                    client_name: Some("sory-tui".to_string()),
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(soryReviewEventRequest {
        event_type: "sory_review_event",
        event_params: soryReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "sory_thread_initialized");
    assert_eq!(
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["sory_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        sory_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sory_compaction_event");
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sory_guardian_review");
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sory_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sory_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "sory_review_event");
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            parent_thread_id: None,
            product_client_id: "sory-tui".to_string(),
            client_name: "sory-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_parent_thread_id() {
    let parent_thread_id =
        sory_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
...
async fn subagent_thread_started_inherits_parent_connection_for_new_thread() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        sory_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(soryPluginUsedEventRequest {
        event_type: "sory_plugin_used",
        event_params: sory_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(soryPluginEventRequest {
        event_type: "sory_plugin_installed",
        event_params: sory_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_management_event_can_use_remote_plugin_id_override() {
    ...
    let event = TrackEventRequest::PluginInstalled(soryPluginEventRequest {
        event_type: "sory_plugin_installed",
        event_params: sory_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(soryHookRunEventRequest {
        event_type: "sory_hook_run",
        event_params: sory_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(sory_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(sory_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(sory_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(sory_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(sory_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: sory_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_includes_plugin_id_for_plugin_skill_invocations() {
    ...
    let skill_path =
        PathBuf::from("/Users/abc/.sory/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sory_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "sory_app_mentioned");
    assert_eq!(payload[1]["event_type"], "sory_app_used");
    assert_eq!(payload[2]["event_type"], "sory_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "sory_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "marketplace_name": "test",
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(soryTurnEventRequest {
        event_type: "sory_turn_event",
        event_params: crate::events::soryTurnEventParams {
            thread_id: "thread-2".to_string(),
            turn_id: "turn-2".to_string(),
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("sory_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["sory_rs_version"],
        json!("0.1.0")
    );
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(sory_app_server_protocol::SoryErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
