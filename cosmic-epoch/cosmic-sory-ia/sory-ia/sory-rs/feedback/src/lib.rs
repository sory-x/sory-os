...
/// Filename used for the redacted `sory doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "sory-doctor-report.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_sory_api_key_present = tracing::field::debug(auth_env.sory_api_key_env_present),
        auth_env_sory_api_key_enabled = tracing::field::debug(auth_env.sory_api_key_env_enabled),
...
#[derive(Clone)]
pub struct soryFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for soryFeedback {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `sory-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("sory-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("sory-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = soryFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("sory-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = soryFeedback::new()
        ...
        let attachments_without_diagnostics = soryFeedback::new()
            .snapshot(/*session_id*/ None)
