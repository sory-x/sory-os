...
use crate::environment_toml::environment_provider_from_sory_home;
...
pub const SORY_EXEC_SERVER_URL_ENV_VAR: &str = "sory_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the sory runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `sory_EXEC_SERVER_URL` behavior
/// while provider-based construction accepts a provider-supplied snapshot.
///
/// Setting `sory_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `sory_EXEC_SERVER_URL` and local runtime paths
    /// used when creating local filesystem helpers.
    pub async fn new(args: EnvironmentManagerArgs) -> Self {
        let EnvironmentManagerArgs {
            local_runtime_paths,
        } = args;
        let exec_server_url = std::env::var(SORY_EXEC_SERVER_URL_ENV_VAR).ok();
        Self::from_default_provider_url(exec_server_url, local_runtime_paths).await
    }

    /// Builds a manager from `sory_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `sory_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `sory_EXEC_SERVER_URL` behavior.
    pub async fn from_sory_home(
        sory_home: impl AsRef<std::path::Path>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
        let provider = environment_provider_from_sory_home(sory_home.as_ref())?;
    ...
    /// reading user config files from `sory_HOME`.
...
impl Environment {
    /// Builds an environment from the raw `sory_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `sory_EXEC_SERVER_URL` value and
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = sory_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            sory_protocol::models::PermissionProfile::from_runtime_permissions(
                &sory_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                sory_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
