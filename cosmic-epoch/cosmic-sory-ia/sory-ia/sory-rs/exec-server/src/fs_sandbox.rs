...
use sory_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::sory_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.sory_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![sory_FS_HELPER_ARG1.to_string()],
            cwd: cwd.clone(),
        ...
        sandbox_manager
            .transform(SandboxTransformRequest {
                ...
                sory_linux_sandbox_exe: self.runtime_paths.sory_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.sory_self_exe.as_path())
        .chain(runtime_paths.sory_linux_sandbox_exe.as_deref().into_iter())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let sory_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(sory_self_exe, /*sory_linux_sandbox_exe*/ None)
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/sory".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let sory_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(sory_self_exe.clone(), Some(sory_self_exe))
                .expect("runtime paths");
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let sory_self_exe = root.path().join("bin").join("sory");
        let sory_linux_sandbox_exe = root.path().join("aliases").join("sory-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(sory_self_exe, Some(sory_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let sory_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute sory parent");
        ...
        assert!(policy.can_read_path_with_cwd(sory_parent.as_path(), cwd.as_path()));
