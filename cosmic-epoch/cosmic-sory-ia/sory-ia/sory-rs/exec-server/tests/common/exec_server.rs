...
pub(crate) struct ExecServerHarness {
    _sory_home: TempDir,
    _helper_paths: TestsoryHelperPaths,
    child: Child,
...
pub(crate) struct TestsoryHelperPaths {
    pub(crate) sory_exe: PathBuf,
    pub(crate) sory_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_sory_helper_paths() -> anyhow::Result<TestsoryHelperPaths> {
    let (helper_binary, sory_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestsoryHelperPaths {
        sory_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.sory_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("sory_HOME", sory_home.path());
    child.envs(env);
