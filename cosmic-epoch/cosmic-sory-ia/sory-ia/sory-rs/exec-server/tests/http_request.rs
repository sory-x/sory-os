...
use sory_exec_server::HttpHeader;
use sory_exec_server::HttpRequestBodyDeltaNotification;
use sory_exec_server::HttpRequestParams;
use sory_exec_server::HttpRequestResponse;
...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
    ...
    assert_eq!(
        (
            ...
            captured.headers.get("x-sory-test").map(String::as_str),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<sory_app_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
