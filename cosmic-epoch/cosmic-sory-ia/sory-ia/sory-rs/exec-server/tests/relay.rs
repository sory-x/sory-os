...
#[path = "../src/proto/sory.exec_server.relay.v1.rs"]
mod relay_proto;
...
use sory_exec_server::InitializeParams;
use sory_exec_server::InitializeResponse;
use sory_exec_server::RemoteExecutorConfig;
use std::sync::Arc;
...
fn static_registry_auth_provider() -> sory_api::SharedAuthProvider {
...
async fn multiplexed_remote_executor_routes_independent_virtual_streams() -> Result<()> {
    ...
    let (sory_exe, sory_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(sory_exe, sory_linux_sandbox_exe)?;
    ...
    let remote_executor =
        tokio::spawn(sory_exec_server::run_remote_executor(config, runtime_paths));
