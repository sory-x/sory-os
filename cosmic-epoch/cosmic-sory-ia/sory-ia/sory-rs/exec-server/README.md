...
`sory-exec-server` is the library backing `sory exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`sory-utils-pty`.

It provides:

- a CLI entrypoint: `sory exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `sory` binary owns hidden helper dispatch for sandboxed
filesystem operations and `sory-linux-sandbox`.

## Transport

The server speaks the shared `sory-app-server-protocol` message envelope on
the wire.
...
It uses the standard sory ChatGPT sign-in state; run `sory login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `sory_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; sory then registers an Agent task and sends the
...
`sory.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/sory.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `sory-utils-pty`.
...
hidden helper process launched from the top-level `sory` executable and
...
`sory exec-server` command builds these paths from the `sory` arg0 dispatch
state. `RemoteExecutorConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from sory auth
