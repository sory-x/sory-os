...
impl<B: MemoriesBackend> ServerHandler for MemoriesMcpServer<B> {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some(
                "Use these tools to list, read, and search sory memory files.".to_string(),
            ),
...
pub async fn run_server<T, E, A>(sory_home: &AbsolutePathBuf, transport: T) -> anyhow::Result<()>
where
    T: rmcp::transport::IntoTransport<rmcp::RoleServer, E, A>,
    E: std::error::Error + Send + Sync + 'static,
{
    let backend = LocalMemoriesBackend::from_sory_home(sory_home);
...
pub async fn run_stdio_server(sory_home: &AbsolutePathBuf) -> anyhow::Result<()> {
    run_server(sory_home, (tokio::io::stdin(), tokio::io::stdout())).await
}
...
fn list_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "List immediate files and directories under a path in the sory memories store.",
        ),
...
fn read_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "Read a sory memory file by relative path, optionally starting at a 1-indexed line offset and limiting the number of lines returned.",
        ),
...
fn search_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "Search sory memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        ),
