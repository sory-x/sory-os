...
use sory_rollout::INTERACTIVE_SESSION_SOURCES;
use sory_rollout::should_persist_response_item_for_memories;
use sory_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<sory_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            sory_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<sory_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: sory_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> sory_protocol::error::Result<String> {
        let filtered = items
            .iter()
            .filter_map(|item| {
        ...
        let serialized = serde_json::to_string(&filtered).map_err(|err| {
            SoryErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })?;
        ...
        let serialized =
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: sory_protocol::models::FunctionCallOutputPayload {
                        body: sory_protocol::models::FunctionCallOutputBody::Text(
