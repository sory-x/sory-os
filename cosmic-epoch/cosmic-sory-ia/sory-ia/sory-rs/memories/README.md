...
Runtime orchestration for Phase 1 and Phase 2 still lives in `sory-core` under
`sory-rs/core/src/memories/`.

## Crates

- `sory-rs/memories/read` (`sory-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `sory-rs/memories/mcp` (`sory-memories-mcp`) owns the read-only memory
  filesystem MCP server implementation.
- `sory-rs/memories/write` (`sory-memories-write`) owns the write path:
...
- In `sory`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.sory/memories/.git` by `sory-git-utils`
